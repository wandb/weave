...
def test_optimize_merge_empty_dict():
    ...
    assert (
        compile.compile_simple_optimizations(
            [weave_query.weave_query.ops.TypedDict.merge(non_empty_dict, weave_query.weave_query.ops.dict_())]
        )[0].to_json()
        == non_empty_dict.to_json()
    )
    assert (
        compile.compile_simple_optimizations(
            [weave_query.weave_query.ops.TypedDict.merge(weave_query.weave_query.ops.dict_(), non_empty_dict)]
        )[0].to_json()
        == non_empty_dict.to_json()
    )
...
def test_compile_lambda_uniqueness():
    list_node_1 = weave_query.weave_query.ops.make_list(a=make_const_node(weave.types.Number(), 1))
    list_node_2 = weave_query.weave_query.ops.make_list(a=make_const_node(weave.types.Number(), 2))
...
def test_compile_through_function_call(user_by_api_key_in_env):
    ...
    fn_node = define_fn(
        ...
        lambda entity_name: (
            weave_query.weave_query.ops.project(entity_name, run.project).run(run.id).history2()
        ),
    )
