...
import weave_query
from weave_query import async_demo, compile, graph
from weave_query import weave_types as types
from weave_query.api import use
from weave_query.dispatch import RuntimeOutputNode
from weave_query.ops_arrow import to_arrow
from weave_query.ops_arrow.vectorize import raise_on_python_bailout
from weave_query.wandb_interface.wandb_stream_table import StreamTable
from weave_query.weave_internal import const, define_fn, make_const_node
...
def test_optimize_merge_empty_dict():
    non_empty_dict = weave_query.ops.dict_(a=5, b=2)
    assert (
        compile.compile_simple_optimizations(
            [weave_query.ops.TypedDict.merge(non_empty_dict, weave_query.ops.dict_())]
        )[0].to_json()
        == non_empty_dict.to_json()
    )
    assert (
        compile.compile_simple_optimizations(
            [weave_query.ops.TypedDict.merge(weave_query.ops.dict_(), non_empty_dict)]
        )[0].to_json()
        == non_empty_dict.to_json()
    )
    non_simplified_merge = weave_query.ops.TypedDict.merge(
        weave_query.ops.dict_(j=3), non_empty_dict
    )
    assert (
...
def test_compile_lambda_uniqueness():
    list_node_1 = weave_query.ops.make_list(a=make_const_node(weave.types.Number(), 1))
    list_node_2 = weave_query.ops.make_list(a=make_const_node(weave.types.Number(), 2))
    ...
    combined = weave_query.ops.make_list(a=mapped_1, b=mapped_2)
...
#     """
#     history_node = weave_query.ops.project(run.entity, run.project).run(run.id).history2()
...
def test_compile_through_function_call(user_by_api_key_in_env):
    ...
    fn_node = define_fn(
        ...
        lambda entity_name: (
            weave_query.ops.project(entity_name, run.project).run(run.id).history2()
        ),
    )
...
def test_compile_list_flatten_to_awl_concat():
    ...
    # list of lists
    list_list_node = weave_query.ops.make_list(a=[1], b=[2])
    ...
    # list of awls
    list_awl_node = weave_query.ops.make_list(a=to_arrow([1]), b=to_arrow([2]))
