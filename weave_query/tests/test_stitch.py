...
def test_traverse_dict():
    obj_node = weave.save(_TestPlanObject("a", 1))
    p = stitch.stitch([weave.weave_query.ops.dict_(x=obj_node)["x"].horse()])
    obj_recorder = p.get_result(obj_node)
...
def test_travese_groupby_dict():
    ...
    grouped = obj_node.groupby(lambda row: weave.weave_query.ops.dict_(x=row["o"]))
    output = grouped[0]["x"]
...
def test_shared_fn_node():
    const_list_node = weave.weave_query.ops.make_list(a=1, b=2)
    indexed_node = const_list_node[0]
    arr_1_node = weave.weave_query.ops.make_list(a=1, b=2, c=3)
    arr_2_node = weave.weave_query.ops.make_list(a=10, b=20, c=30)

    mapped_1_node = arr_1_node.map(
        lambda row: weave.weave_query.ops.dict_(item=row, const=indexed_node)
    )
    ...
    list_of_list_node = weave.weave_query.ops.make_list(
...
def test_stitch_keytypes_override_fetch_all_columns(fake_wandb):
    ...
    keytypes_node = weave.weave_query.ops.object_keytypes(
        run_ops.run_tag_getter_op(
            weave.weave_query.ops.project("stacey", "mendeleev")
...
def test_refine_history_type_included_in_gql():
    ...
    map_node = runs_node.map(lambda row: weave.weave_query.ops.dict_(variant=row))
...
def test_stitch_missing_key():
    ...
    dict_node = weave.weave_query.ops.dict_(a=a_node)
