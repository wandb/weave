...
def use_static_artifact_node(
    ...
    return weave_query.weave_query.ops.project(
        entity_name, project_name
    ).artifactVersion(collection_name, version)
...
def test_metric_table_join(fake_wandb):
    ...
    grouped = table_rows.groupby(
        lambda row: weave_query.weave_query.ops.dict_(label=row["label"])
    )
    sorted = grouped.sort(
        lambda row: weave_query.weave_query.ops.make_list(a=row.groupkey()["label"]),
        ["asc"],
    )
...
def test_join_group_combo(fake_wandb):
    ...
    list_of_tables = weave_query.weave_query.ops.make_list(
    ...
    grouped = indexed.groupby(
        lambda row: weave_query.weave_query.ops.dict_(label=row["label"][0])
    )
    sorted = grouped.sort(
        lambda row: weave_query.weave_query.ops.make_list(
            label=row.groupkey()["label"]
        ),
        ["asc"],
    )
...
def test_group_avg_sort_combo(fake_wandb):
    ...
    sorted = grouped.sort(
        lambda row: weave_query.weave_query.ops.make_list(label=row["score"].avg()),
        ["asc"],
    )
    assert weave.use(sorted[2].groupkey()["label"]) == "C"
