...
def test_stream_logging(user_by_api_key_in_env):
    ...
    hist_node = (
        weave_query.weave_query.ops.project(user_by_api_key_in_env.username, "stream-tables")
        .run("test_table")
        .history3()
    )
...
def test_multi_writers_sequential(user_by_api_key_in_env):
    ...
    def do_asserts():
        hist_node = (
            weave_query.weave_query.ops.project(
...
def test_multi_writers_parallel(user_by_api_key_in_env):
    ...
    hist_node = (
        weave_query.weave_query.ops.project(entity_name, "stream-tables")
        .run(table_name)
        .history2()
    )
