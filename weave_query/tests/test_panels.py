...
import weave_query
from weave_query import storage, weave_internal
from weave_query.panels import panel_plot

from weave_query.panels.panel_group import Group
from weave_query.panels.panel_slider import Slider
...
def test_object_picker_choice_type():
    ...
    panel = weave_query.panels.ObjectPicker(ints)
...
def test_facet_selected():
    ...
    facet = weave_query.panels.Group(
        equalSize=True,
        items={
...
def test_board():
    ...
    weave_query.panels.Board(
        {
            "nums": weave_query.ops.range(0, 3, 1),
        },
        [weave_query.panels.BoardPanel(id="panel0", panel=lambda nums: nums)],
    )
...
def test_plot_assign():
    assert weave_query.panels.Plot.WeaveType().assign_type(
        weave.type_of(weave_query.panels.Plot([{"a": 5}]))
    )
