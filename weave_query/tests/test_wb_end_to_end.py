...
def test_run_logging(user_by_api_key_in_env):
    ...
    summary_node = (
        weave_query.weave_query.ops.project(run.entity, run.project).run(run.id).summary()["a"]
    )
...
def test_run_histories(user_by_api_key_in_env):
    ...
    history_node = (
        weave_query.weave_query.ops.project(run.entity, run.project).runs().history().concat()["a"]
    )
