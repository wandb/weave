...
def test_merge():
    ...
    weave_query.weave_query.ops.set(
        dict_obj["a"], 17, root_args={"branch": "my-branch"}
    )
    ...
    assert (
        ...
        == weave.use(
            weave_query.weave_query.ops.get("local-artifact:///my-dict:latest/obj")
        )
        == {"a": 17, "b": 6}
    )
...
def test_artifact_history_local():
    ...
    for i in range(num_versions):
        new_uri = weave_query.weave_query.ops.undo_artifact(
            weave_query.weave_query.ops.get(uri)
        )
...
def test_artifact_history_remote_with_branch(user_by_api_key_in_env):
    ...
    weave.save([0], "art:main")
    art = weave_query.weave_query.ops.get(uri)
    published_art_uri = weave_query.weave_query.ops.publish_artifact(
        art, "art", None, None
    )
