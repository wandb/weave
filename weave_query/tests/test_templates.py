...
def dummy_board(
    ...
    return weave.weave_query.panels.Board(vars=control_items, panels=[])
...
def _assert_valid_node_raw(node):
    ...
    assert isinstance(output_group, weave.weave_query.panels.Group)
    ...
    assert isinstance(data_node, weave.weave_query.graph.OutputNode)
...
def test_templates_for_run_logs_valid(user_by_api_key_in_env):
    ...
    run_history_node = (
        weave.weave_query.ops.project(run.entity, run.project).run(run.id).history()
    )
