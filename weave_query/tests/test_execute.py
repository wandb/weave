...
import weave_query
from weave_query import api, environment, execute, ops, weave_internal
...
def test_cache_column():
    ...
    mapped = l.map(lambda x: weave_query.ops.dict_(x=x, y=expensive_op(x)))
    res = weave.use(mapped)
    assert res == expected_result

    latest_obj = weave.use(
        weave_query.ops.get("local-artifact:///run-op-expensive_op:latest/obj")
    )
