...
class ShapValues:
    ...
    def summary_plot(self):
        ...
        plt.savefig(path)
        plt.close()
        return weave.weave_query.ops.LocalFile(path)
...
@weave.op()
def shap_plot_text(shap_values: shap.Explanation) -> weave.weave_query.ops.Html:
...
class ShapPlotText(weave.Panel):
    ...
    @weave.op()
    def render(self) -> weave.weave_query.panels.PanelHtml:
        return weave.weave_query.panels.PanelHtml(shap_plot_text(self.input_node))
