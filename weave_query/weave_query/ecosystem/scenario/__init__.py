...
class MetricsBankPanel(weave.Panel):
    ...
    @weave.op()
    def render(self) -> weave.weave_query.panels.Each:
        input = typing.cast(MetricsBankInput, self.input_node)

        baseline = input["baseline"]
        candidate = input["candidate"]

        joined = weave.weave_query.ops.join_all(
            weave.weave_query.ops.make_list(l0=baseline, l1=candidate),
        ...
        metrics = weave.weave_query.ops.difference(
            joined_keys, [val_const.const("scenario_id")]
        )

        # TODO: broken
        return weave.weave_query.panels.Each(
            metrics,
            render=lambda metric_name: weave.weave_query.panels.Group(
                items={
                    "title": metric_name,
