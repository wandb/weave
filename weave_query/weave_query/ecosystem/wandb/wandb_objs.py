...
GHOSTWRITE_MD = weave_query.ops.Markdown(
    """
    ...
    return weave_query.panels.Card(
        ...
        content=[
            weave_query.panels.CardTab(
                name="Description",
                content=weave_query.panels.PanelMarkdown(GHOSTWRITE_MD),  # type: ignore
            ),
            weave_query.panels.CardTab(
                name="Predictions",
                content=weave_query.ops.project("shawn", "ghostwrite-test1")
                .runs()
                .summary()["predictions"]
                .table()
                .rows()
        ...
        content=[
            ...
            weave_query.panels.CardTab(
                ...
                content=weave_query.panels.Table(
                    ...
                    columns=[
                        lambda model_name: weave_query.panels.WeaveLink(
                            model_name,
                            vars={
...
class ProjectRunsTable(weave.Panel):
    ...
    def render(self) -> weave_query.panels.Table:
        return weave_query.panels.Table(
            self.input_node,
            columns=[
                lambda run: weave_query.panels.WeaveLink(
                    run.id(),
                    vars={
...
class ProjectArtifactsTable(weave.Panel):
    ...
    def render(self) -> weave_query.panels.Table:
        ...
        #     subtitle="",
        #     content=[
        #         weave_query.panels.CardTab(name=type_name, content=[type_name])
        ...
        return weave_query.panels.Table(
            self.input_node,
            columns=[
                lambda artifact: weave_query.panels.WeaveLink(
