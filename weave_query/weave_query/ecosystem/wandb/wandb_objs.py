...
GHOSTWRITE_MD = weave.weave_query.ops.Markdown(
    """
    ...
    return weave.weave_query.panels.Card(
        ...
        content=[
            weave.weave_query.panels.CardTab(
                name="Description",
                content=weave.weave_query.panels.PanelMarkdown(GHOSTWRITE_MD),  # type: ignore
            ),
            weave.weave_query.panels.CardTab(
                name="Predictions",
                content=weave.weave_query.ops.project("shawn", "ghostwrite-test1")
                .runs()
                .summary()["predictions"]
                .table()
                .rows()
        ...
        content=[
            ...
            weave.weave_query.panels.CardTab(
                ...
                content=weave.weave_query.panels.Table(
                    ...
                    columns=[
                        lambda model_name: weave.weave_query.panels.WeaveLink(
                            model_name,
                            vars={
...
class ProjectRunsTable(weave.Panel):
    ...
    def render(self) -> weave.weave_query.panels.Table:
        return weave.weave_query.panels.Table(
            self.input_node,
            columns=[
                lambda run: weave.weave_query.panels.WeaveLink(
                    run.id(),
                    vars={
...
class ProjectArtifactsTable(weave.Panel):
    ...
    def render(self) -> weave.weave_query.panels.Table:
        ...
        #     subtitle="",
        #     content=[
        #         weave.weave_query.panels.CardTab(name=type_name, content=[type_name])
        ...
        return weave.weave_query.panels.Table(
            self.input_node,
            columns=[
                lambda artifact: weave.weave_query.panels.WeaveLink(
