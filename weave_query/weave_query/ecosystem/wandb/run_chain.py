...
import weave_query
from weave_query import compile, op_def, ops_arrow
from weave_query.ecosystem.wandb import gql_artifact_dag
...
class RunChain:
    ...
    def _history_node(self):
        with op_def.no_refine():
            proj = weave_query.ops.project(self.entity_name, self.project_name)
            history_nodes = []
            ...
            history_node = weave_query.ops.List.concat(
...
def run_chain(run_path: str) -> RunChain:
    with op_def.no_refine():
        ...
        proj = weave_query.ops.project(entity, project)
        run = proj.run(run_id)
