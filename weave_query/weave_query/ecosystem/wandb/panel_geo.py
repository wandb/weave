...
def geo(
    ...
    plot_data = unnested.map(
        lambda item: weave.weave_query.ops.dict_(
...
def geo_config(
    ...
    return weave.weave_query.panels.Group(
        items={
            "x_fn": weave.weave_query.panels.LabeledItem(
                label="x",
                item=weave.weave_query.panels.FunctionEditor(
                    config=weave.weave_query.panels.FunctionEditorConfig(config.x_fn)
                ),
            ),
            "y_fn": weave.weave_query.panels.LabeledItem(
                label="y",
                item=weave.weave_query.panels.FunctionEditor(
                    config=weave.weave_query.panels.FunctionEditorConfig(config.y_fn)
                ),
            ),
            "color_fn": weave.weave_query.panels.LabeledItem(
                label="color",
                item=weave.weave_query.panels.FunctionEditor(
                    config=weave.weave_query.panels.FunctionEditorConfig(
                        config.color_fn
                    )
                ),
            ),
...
class Geo(weave.Panel):
    ...
    def __init__(
        ...
        if self.config is None:
            ...
            unnested = weave.weave_query.ops.unnest(self.input_node)
            if "x_fn" in options:
        ...
        filtered = unnested.filter(
            lambda item: weave.weave_query.ops.Boolean.bool_and(
                weave.weave_query.ops.Boolean.bool_and(
                    weave.weave_query.ops.Boolean.bool_and(
                        config.x_fn(item)
                        > weave.weave_query.ops.TypedDict.pick(
