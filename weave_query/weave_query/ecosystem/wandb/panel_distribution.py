...
class Distribution(weave.Panel):
    ...
    def render_config(self) -> weave_query.panels.Group:
        ...
        return weave_query.panels.Group(
            items={
                "value_fn": weave_query.panels.LabeledItem(
                    label="value",
                    item=weave_query.panels.FunctionEditor(config.value_fn),
                ),
                "label_fn": weave_query.panels.LabeledItem(
                    label="label",
                    item=weave_query.panels.FunctionEditor(config.label_fn),
                ),
                "bin_size": weave_query.panels.LabeledItem(
                    label="bin_size",
    ...
    def render(self) -> weave_plotly.PanelPlotly:
        ...
        if not weave.types.union(
            ...
            return weave_query.panels.PanelString("Invalid value_fn")  # type: ignore
...
def distribution_panel_plot_render(
    ...
    unnested = weave_query.ops.unnest(input_node)
    bin_size = weave_query.ops.execute(config.bin_size)
    ...
    def bin_func(item):
        ...
        res = weave_query.ops.dict_(**group_items)
        return res
