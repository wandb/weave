...
class ClassificationResultPanel(weave.Panel):
    ...
    def render(self) -> weave_query.panels.Table:
        from weave_query.ecosystem.huggingface.huggingface_models import huggingface

        return weave_query.panels.Table(
            self.input_node,
            columns=[
                lambda result_row: weave_query.panels.WeaveLink(
