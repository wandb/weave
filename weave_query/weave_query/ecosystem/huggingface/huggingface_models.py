...
class HuggingfaceModelsPanel(weave.Panel):
    ...
    def render(self) -> weave.weave_query.panels.Table:
        return weave.weave_query.panels.Table(
            self.input_node,
            columns=[
                lambda model_row: weave.weave_query.panels.WeaveLink(
                    model_row.id(),
        ...
        return weave.weave_query.panels.Card(
            ...
            content=[
                weave.weave_query.panels.CardTab(
                    name="Model Card",
                    content=weave.weave_query.panels.PanelMarkdown(model.readme()),  # type: ignore
                ),
                weave.weave_query.panels.CardTab(
                    name="Metadata",
                    content=weave.weave_query.panels.Group(
                        items={
                            "id": weave.weave_query.panels.LabeledItem(
                                item=model.id(), label="ID"
                            ),
                            "pipeline_tag": weave.weave_query.panels.LabeledItem(
                ...
                #         weave.weave_query.ops.used_by(model, model.call.op_name()),
                #         columns=[
...
class HuggingfacePackagePanel(weave.Panel):
    ...
    def render(self) -> weave.weave_query.panels.Card:
        ...
        return weave.weave_query.panels.Card(
            ...
            content=[
                weave.weave_query.panels.CardTab(name="Models", content=pack.models()),  # type: ignore
            ],
        )
