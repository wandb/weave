...
def adder_config(
    ...
    return weave.weave_query.panels.LabeledItem(
        label="operand",
        item=weave.weave_query.panels.Slider(
            config=weave.weave_query.panels.SliderConfig(
                weave.weave_query.ops.execute(config.operand)
            )
        ),
    )
