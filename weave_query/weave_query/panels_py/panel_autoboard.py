...
from weave_query.panels import panel_plot
...
def timeseries(
    ...
    if weave.types.optional(weave.types.Timestamp()).assign_type(x_axis_type):
        x_title = ""
        bin_fn = weave_query.ops.timestamp_bins_nice
    elif weave.types.optional(weave.types.Number()).assign_type(x_axis_type):
        x_title = x_axis_key
        bin_fn = weave_query.ops.numbers_bins_equal
    else:
...
def auto_panels(
    ...
    metrics = weave_query.panels.Group(
        layoutMode=weave_query.panels.GroupLayoutFlow(2, 3),
        items={"panel%s" % i: panel for i, panel in enumerate(metric_panels)},
    )
    categoricals = weave_query.panels.Group(
        layoutMode=weave_query.panels.GroupLayoutFlow(2, 3),
    ...
    control_items = [
        ...
        weave_query.panels.GroupPanel(
            lambda data: weave_query.ops.make_list(
                a=data[x_axis].min(), b=data[x_axis].max()
            ),
            id="data_range",
            hidden=True,
        ),
        weave_query.panels.GroupPanel(None, id="zoom_range", hidden=True),
        weave_query.panels.GroupPanel(
        ...
        weave_query.panels.GroupPanel(
            lambda data, zoom_range: weave_query.panels.DateRange(
    ...
    return weave_query.panels.Board(vars=control_items, panels=panels, editable=False)
