...
def eval_board(dataset, eval_result0, eval_result1):  # type: ignore
    ...
    summary = varbar.add(
        ...
        weave_query.ops.make_list(
            a=weave_query.ops.TypedDict.merge(
                weave_query.ops.dict_(name="baseline"),
                baseline_eval_result["summary"],
            ),
            b=weave_query.ops.TypedDict.merge(
                weave_query.ops.dict_(name="candidate"),
            ...
            weave_query.ops.make_list(
                a=baseline_eval_result["eval_table"].map(
                    lambda row: weave_query.ops.TypedDict.merge(
                        weave_query.ops.dict_(name="baseline"), row
                    )
                ),
                b=candidate_eval_result["eval_table"].map(
                    lambda row: weave_query.ops.TypedDict.merge(
                        weave_query.ops.dict_(name="candidate"), row
                    )
                ),
            )
        ),
    ...
    main.add(
        ...
        layout=weave_query.panels.GroupPanelLayout(x=0, y=4, w=8, h=4),
    )
    main.add(
        "field_shares_f1",
        weave_query.panels.Plot(
        ...
        layout=weave_query.panels.GroupPanelLayout(x=8, y=4, w=8, h=4),
    )
    main.add(
        "field_directors_f1",
        weave_query.panels.Plot(
        ...
        layout=weave_query.panels.GroupPanelLayout(x=16, y=4, w=8, h=4),
    )
    ...
    #     "dataset_evals",
    #     dataset_evals,
    #     layout=weave_query.panels.GroupPanelLayout(x=0, y=4, w=24, h=6),
    # )
    ...
    # facet_f1 = weave_query.panels.Facet(
    #     dataset_evals,
    ...
    #     "f1_comparison",
    #     facet_f1,
    #     layout=weave_query.panels.GroupPanelLayout(x=0, y=8, w=12, h=6),
    # )

    facet_correct = weave_query.panels.Facet(
        dataset_evals,
    ...
    main.add(
        "help",
        weave_query.panels.PanelString(
        ...
        layout=weave_query.panels.GroupPanelLayout(x=12, y=8, w=12, h=6),
    )
    # main.add(
    #     "example_latencies",
    #     weave_query.panels.Plot(
    ...
    sel_ex_table = weave_query.panels.Table(correct_comparison.selected())
    ...
    selected_examples = main.add(
        "selected_examples",
        sel_ex_table,
        layout=weave_query.panels.GroupPanelLayout(x=0, y=14, w=24, h=12),
    )

    main.add(
        "baseilne_detail",
        selected_examples.active_data()["evals.summary"][0],
        layout=weave_query.panels.GroupPanelLayout(x=0, y=26, w=12, h=8),
    )

    main.add(
        "candidate_detail",
        selected_examples.active_data()["evals.summary"][1],
        layout=weave_query.panels.GroupPanelLayout(x=12, y=26, w=12, h=8),
    )

    return weave_query.panels.Board(vars=varbar, panels=main)
