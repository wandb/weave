...
panels = weave.weave_query.panels
...
def observability(
    ...
    now = weave.weave_query.ops.datetime_now()

    dashboard = weave.weave_query.panels.Group(
        layoutMode="grid",
    ...
    display_states = weave.weave_query.ops.dict_(
        **{
            "running": "running",
            "failed": "failed",
            "crashed": "crashed",
    ...
    window_start = varbar.add(
        "window_start",
        weave.weave_query.ops.from_number(now - seven_days_in_seconds),
        hidden=True,
    )
    window_end = varbar.add(
        "window_end",
        weave.weave_query.ops.from_number(now),
        hidden=True,
    )
    ...
    window_data = source_data.filter(
        lambda row: weave.weave_query.ops.Boolean.bool_and(
            ...
            weave.weave_query.ops.Boolean.bool_and(
    ...
    colors_node = weave.weave_query.ops.dict_(
        **{
    ...
    queued_time_data = varbar.add(
        ...
        filtered_window_data.filter(
            lambda row: weave.weave_query.ops.Boolean.bool_or(
                row["state"] == "queued",
                row["state"] == "pending",
            ),
        ),
    ...
    is_start_stop_state = weave_internal.define_fn(
        ...
        lambda row: weave.weave_query.ops.Boolean.bool_or(
            row["state"] == "running",
            weave.weave_query.ops.Boolean.bool_or(
                row["state"] == "finished",
                weave.weave_query.ops.Boolean.bool_or(
                    row["state"] == "crashed",
                    row["state"] == "failed",
    ...
    state_transitions_plot = panels.Plot(
        ...
        x=lambda row: row[timestamp_col_name].bin(
            weave.weave_query.ops.timestamp_bins_nice(bin_range, num_buckets)
        ),
        x_title="Time",
        y=lambda row: row.count(),
        y_title="Count of transitions by state",
        label=lambda row: row["state"],
        tooltip=lambda row: weave.weave_query.ops.dict_(
            **{
                "State": row["state"][0],
                "Count": row.count(),
                "Job (s)": weave.weave_query.ops.join_to_str(row["job"].unique(), ","),
            }
        ),
        color_title="state",
    ...
    runs_grouped = weave.weave_query.ops.List.groupby(
        queued_time_data, lambda row: row["trace_id"]
    )
    runs_mapped = weave.weave_query.ops.List.map(
        runs_grouped,
        lambda row: weave.weave_query.ops.dict_(
            **{
                ...
                "duration": weave.weave_query.ops.Number.__truediv__(
                    weave.weave_query.ops.Number.__mul__(
                        weave.weave_query.ops.timedelta_total_seconds(
                            weave.weave_query.ops.datetime_sub(
    ...
    runs_mapped_filtered = varbar.add(
        ...
        weave.weave_query.ops.List.filter(
            runs_mapped,
    ...
    queued_time_plot = panels.Plot(
        ...
        tooltip=lambda row: weave.weave_query.ops.dict_(
            **{
                "Job (s)": weave.weave_query.ops.join_to_str(row["job"].unique(), ","),
                "User (s)": weave.weave_query.ops.join_to_str(
                    row["entity_name"].unique(), ","
                ),
                "Project (s)": weave.weave_query.ops.join_to_str(
    ...
    start_stop_states = varbar.add(
        "start_stop_states",
        weave.weave_query.ops.List.filter(filtered_window_data, is_start_stop_state),
        hidden=True,
    )
    ...
    latest_runs_plot = panels.Plot(
        ...
        tooltip=lambda row: weave.weave_query.ops.dict_(
            **{
                ...
                "Run ID": row["run_id"][-1],
                "Status": row["state"][-1],
                "Duration (m)": weave.weave_query.ops.Number.__truediv__(
                    weave.weave_query.ops.Number.__mul__(
                        weave.weave_query.ops.timedelta_total_seconds(
                            weave.weave_query.ops.datetime_sub(
                                row[timestamp_col_name].max(),
    ...
    jobs_table.add_column(
        lambda row: weave.weave_query.ops.run_ops.str_run_link(
    ...
    jobs_table.add_column(
        lambda row: weave.weave_query.ops.cond(
            weave.weave_query.ops.dict_(
                a=row["state"][-1] != "running",
                b=row["state"][-1] == "running",
            ),
            weave.weave_query.ops.dict_(
                a=weave.weave_query.ops.Number.__truediv__(
                    weave.weave_query.ops.timedelta_total_seconds(
                        weave.weave_query.ops.datetime_sub(
                            row[timestamp_col_name].max(),
                            row[timestamp_col_name].min(),
                ...
                b=weave.weave_query.ops.Number.__truediv__(
                    weave.weave_query.ops.timedelta_total_seconds(
                        weave.weave_query.ops.datetime_sub(
                            weave.weave_query.ops.from_number(
                                weave.weave_query.ops.datetime_now()
                            ),
                            row[timestamp_col_name].min(),
                        )
                    ),
    ...
    jobs_table.add_column(
        lambda row: weave.weave_query.ops.cond(
            ...
            weave.weave_query.ops.dict_(
                ...
                b=weave.weave_query.ops.make_const_node(types.NoneType(), None),
            ),
        ),
        "Avg. CPU %",
    )
    ...
    runs_table_data = weave.weave_query.ops.List.groupby(
        filtered_window_data, lambda row: row["trace_id"]
    )
    runs_table_data_mapped = varbar.add(
        "runs_table_data",
        weave.weave_query.ops.List.map(
            runs_table_data,
            lambda row: weave.weave_query.ops.dict_(
                **{
                    "trace_id": row["trace_id"][0],
    ...
    metric_plot_data = weave.weave_query.ops.List.groupby(
