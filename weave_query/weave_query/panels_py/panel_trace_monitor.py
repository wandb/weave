...
def board(
    ...
    overview_tab.add(
        ...
        layout=weave.weave_query.panels.GroupPanelLayout(x=0, y=0, w=6, h=6),
    )

    overview_tab.add(
        "latency_distribution",
        filtered_window_data.map(lambda row: row["end_time_s"] - row["start_time_s"]),
        layout=weave.weave_query.panels.GroupPanelLayout(x=6, y=0, w=6, h=6),
    )
    ...
    overview_tab.add(
        ...
        layout=weave.weave_query.panels.GroupPanelLayout(x=12, y=0, w=6, h=6),
    )
    ...
    overview_tab.add(
        ...
        layout=weave.weave_query.panels.GroupPanelLayout(x=18, y=0, w=6, h=6),
    )

    traces_table_var = overview_tab.add(
        "traces_table",
        make_span_table(filtered_window_data),
        layout=weave.weave_query.panels.GroupPanelLayout(x=0, y=6, w=24, h=6),
    )
    ...
    trace_viewer_var = overview_tab.add(
        "trace_viewer",
        trace_viewer,
        layout=weave.weave_query.panels.GroupPanelLayout(x=0, y=12, w=16, h=6),
    )

    selected_trace_model = overview_tab.add(
        "selected_trace_model",
        TraceSpanModelPanel(traces_table_var.active_data()),
        layout=weave.weave_query.panels.GroupPanelLayout(x=0, y=18, w=16, h=6),
    )
    ...
    selected_span_details = overview_tab.add(
        ...
        layout=weave.weave_query.panels.GroupPanelLayout(x=16, y=12, w=8, h=12),
    )
    ...
    similar_spans_table_var = overview_tab.add(
        "similar_spans_table",
        similar_spans_table,
        layout=weave.weave_query.panels.GroupPanelLayout(x=0, y=22, w=24, h=6),
    )
