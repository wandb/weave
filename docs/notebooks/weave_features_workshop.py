...
def classify_urgency(email: str, sentiment: str) -> str:
    ...
    urgent_keywords = ["urgent", "asap", "immediately", "frustrated", "broken", "stopped working"]
...
@weave.op
def sentiment_accuracy(expected_sentiment: str, output: CustomerEmail) -> Dict[str, Any]:
...
def extraction_quality(email: str, output: CustomerEmail) -> Dict[str, Any]:
    ...
    return {
        "score": score,
        "feedback": "; ".join(feedback) if feedback else "All fields extracted successfully"
    }
...
# %%
@weave.op
def analyze_with_fallback(email: str, primary_model: str = "gpt-4o-mini", fallback_model: str = "gpt-3.5-turbo") -> CustomerEmail:
