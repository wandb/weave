...
class DetailedCustomerEmail(BaseModel):
    ...
    issue: str
    issue_category: str = Field(description="technical, billing, feature_request, or other")
...
class BusinessImpactScorer(Scorer):
    ...
    def score(self, expected: dict, output: DetailedCustomerEmail) -> dict[str, Any]:
        ...
        # Business rules
        requires_escalation = (
            output.severity in ["critical", "high"] or
            output.issue_category == "billing" or
            (output.company and "enterprise" in output.product.lower())
        )
        ...
        return {
            ...
            "priority_level": "P1" if impact_score > 0.7 else ("P2" if impact_score > 0.4 else "P3")
        }
...
class SLAComplianceScorer(Scorer):
    ...
    @weave.op
    def score(self, output: DetailedCustomerEmail, response_time_hours: float = 0) -> dict[str, Any]:
...
class MultiStageEvaluator:
    ...
    def __init__(self, model: Model, dataset: Dataset):
        ...
        self.logger = EvaluationLogger(
            ...
            dataset=f"{dataset.name}_multistage"
        )
        self.stage_results = {}
    ...
    def stage2_accuracy_check(self, example: dict, pred_logger) -> None:
        ...
        print(f"  🎯 Stage 2: Accuracy Check")
        ...
        for field, expected_value in expected.items():
            ...
            if actual_value:
                ...
                pred_logger.log_score(scorer=f"{field}_accuracy", score=accuracy_scores[field])
    ...
    def stage3_quality_assessment(self, example: dict, pred_logger) -> None:
        ...
        print(f"  📏 Stage 3: Quality Assessment")
    ...
    def run_full_evaluation(self) -> dict:
        ...
        aggregate_metrics = {
            "avg_completeness": statistics.mean([r["extraction"]["completeness"] for r in all_results]),
            "avg_accuracy": statistics.mean([r["accuracy"]["overall"] for r in all_results]),
            "avg_quality": statistics.mean([r["quality"][m] for r in all_results for m in r["quality"]]) / 3
        }
...
def statistical_ab_test(
    ...
    print(f"   Winner: {results['comparison']['winner']} ({results['comparison']['confidence']} confidence)")
...
def cross_validate_model(
    ...
    for fold in range(n_folds):
        ...
        for example in test_rows:
            ...
            # Consistency check
            if (output.severity in ["critical", "high"] and output.sentiment == "negative") or \
...
class ProductionEvaluationPipeline:
    ...
    def run_comprehensive_evaluation(self) -> dict:
        ...
        print(f"   Baseline: {self.baseline_model.__class__.__name__ if self.baseline_model else 'None'}")
        ...
        for model_name, results in self.results.items():
            if isinstance(results, dict) and "accuracy" in results:
                comparison[model_name] = {
                    "accuracy_improvement": results["accuracy"]["mean"] - baseline_results["accuracy"]["mean"],
                    "latency_change": results["latency_ms"]["mean"] - baseline_results["latency_ms"]["mean"],
                    "business_value_improvement": results["business_value"]["mean"] - baseline_results["business_value"]["mean"]
                }
        ...
        for model_name, results in self.results.items():
            if isinstance(results, dict) and "accuracy" in results:
                readiness[model_name] = {
                    "accuracy_ok": results["accuracy"]["mean"] >= criteria["min_accuracy"],
                    "latency_ok": results["latency_ms"]["mean"] <= criteria["max_latency_ms"],
                    "completeness_ok": results["completeness"]["mean"] >= criteria["min_completeness"],
                    "business_value_ok": results["business_value"]["mean"] >= criteria["min_business_value"]
                }
                readiness[model_name]["production_ready"] = all(readiness[model_name].values())
    ...
    def _generate_report(self):
        ...
        for model_name, readiness in prod_ready.items():
            if readiness.get("production_ready"):
                ...
                issues = [k.replace("_ok", "") for k, v in readiness.items() if not v and k != "production_ready"]
                report["recommendations"].append(f"❌ {model_name} needs improvement in: {', '.join(issues)}")
