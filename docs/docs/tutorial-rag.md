...
In some applications we want to create custom evaluation classes - where for example a standardized `LLMJudge` class should be created with specific parameters (e.g. chat model, prompt), specific scoring of each row, and specific calculation of an aggregate score. In order to do that Weave defines a list of ready-to-use `Scorer` classes and also makes it easy to create a custom `Scorer`. Next we'll see how to create a custom `class CorrectnessLLMJudge(Scorer)`. 

At a high-level, the steps to create custom Scorer are quite simple: 
...
class CorrectnessLLMJudge(Scorer):
    ...
    async def score(self, model_output: Optional[dict], query: str, answer: str) -> Any:
        ...
        # output parsing - could be done more robustly with pydantic
