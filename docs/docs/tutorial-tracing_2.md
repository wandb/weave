  ...
  <TabItem value="typescript" label="TypeScript">
    ...
    const extractDinos = weave.op(async (sentence: string) => {
      const response = await openai.chat.completions.create({
        model: 'gpt-4o',
        messages: [
          { role: 'system', content: """Extract any dinosaur `name`, their `common_name`, names and whether its `diet` is a herbivore or carnivore, in JSON format.""" },
    ...
    const countDinos = weave.op(async (dinoData: string) => {
      const dinoData = JSON.parse(dinoData);
      return Object.keys(dinoData).length;
    });
