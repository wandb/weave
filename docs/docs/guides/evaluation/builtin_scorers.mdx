...
<Tabs groupId="programming-language" queryString>
  <TabItem value="python" label="Python" default>
    ...
    **Column Mapping:**
    ...
    You can the `column_map` attribute to tell the scorer which dataset columns to use. The available keys for `column_map` are:
    - `output` (required): Model prediction
    - `target` (required): Reference answer to compare with
    - `kwargs` (optional): Additional input context such as a question or prompt
    
    An example of using `column_map`:
    ...
    similarity_scorer.column_map = {
        ...
        "kwargs": "question"        # Optional: additional context (e.g. the original prompt)
    }
    ```

    **Example Usage:**
    
    Here is an example usage of `EmbeddingSimilarityScorer` in the context of an evaluation:
