...
<Tabs groupId="programming-language">
  <TabItem value="python" label="Python" default>
    ...
    :::tip
    You can control Weave's default tracking behavior. See [Configure autopatching](#configure-autopatching) for details.
    :::
...
The `autopatch_settings` argument is deprecated. Use `settings={"implicitly_patch_integrations": False}` to disable implicit patching, or call specific patch functions like `weave.integrations.patch_openai(settings={...})` to configure settings per integration.
:::

### Disable all autopatching

```python showLineNumbers
weave.init(..., settings={"implicitly_patch_integrations": False})
```
...
# Don't call patch_openai() to keep OpenAI disabled
```

### Post-process inputs and outputs

You can customize how inputs and outputs (e.g. for PII data) are handled by passing settings to the patch function:
...
weave.init(...)
weave.integrations.patch_openai(settings={"op_settings": {"postprocess_inputs": redact_inputs}})
```
