...
Those familiar with the [OpenTelemetry](https://opentelemetry.io) data model will find Calls to be quite similar.
A Call can belong to a Trace, which is a collection of calls that belong to the same execution context.
...
Calls can be created using a number of different methods:

1. Automatically by integrations with common LLM libraries
2. Captured at runtime by Python decorators
3. Manually created by directly using the API

### 1. Automatic Tracking of LLM Libraries

The vast majority of Weave users will be using one of many standard LLM libraries, such as `openai`, `anthropic`, `cohere`, `mistral`, etc. 
Weave automatically tracks [calls to these libraries](../integrations/index.md). All you need to do is call [`weave.init('project_name')`](../../reference/python-sdk/weave/index.md#function-init) once at the beginning of your program.
...
2. (Coming soon) Change the display name for all Calls of a given Op:

```python showLineNumbers
@weave.op(display_name="My Custom Display Name")
def my_function(name: str):
    return f"Hello, {name}!"
```

**Technical Note:** "Calls" are produced by "Ops". An Op is a function or method that is decorated with `@weave.op`. 
By default, the Op's name is the function name, and the associated calls will have the same display name. The above example shows how to override the display name for all Calls of a given Op.
Sometimes, users wish to override the name of the Op itself. This can be achieved by assigning to the `name` property of the Op, before any calls are logged.
...
my_function.name = "My Custom Display Name"
```
...
<Tabs groupId="client-layer">
    <TabItem value="python_sdk" label="Python">
    In order to set the display name of a call, you can use the [`Call.set_display_name`](../../reference/python-sdk/weave/trace/weave.trace.weave_client.md#function-set_display_name) method.
...
<Tabs groupId="client-layer">
    <TabItem value="python_sdk" label="Python">
    To delete a call using the Python API, you can use the [`Call.delete`](../../reference/python-sdk/weave/trace/weave.trace.weave_client.md#function-delete) method.
    ...
    <TabItem value="service_api" label="HTTP API">
    To delete a call using the Service API, you can make a request to the [`/call/delete`](../../reference/service-api/call-delete-call-delete-post.api.mdx) endpoint.
