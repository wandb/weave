...
In situations where you want to unconditionally disable tracing for the entire program, you can set the environment variable `WEAVE_TRACING_ENABLED` to `false`.
...
# Initialize the client
client = weave.init(..., tracing_enabled=False)
```

**Context Manager:**

Finally, you may want to conditionally disable tracing for a single function based on some application logic. In this case, you can use the context manager `with set_tracing_enabled(False)` which can be import from `weave.trace.context.call_context`.
