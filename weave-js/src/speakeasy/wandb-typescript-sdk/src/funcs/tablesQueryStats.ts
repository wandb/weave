 ...
 * Table Query Stats
 */
export async function tablesQueryStats(
  client: WandbCore,
  request: components.TableQueryStatsReq,
  options?: RequestOptions,
): Promise<
  Result<
    components.TableQueryStatsRes,
    | errors.BadRequest
    | errors.Unauthorized
    | errors.NotFound
    | errors.Timeout
...
> {
  const parsed = safeParse(
    request,
    (value) => components.TableQueryStatsReq$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/table/query_stats")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.TableQueryStatsRes$inboundSchema),
