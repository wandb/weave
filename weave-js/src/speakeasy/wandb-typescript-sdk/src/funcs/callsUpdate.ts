 ...
 * Call Update
 */
export async function callsUpdate(
  client: WandbCore,
  request: components.CallUpdateReq,
  options?: RequestOptions,
): Promise<
  Result<
    components.CallUpdateRes,
    | errors.BadRequest
    | errors.Unauthorized
    | errors.NotFound
    | errors.Timeout
...
> {
  const parsed = safeParse(
    request,
    (value) => components.CallUpdateReq$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/call/update")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.CallUpdateRes$inboundSchema),
