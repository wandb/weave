 ...
 */
export async function feedbackPurge(
  client: WandbCore,
  request: components.FeedbackPurgeReq,
  options?: RequestOptions,
): Promise<
  Result<
    components.FeedbackPurgeRes,
    | errors.BadRequest
    | errors.Unauthorized
    | errors.NotFound
    | errors.Timeout
...
> {
  const parsed = safeParse(
    request,
    (value) => components.FeedbackPurgeReq$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/feedback/purge")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.FeedbackPurgeRes$inboundSchema),
