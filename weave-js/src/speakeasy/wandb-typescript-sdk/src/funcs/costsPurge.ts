 ...
 * Cost Purge
 */
export async function costsPurge(
  client: WandbCore,
  request: components.CostPurgeReq,
  options?: RequestOptions,
): Promise<
  Result<
    components.CostPurgeRes,
    | errors.BadRequest
    | errors.Unauthorized
    | errors.NotFound
    | errors.Timeout
...
> {
  const parsed = safeParse(
    request,
    (value) => components.CostPurgeReq$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/cost/purge")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.CostPurgeRes$inboundSchema),
