 ...
 * Calls Delete
 */
export async function callsDelete(
  client: WandbCore,
  request: components.CallsDeleteReq,
  options?: RequestOptions,
): Promise<
  Result<
    components.CallsDeleteRes,
    | errors.BadRequest
    | errors.Unauthorized
    | errors.NotFound
    | errors.Timeout
...
> {
  const parsed = safeParse(
    request,
    (value) => components.CallsDeleteReq$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/calls/delete")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.CallsDeleteRes$inboundSchema),
