 ...
 * Cost Query
 */
export async function costsQuery(
  client: WandbCore,
  request: components.CostQueryReq,
  options?: RequestOptions,
): Promise<
  Result<
    components.CostQueryRes,
    | errors.BadRequest
    | errors.Unauthorized
    | errors.NotFound
    | errors.Timeout
...
> {
  const parsed = safeParse(
    request,
    (value) => components.CostQueryReq$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/cost/query")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.CostQueryRes$inboundSchema),
