 ...
 * Objs Query
 */
export async function objectsQuery(
  client: WandbCore,
  request: components.ObjQueryReq,
  options?: RequestOptions,
): Promise<
  Result<
    components.ObjQueryRes,
    | errors.BadRequest
    | errors.Unauthorized
    | errors.NotFound
    | errors.Timeout
...
> {
  const parsed = safeParse(
    request,
    (value) => components.ObjQueryReq$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/objs/query")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.ObjQueryRes$inboundSchema),
