 ...
 * Calls Query Stats
 */
export async function callsQueryStats(
  client: WandbCore,
  request: components.CallsQueryStatsReq,
  options?: RequestOptions,
): Promise<
  Result<
    components.CallsQueryStatsRes,
    | errors.BadRequest
    | errors.Unauthorized
    | errors.NotFound
    | errors.Timeout
...
> {
  const parsed = safeParse(
    request,
    (value) => components.CallsQueryStatsReq$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/calls/query_stats")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.CallsQueryStatsRes$inboundSchema),
