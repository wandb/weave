 ...
 * Obj Read
 */
export async function objectsRead(
  client: WandbCore,
  request: components.ObjReadReq,
  options?: RequestOptions,
): Promise<
  Result<
    components.ObjReadRes,
    | errors.BadRequest
    | errors.Unauthorized
    | errors.NotFound
    | errors.Timeout
...
> {
  const parsed = safeParse(
    request,
    (value) => components.ObjReadReq$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/obj/read")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.ObjReadRes$inboundSchema),
