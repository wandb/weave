 ...
 * Refs Read Batch
 */
export async function refsReadBatch(
  client: WandbCore,
  request: components.RefsReadBatchReq,
  options?: RequestOptions,
): Promise<
  Result<
    components.RefsReadBatchRes,
    | errors.BadRequest
    | errors.Unauthorized
    | errors.NotFound
    | errors.Timeout
...
> {
  const parsed = safeParse(
    request,
    (value) => components.RefsReadBatchReq$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/refs/read_batch")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.RefsReadBatchRes$inboundSchema),
