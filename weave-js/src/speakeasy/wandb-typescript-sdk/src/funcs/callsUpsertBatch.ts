 ...
 * Call Start Batch
 */
export async function callsUpsertBatch(
  client: WandbCore,
  request: components.CallCreateBatchReq,
  options?: RequestOptions,
): Promise<
  Result<
    components.CallCreateBatchRes,
    | errors.BadRequest
    | errors.Unauthorized
    | errors.NotFound
    | errors.Timeout
...
> {
  const parsed = safeParse(
    request,
    (value) => components.CallCreateBatchReq$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/call/upsert_batch")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.CallCreateBatchRes$inboundSchema),
