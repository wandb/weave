 ...
 * Obj Create
 */
export async function objectsCreate(
  client: WandbCore,
  request: components.ObjCreateReq,
  options?: RequestOptions,
): Promise<
  Result<
    components.ObjCreateRes,
    | errors.BadRequest
    | errors.Unauthorized
    | errors.NotFound
    | errors.Timeout
...
> {
  const parsed = safeParse(
    request,
    (value) => components.ObjCreateReq$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/obj/create")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.ObjCreateRes$inboundSchema),
