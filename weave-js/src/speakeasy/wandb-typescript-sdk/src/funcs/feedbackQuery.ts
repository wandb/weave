 ...
 * Query for feedback.
 */
export async function feedbackQuery(
  client: WandbCore,
  request: components.FeedbackQueryReq,
  options?: RequestOptions,
): Promise<
  Result<
    components.FeedbackQueryRes,
    | errors.BadRequest
    | errors.Unauthorized
    | errors.NotFound
    | errors.Timeout
...
> {
  const parsed = safeParse(
    request,
    (value) => components.FeedbackQueryReq$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/feedback/query")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.FeedbackQueryRes$inboundSchema),
