 ...
 * Call Read
 */
export async function callsRead(
  client: WandbCore,
  request: components.CallReadReq,
  options?: RequestOptions,
): Promise<
  Result<
    components.CallReadRes,
    | errors.BadRequest
    | errors.Unauthorized
    | errors.NotFound
    | errors.Timeout
...
> {
  const parsed = safeParse(
    request,
    (value) => components.CallReadReq$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/call/read")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.CallReadRes$inboundSchema),
