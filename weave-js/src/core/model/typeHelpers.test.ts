...
describe('union', () => {
  ...
  describe('performance cases', () => {
    it('handles large unions of typedDicts efficiently', () => {
      const typedDicts = [];
      // Create 100 unique typedDicts
      for (let i = 0; i < 100; i++) {
        typedDicts.push(
          typedDict({
            [`field_${i}`]: 'number',
      ...
      const result = union(typedDicts);
      const endTime = Date.now();

      // Should complete quickly (under 100ms for 150 items)
      expect(endTime - startTime).toBeLessThan(100);

      // Should have deduplicated to 100 unique types
      expect(result.type).toEqual('union');
      expect((result as any).members.length).toEqual(100);
    });

    it('handles deeply nested typedDict properties', () => {
      const deeplyNested = typedDict({
        level1: typedDict({
          level2: typedDict({
            level3: typedDict({
              level4: typedDict({
      ...
      const result = union([deeplyNested, duplicate]);
      expect(result).toEqual(deeplyNested);
    });

    it('handles massive duplicate typedDicts efficiently', () => {
      // Create a complex typedDict
      const complexTd = typedDict({
        id: 'string',
        name: 'string',
        age: 'number',
        active: 'boolean',
      ...
      // Create 1000 duplicates
      const duplicates = Array(1000).fill(complexTd);

      const startTime = Date.now();
      const result = union(duplicates);
      const endTime = Date.now();

      // Should complete very quickly (under 50ms for 1000 identical items)
      expect(endTime - startTime).toBeLessThan(50);

      // Should deduplicate to a single type
      expect(result).toEqual(complexTd);
    });
  });
