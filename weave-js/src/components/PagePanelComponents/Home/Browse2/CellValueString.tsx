...
import {stringIsLikelyImageURL} from '../Browse3/pages/CallPage/heuristics';
...
export const CellValueString = ({value}: CellValueStringProps) => {
  const trimmed = value.trim();
  if (isUrl(trimmed)) {
    // Super hacky way to detect if a string is a URL to an image generated by DALLE API.
    if (stringIsLikelyImageURL(trimmed)) {
      return <img height="100%" src={trimmed} alt="DALLE" />;
    }
