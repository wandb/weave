      ...
      <div className="max-w-xs">
        ...
        {Object.entries(groupedByAlias).map(([alias, aliasReactions]) => {
          ...
          const names = aliasReactions.map(
            r => r.creator ?? userMap[r.wb_user_id]?.username ?? userMap[r.wb_user_id]?.name ?? r.wb_user_id
          );
          const currentViewerName = currentViewerId
            ? userMap[currentViewerId]?.username ?? userMap[currentViewerId]?.name ?? currentViewerId
            : null;
