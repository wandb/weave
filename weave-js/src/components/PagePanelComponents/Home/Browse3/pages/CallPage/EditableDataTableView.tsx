...
export const EditableDataTableView: FC<EditableDataTableViewProps> = props => {
  ...
  const handleAddRowsClick = useCallback(() => {
    setAddedRows(prev => {
      const updatedMap = new Map(prev);
      const newId = `new-${uuidv4()}`; // Generate a unique ID using UUID
      updatedMap.set(newId, {id: newId});
      return updatedMap;
    });
  }, [setAddedRows]);
  ...
  const columns = useMemo(() => {
    const allFields = rows.reduce((acc, row) => {
  ...
  const combinedRows = useMemo(() => {
    const newRows = Array.from(addedRows.values()).map(row => ({
      ...row,
      id: row.id,
    }));
    return [...newRows, ...rows];
