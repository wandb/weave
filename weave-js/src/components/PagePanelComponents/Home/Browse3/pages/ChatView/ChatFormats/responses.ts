...
export const isOpenAIResponsesContentBlock = (block: any): boolean => {
...
export const isOpenAIResponsesCompletionFormat = (output: any): boolean => {
  if (output !== null) {
    ...
    if (
      ...
      output.content.every((c: any) => isOpenAIResponsesContentBlock(c))
    ) {
      return true;
    }
    return false;
...
type OpenAIResponsesContentBlock = {
  type: 'text';
  text: string;
};

export const anthropicContentBlocksToChoices = (
  blocks: OpenAIResponsesContentBlock[],
  stopReason: string
): Choice[] => {
...
export const isTraceCallChatFormatOpenAIResponses = (
...
export const normalizeOpenAIResponsesChatCompletion = (
  completion: any
): ChatCompletion => {
  return {
    id: completion.id,
    ...
    created: 0, // OpenAIResponses doesn't provide `created`
    model: completion.model,
    system_fingerprint: '', // OpenAIResponses doesn't provide `system_fingerprint`
    usage: {
...
export const normalizeOpenAIResponsesChatRequest = (
  request: ChatRequest & {system: string}
) => {
  // OpenAIResponses has system message as a top-level request field
  return {
    ...request,
    messages: [
      {
