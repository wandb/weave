...
export const DisabledProviderTooltip: React.FC<{
  ...
  project: string;
  missingSecrets?: string[];
}> = ({children, entity, project, missingSecrets}) => {
  return (
    <Tooltip
      trigger={children}
      content={
        <Box
          ...
          <div>
            ...
            <Link
              ...
              {missingSecrets?.join(', ') || 'missing secrets'}
            </Link>{' '}
            to enable it.
          </div>
        </Box>
...
const SubMenuOption = ({
  ...
  const {llms, isDisabled, missingSecrets} = props.data;
  ...
  return isDisabled && missingSecrets ? (
    ...
    return (
      <Box>
        ...
        <Box
          ...
          {llms
            .filter(llm => llm.value.includes(inputValue))
            .map(llm => (
              <Box
                key={llm.value}
                onClick={() => {
