 ...
 *     isPredictionIndex (boolean): Whether this is a prediction index plot.
 *
 * Returns:
...
export const calculateBinBoundaries = (
  ...
  isPredictionIndex: boolean = false
): {binStart: number | null; binEnd: number | null} => {
  if (isPredictionIndex) {
    // For prediction index, no binning - just return the exact point
    return {binStart: dataX, binEnd: dataX};
  }
 ...
 *     groupBy (string | undefined): Group by field name.
 *     colorGroupKey (string | undefined): Color group key.
...
export const createBinnedPointsByGroup = (
  ...
  const hasGrouping =
    !!(groupBy || colorGroupKey) && pointsToUse.some(pt => pt.group);
