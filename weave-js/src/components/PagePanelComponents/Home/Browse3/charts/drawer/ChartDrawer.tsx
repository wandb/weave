...
export const ChartDrawer: React.FC<ChartDrawerProps> = ({
  ...
  // Use dynamic axis field functions that include input/output fields
  const isScatterPlot = localConfig.plotType === 'scatter';
  const xAxisOptions = React.useMemo(() => {
    const fields = isScatterPlot
      ? getScatterXAxisFields(callData)
      : getXAxisFields(callData);
    return fields.map(f => ({
      value: f.key,
      label: f.label,
    }));
  ...
  const yAxisOptions = React.useMemo(() => {
    const fields = isScatterPlot
      ? getScatterYAxisFields(callData)
      : getYAxisFields(callData);
    return fields.map(f => ({
      value: f.key,
      label: f.label,
    }));
  ...
  const colorGroupOptions = React.useMemo(() => {
    if (
      !isScatterPlot &&
    ...
    // Add input/output field options
    if (callData.length > 0) {
      ...
      colorGroupFields.forEach(field => {
        options.push({
          value: field.key,
          label: field.label,
        });
      ...
      <Box
        ...
        <div
          ...
          {(localConfig.plotType === 'scatter' ||
            ...
            localConfig.plotType === 'bar') && (
            <>
              <SectionHeader>Color Grouping</SectionHeader>
              <FieldLabel>Group By</FieldLabel>
              <CustomSelect
                value={
                  colorGroupOptions.find(
                    opt => opt.value === (localConfig.colorGroupKey || '')
                  ) || colorGroupOptions[0]
                }
                onChange={option =>
                  setLocalConfig(prev => ({
          ...
          {localConfig.plotType === 'scatter' && (
            <>
              ...
              <CustomSelect
                value={
                  xAxisOptions.find(opt => opt.value === localConfig.xAxis) ||
                  null
                }
                onChange={option =>
                  setLocalConfig(prev => ({
