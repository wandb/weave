...
export function convertSchemaToAxisFields(
  ...
  const createAxisField = (fieldSchema: FieldSchema): ChartAxisField => {
    ...
    let renderFunc = (value: any) => value?.toString() || '';
 ...
 * - scorers: scorer results by name
 ...
 *   "wandb.runnable.toxicity": { payload: { output: { score: 0.2 } } },
 *   "wandb.note.1": { payload: { note: "Good result" } }
 * };
 * const processed = processFeedback(raw);
 * // Returns: { annotations: { Quality: 8.5 }, scorers: { toxicity: 0.2 }, notes: ["Good result"], reactions: [] }
 */
export function processFeedback(rawFeedback?: {
  ...
  Object.entries(rawFeedback).forEach(([feedbackType, feedbackItem]) => {
    ...
    // Handle scorers: "wandb.runnable.toxicity" -> scorers.toxicity
      ...
      if (payload.output && typeof payload.output === 'object') {
        // If output is an object, try to find a numeric/boolean value
        const outputEntries = Object.entries(payload.output);
        if (outputEntries.length === 1) {
          value = outputEntries[0][1];
        } else {
      ...
      if (value !== undefined && value !== null) {
        processed.scorers[scorerName] = value;
        hasAnyFeedback = true;
      }
    }
