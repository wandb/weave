...
// Utility function to get nested field depth
export function getFieldDepth(fieldKey: string): number {
  // Remove the source prefix (input./output.) and count dots
  const [, ...pathParts] = fieldKey.split('.');
  return pathParts.join('.').split('.').length;
}

// Utility function to filter fields by maximum depth
export function filterFieldsByDepth(
  fields: ChartAxisField[],
  maxDepth: number
): ChartAxisField[] {
  return fields.filter(field => getFieldDepth(field.key) <= maxDepth);
}

// Utility function to group fields by their parent path
export function groupFieldsByParent(schema: InputOutputSchema): {
  [parentPath: string]: FieldSchema[];
} {
  const grouped: {[parentPath: string]: FieldSchema[]} = {};

  const processFields = (fields: Map<string, FieldSchema>) => {
    fields.forEach(field => {
      const pathParts = field.fullPath.split('.');
      const parentPath =
        pathParts.length > 1
          ? `${field.source}.${pathParts.slice(0, -1).join('.')}`
          : field.source;
  ...
  processFields(schema.inputFields);
  processFields(schema.outputFields);
...
// New function to get all fields that can be used for grouping/coloring
