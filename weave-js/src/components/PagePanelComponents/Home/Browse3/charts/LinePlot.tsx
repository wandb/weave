  ...
  return (
    <div style={tooltipContainerStyle(isFullscreen)}>
      ...
      {validData.map((item, index) => {
        ...
        const displayName = traceIdToDisplayName?.get(item.group) || item.group;
...
export const LinePlot: React.FC<LinePlotProps> = ({
  ...
  const points = React.useMemo(() => {
    ...
    // Special handling for prediction index in evaluations
    if (initialXAxis === 'prediction_index') {
      const result: DataPoint[] = [];

      data.forEach(d => {
        // Use the populated prediction_index field
        if (d.prediction_index === undefined) {
          return; // Skip if prediction_index is not set
        }
        ...
        const point = {
          x: d.prediction_index, // Use the populated prediction_index field
          y:
            yField?.type === 'date'
              ? new Date(yValue as any).getTime()
              : yValue,
          display_name: d.display_name || '',
          group: colorGroup || (groupBy ? groupName : undefined),
        };
  ...
  const binnedPointsByGroup = React.useMemo(() => {
    ...
    const pointsToUse = filteredPoints.length > 0 ? filteredPoints : points;
  ...
  const tooltipPosition = React.useMemo(() => {
    ...
    // Dynamic positioning relative to cursor
    ...
    if (isLeftSide) {
      // Cursor on left side: position tooltip to the right of cursor
      x = crosshair.x + tooltipOffset;
      anchorRight = false;
    } else {
      // Cursor on right side: anchor tooltip's right edge to the left of cursor
      x = containerRect.width - crosshair.x + tooltipOffset;
      anchorRight = true;
    }
