...
type SelectObjVersionProps = {
  ...
  onChange: (versionHash: string) => void;
};

export const SelectObjVersion = ({
  objVersions,
  valueHash,
  // currentVersionURI,
  onChange,
}: SelectObjVersionProps) => {
  const options = objVersions.map(objv => ({
    id: objectVersionKeyToRefUri(objv),
    versionHash: objv.versionHash,
    label: 'v' + objv.versionIndex,
    created: objv.createdAtMs,
    isCurrent: false,
    // isCurrent: objectVersionKeyToRefUri(objv) === currentVersionURI,
  }));
  const value = options.find(o => o.versionHash === valueHash);
  return (
    <Autocomplete
      options={options}
      size="small"
