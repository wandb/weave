    ...
    if (inputValue !== '') {
      ...
      if (existingRow.___weave?.isNew) {
        setAddedRows((prev: Map<string, DatasetRow>) => {
          const newMap = new Map(prev);
          newMap.set(existingRow.___weave?.id, {
            ...existingRow,
            [field]: numValue,
          });
          return newMap;
        ...
        const updatedRow = {...existingRow, [field]: numValue};
  ...
  const getPopoverWidth = useCallback(
    (mode: EditorMode = editorMode) => {
      ...
      const approximateWidth = Math.min(
        Math.max(value.length, minWidth),
        maxWidth
      );
      return mode === 'diff'
...
> = params => {
  const {setEditedRows, setAddedRows} = useDatasetEditContext();
  const {id, value, field, api, serverValue} = params;
  ...
  return (
    <StringEditor
      ...
      onClose={() => {
        ...
        const updatedRow = {...existingRow, [params.field]: value};
        if (existingRow.___weave?.isNew) {
          setAddedRows(prev => {
            const newMap = new Map(prev);
