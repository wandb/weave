...
const CreateDatasetDrawerContent: React.FC<{
  ...
  const handleDescriptionChange = useCallback(
    (value: string) => {
      dispatch({
        type: CREATE_DATASET_ACTIONS.SET_DATASET_DESCRIPTION,
        payload: value,
      });
    },
    [dispatch]
            ...
            {!parsedData ? (
              <Box
                ...
                <Typography variant="h6" sx={typographyStyle} gutterBottom>
                  {isDragging ? 'Drop CSV file here' : 'Upload a CSV file'}
                </Typography>
                <Typography
                  variant="body2"
                  sx={typographyStyle}
                  ...
                  align="center"
                  gutterBottom>
                  Your CSV will be converted to a dataset that you can edit
                  before saving
                </Typography>
                <Box sx={{mt: 2}}>
                  <input
