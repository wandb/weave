...
export const MagicianContextProvider: React.FC<
  MagicianContextProviderProps
> = props => {
  const magicianContext = React.useMemo(() => {
    ...
    if (serviceType === 'openai') {
      try {
        ...
        console.error(
          'Failed to initialize OpenAI service, falling back to demo:',
          error
        );
...
export function useRespond(params: UseRespondParams): UseRespondResponse {
  ...
  const refetch = React.useCallback(
    async (overrideParams?: Partial<RespondParams>) => {
      setLoading(true);
      setError(null);
      setData(null);
