...
def create_sound_wave(
    ...
    for i in range(num_frames):
        ...
        scaled_value = max(
            min(scaled_value, max_amplitude_8bit), max(-max_amplitude_8bit - 1, 0)
        )
        yield scaled_value
...
def make_wav_file_with_n_frames(tmp_path: Path, n_frames: int) -> str:
    ...
    with wave.open(filename, mode="wb") as wav_file:
        ...
        wav_file.writeframes(
            bytes(create_sound_wave(n_frames, sample_rate=FRAMES_PER_SECOND))
        )
    return filename
...
def make_wav_file(tmp_path: Path) -> str:
    ...
    with wave.open(filename, mode="wb") as wav_file:
        ...
        wav_file.setframerate(FRAMES_PER_SECOND)
        wav_file.writeframes(
            bytes(create_sound_wave(duration=2.5, sample_rate=FRAMES_PER_SECOND))
        )
    return filename


def test_audio_publish(client: WeaveClient, make_wav_file: str) -> None:
