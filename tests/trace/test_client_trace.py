...
def test_threads_query_endpoint(client):
    ...
    turn_counts = [t.turn_count for t in count_sorted_res.threads]
...
def test_threads_query_aggregation_fields(client):
    """Test the new aggregation fields in threads query: first_call_id, latest_call_id, and duration percentiles."""
    import time
    ...
    assert hasattr(test_thread, "first_call_id")
    assert hasattr(test_thread, "latest_call_id")
    assert hasattr(test_thread, "p50_call_duration_ms")
    assert hasattr(test_thread, "p99_call_duration_ms")

    # Test that aggregation fields have valid values
    assert test_thread.first_call_id is not None
    assert test_thread.latest_call_id is not None
    assert isinstance(test_thread.first_call_id, str)
    assert isinstance(test_thread.latest_call_id, str)

    # Duration percentiles should be numeric and reasonable
    assert test_thread.p50_call_duration_ms is not None
    assert test_thread.p99_call_duration_ms is not None
    assert isinstance(test_thread.p50_call_duration_ms, (int, float))
    assert isinstance(test_thread.p99_call_duration_ms, (int, float))
    ...
    assert 5 <= test_thread.p50_call_duration_ms <= 200  # Reasonable range for P50
    assert 5 <= test_thread.p99_call_duration_ms <= 200  # Reasonable range for P99
    assert (
        test_thread.p50_call_duration_ms <= test_thread.p99_call_duration_ms
    )  # P99 >= P50

    # Verify first_call_id and latest_call_id point to actual calls
    ...
    for call in all_calls:
        if call.thread_id == test_thread_id:
            ...
            if call.id == test_thread.first_call_id:
                first_call = call
            if call.id == test_thread.latest_call_id:
                latest_call = call

    assert first_call is not None, (
        f"Could not find first_call_id {test_thread.first_call_id}"
    )
    assert latest_call is not None, (
        f"Could not find latest_call_id {test_thread.latest_call_id}"
    )

    # Test that first_call_id has the earliest start_time
    earliest_start = min(call.started_at for call in thread_calls)
    assert first_call.started_at == earliest_start

    # Test that latest_call_id has the latest end_time (last_updated)
    ...
    # Test sorting by p50_call_duration_ms
    sorted_by_p50 = client.server.threads_query(
        tsi.ThreadsQueryReq(
            project_id=get_client_project_id(client),
            sort_by=[tsi.SortBy(field="p50_call_duration_ms", direction="desc")],
        )
    )
    assert len(sorted_by_p50.threads) >= 1

    # Test sorting by p99_call_duration_ms
    sorted_by_p99 = client.server.threads_query(
        tsi.ThreadsQueryReq(
            project_id=get_client_project_id(client),
            sort_by=[tsi.SortBy(field="p99_call_duration_ms", direction="asc")],
        )
    )
    assert len(sorted_by_p99.threads) >= 1
