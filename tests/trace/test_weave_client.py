...
def test_table_create_parallel_upload(client):
    ...
    seq_res = client.server.table_create(
        TableCreateReq(
            table=TableSchemaForInsert(project_id=client._project_id(), rows=data)
        )
    )

    par_res = client.server.table_create(
        TableCreateReq(
            table=TableSchemaForInsert(
                project_id=client._project_id(),
                rows=data,  # Same data as sequential
                row_order=[0, 1, 2, 3],  # Same order as sequential
            )
        )
    )

    rev_res = client.server.table_create(
        TableCreateReq(
            table=TableSchemaForInsert(
                project_id=client._project_id(), rows=data, row_order=[3, 2, 1, 0]
            )
        )
    )

    assert par_res.digest == seq_res.digest
    assert rev_res.digest != seq_res.digest

    # Test actual parallel upload: different row order with reordering
    parallel_reorder_res = client.server.table_create(
        TableCreateReq(
            table=TableSchemaForInsert(
                project_id=client._project_id(),
                rows=[
                    ...
                    data[3],
                    data[1],
                ],  # Uploaded in different order
                row_order=[
                    1,
                    0,
                    3,
                    2,
                ],  # Final order: data[0], data[2], data[1], data[3]
            )
        )
    )
    # Should produce different digest due to different final order
    assert parallel_reorder_res.digest != seq_res.digest

    # Verify the final order is correct
    parallel_query = client.server.table_query(
        tsi.TableQueryReq(
            project_id=client._project_id(), digest=parallel_reorder_res.digest
        )
    )
    assert [r.val["i"] for r in parallel_query.rows] == [0, 2, 1, 3]

    rev_query = client.server.table_query(
        tsi.TableQueryReq(project_id=client._project_id(), digest=rev_res.digest)
    )
    assert [r.val["i"] for r in rev_query.rows] == [3, 2, 1, 0]

    # Edge cases
    with pytest.raises(ValueError, match="row_order length"):
        client.server.table_create(
            TableCreateReq(
                table=TableSchemaForInsert(
                    project_id=client._project_id(), rows=data, row_order=[0, 1]
                )
            )
        )

    with pytest.raises(ValueError, match="row_order indices"):
        client.server.table_create(
            TableCreateReq(
                table=TableSchemaForInsert(
                    project_id=client._project_id(), rows=data, row_order=[0, 1, 2, 5]
                )
            )
        )


def test_table_update_parallel_upload_edge_cases(client):
    base_data = [{"id": i} for i in range(3)]
    base_res = client.server.table_create(
        TableCreateReq(
            table=TableSchemaForInsert(project_id=client._project_id(), rows=base_data)
        )
    )

    # Multiple workers with different operations
    w1_res = client.server.table_update(
        tsi.TableUpdateReq(
            project_id=client._project_id(),
            base_digest=base_res.digest,
            updates=[
                tsi.TableAppendSpec(append=tsi.TableAppendSpecPayload(row={"id": 3})),
                tsi.TableInsertSpec(
                    insert=tsi.TableInsertSpecPayload(index=0, row={"id": 4})
                ),
            ],
            row_order=[4, 0, 1, 2, 3],
        )
    ...
    w2_res = client.server.table_update(
        tsi.TableUpdateReq(
            project_id=client._project_id(),
            base_digest=base_res.digest,
            updates=[tsi.TablePopSpec(pop=tsi.TablePopSpecPayload(index=1))],
            row_order=[0, 1],
        )
    )

    w3_res = client.server.table_update(
        tsi.TableUpdateReq(
            project_id=client._project_id(),
            base_digest=base_res.digest,
            updates=[],
    ...
    assert len({w1_res.digest, w2_res.digest, w3_res.digest}) == 3

    w1_query = client.server.table_query(
        tsi.TableQueryReq(project_id=client._project_id(), digest=w1_res.digest)
    )
    w2_query = client.server.table_query(
        tsi.TableQueryReq(project_id=client._project_id(), digest=w2_res.digest)
    )
    w3_query = client.server.table_query(
        tsi.TableQueryReq(project_id=client._project_id(), digest=w3_res.digest)
    )

    # w1: append 3, insert 4 at 0, then reorder with [4, 0, 1, 2, 3]
    # After updates: [4, 0, 1, 2, 3] (4 inserted at 0, 3 appended)
    # row_order [4, 0, 1, 2, 3] means: final[0]=current[4], final[1]=current[0], etc.
    # So final result should be: [3, 4, 0, 1, 2]
    assert [r.val["id"] for r in w1_query.rows] == [3, 4, 0, 1, 2]
    assert [r.val["id"] for r in w2_query.rows] == [0, 2]
    assert [r.val["id"] for r in w3_query.rows] == [2, 1, 0]

    # Error cases
    with pytest.raises(ValueError, match="row_order length"):
        client.server.table_update(
            tsi.TableUpdateReq(
                project_id=client._project_id(),
                base_digest=base_res.digest,
                updates=[
                    tsi.TableAppendSpec(
                        append=tsi.TableAppendSpecPayload(row={"id": 5})
                    )
                ],
                row_order=[0, 1],
            )
    ...
    with pytest.raises(ValueError, match="Index out of range"):
        client.server.table_update(
            tsi.TableUpdateReq(
                project_id=client._project_id(),
                base_digest=base_res.digest,
                updates=[tsi.TablePopSpec(pop=tsi.TablePopSpecPayload(index=10))],
            )
        )
