...
row_digest) to their provenance sources. A source is either a Weave call
(source_kind='call') or an agent span (source_kind='span'). This is the
...
exercise both backends via the `client` fixture (SQLite by default,
ClickHouse with --trace-server=clickhouse). dataset_sources has a real
SQLite implementation, so unlike annotation queues these tests do NOT skip
on SQLite -- EXCEPT the span-kind tests, which require the ClickHouse-only
`spans` table and are guarded accordingly.

Notes on test data:
- Call sources: real calls are created via an @weave.op so a real call_id
  exists in calls_merged for link validation.
- Span sources: spans live only in the ClickHouse `spans` table (populated
  via OTEL ingestion / direct insert), so span-kind tests are ClickHouse-only.
...
def test_dataset_sources_conversation_five_calls_one_row_digest(client):
    ...
    assert digest in membership.row_digests
...
def test_dataset_sources_reverse_query_row_digests_truncation(client):
    ...
    for i in range(total):
        client.server.dataset_sources_link(
            link_req(
                ...
                links=[
                    tsi.DatasetSourceLinkPayload(
                        row_digest=new_digest(f"trunc{i}"),
    ...
    assert len(membership.row_digests) == tsi.MAX_ROW_DIGESTS_PER_RESULT
...
# ---------------------------------------------------------------------------
# Backend parity: relink-overwrite sequence with exact end state
# ---------------------------------------------------------------------------


def test_dataset_sources_relink_overwrite_sequence_end_state_parity(client):
    """Backend parity: link -> relink -> delete -> relink must end in the same
    observable state on both SQLite and ClickHouse: exactly one visible link,
    same deterministic id throughout, deleted_at None.
    """
    calls = create_test_calls(client, 1)
    call = calls.calls[0]
    ds_obj = "parity_dataset"
    digest = new_digest("parity")
    payload = tsi.DatasetSourceLinkPayload(
        row_digest=digest, sources=[call_source(call)]
    )

    # 1) Link (new).
    r1 = client.server.dataset_sources_link(
        link_req(
            client,
            dataset_object_id=ds_obj,
            links=[payload],
    ...
    link_id = r1.entries[0].link_id
    assert r1.entries[0].created is True

    # 2) Relink (existing).
    r2 = client.server.dataset_sources_link(
        link_req(
            client,
            dataset_object_id=ds_obj,
            links=[payload],
    ...
    assert r2.entries[0].link_id == link_id
    assert r2.entries[0].created is False

    # 3) Delete.
    d = client.server.dataset_sources_link_delete(
        tsi.DatasetSourcesLinkDeleteReq(
            project_id=client.project_id, link_ids=[link_id], wb_user_id="test_user"
        )
    )
    assert d.entries[0].deleted is True
    assert len(forward_query(client, dataset_object_id=ds_obj).links) == 0

    # 4) Relink (restore).
    r3 = client.server.dataset_sources_link(
        link_req(
            client,
            dataset_object_id=ds_obj,
            links=[payload],
    ...
    assert r3.entries[0].link_id == link_id
    assert r3.entries[0].created is False

    # Exact end state: exactly one visible link, same id, deleted_at None.
    fq = forward_query(client, dataset_object_id=ds_obj)
    assert len(fq.links) == 1
    assert fq.links[0].id == link_id
    assert fq.links[0].deleted_at is None
    assert fq.links[0].row_digest == digest
    assert fq.links[0].source_id == call.id
...
def _insert_span(client, *, span_id: str, trace_id: str, span_name: str) -> None:
    ...
    Mirrors tests/trace_server/test_genai_agent_queries.py. Span-kind tests
    are ClickHouse-only because SQLite has no `spans` table.
    """
...
def test_dataset_sources_link_span_source_forward_query(client):
    """Span-kind source link + forward query. ClickHouse-only: spans live only
    in the CH `spans` table; SQLite has no spans table to validate against.
    """
    if client_is_sqlite(client):
        pytest.skip("Span sources require the ClickHouse-only `spans` table")
