...
def test_annotation_queues_stats(client):
    ...
    # Get queue_item_ids and call_ids for Queue 0
    result = ch_client.query(
        f"SELECT id, call_id FROM annotation_queue_items WHERE project_id = '{internal_project_id}' AND queue_id = '{queue_ids[0]}' AND deleted_at IS NULL ORDER BY created_at LIMIT 3"
    )
    queue0_items = [(row[0], row[1]) for row in result.result_rows]
    assert len(queue0_items) == 3

    # Mark first 2 items of Queue 0 as completed by inserting progress records
    from weave.trace_server.ids import generate_id
    for item_id, call_id in queue0_items[:2]:
        progress_id = generate_id()
        ch_client.command(
            f"""
            INSERT INTO annotator_queue_items_progress (id, project_id, queue_item_id, queue_id, call_id, annotator_id, annotation_state)
            VALUES ('{progress_id}', '{internal_project_id}', '{item_id}', '{queue_ids[0]}', '{call_id}', 'test_annotator', 'completed')
            """
        )

    # Get queue_item_ids and call_ids for Queue 1
    result = ch_client.query(
        f"SELECT id, call_id FROM annotation_queue_items WHERE project_id = '{internal_project_id}' AND queue_id = '{queue_ids[1]}' AND deleted_at IS NULL ORDER BY created_at LIMIT 5"
    )
    queue1_items = [(row[0], row[1]) for row in result.result_rows]
    assert len(queue1_items) == 5

    # Mark first 3 items of Queue 1 as completed by inserting progress records
    for item_id, call_id in queue1_items[:3]:
        progress_id = generate_id()
        ch_client.command(
            f"""
            INSERT INTO annotator_queue_items_progress (id, project_id, queue_item_id, queue_id, call_id, annotator_id, annotation_state)
            VALUES ('{progress_id}', '{internal_project_id}', '{item_id}', '{queue_ids[1]}', '{call_id}', 'test_annotator', 'completed')
            """
        )

    # Mark 4th item of Queue 1 as skipped by inserting a progress record
    item_id, call_id = queue1_items[3]
    progress_id = generate_id()
    ch_client.command(
        f"""
        INSERT INTO annotator_queue_items_progress (id, project_id, queue_item_id, queue_id, call_id, annotator_id, annotation_state)
        VALUES ('{progress_id}', '{internal_project_id}', '{item_id}', '{queue_ids[1]}', '{call_id}', 'test_annotator', 'skipped')
        """
    )

    # Get queue_item_ids and call_ids for Queue 2
    result = ch_client.query(
        f"SELECT id, call_id FROM annotation_queue_items WHERE project_id = '{internal_project_id}' AND queue_id = '{queue_ids[2]}' AND deleted_at IS NULL ORDER BY created_at LIMIT 7"
    )
    queue2_items = [(row[0], row[1]) for row in result.result_rows]
    assert len(queue2_items) == 7

    # Mark first 4 items of Queue 2 as skipped by inserting progress records
    for item_id, call_id in queue2_items[:4]:
        progress_id = generate_id()
        ch_client.command(
            f"""
            INSERT INTO annotator_queue_items_progress (id, project_id, queue_item_id, queue_id, call_id, annotator_id, annotation_state)
            VALUES ('{progress_id}', '{internal_project_id}', '{item_id}', '{queue_ids[2]}', '{call_id}', 'test_annotator', 'skipped')
            """
        )
