...
def test_annotation_queue_create_and_read(client):
    ...
    assert read_res.queue.claim_timeout_seconds == 1800
    # Verify due date was saved correctly
    assert read_res.queue.due_date is not None
    time_diff = abs((read_res.queue.due_date - due_date).total_seconds())
    assert time_diff < 1.0, f"Due date mismatch: expected {due_date}, got {read_res.queue.due_date}"
...
def test_annotation_queue_multiple_scorer_refs(client):
    ...
    assert "weave:///entity/project/scorer/accuracy:abc123" in read_res.queue.scorer_refs
    assert "weave:///entity/project/scorer/relevance:def456" in read_res.queue.scorer_refs
