...
@pytest.mark.disable_logging_error_check
def test_http_422_does_not_disable_fast_path(
    client: WeaveClient, fast_path: None, monkeypatch, caplog
) -> None:
    """A 422 is not a digest mismatch (only 409 is), so the fast path stays enabled."""
    original_obj_create = client.server.server.obj_create
    seen_expected_digests: list[str | None] = []
    ...
    weave.publish({"first": True}, name="http-422-first")
    client._flush()

    # 422 should NOT disable the fast path — only 409 (digest mismatch) does.
    assert not client._client_side_digests_disabled_event.is_set()
    assert client._internal_project_id is not None
    assert not any(
        "disabling fast path for this session" in record.getMessage()
        for record in caplog.records
        if record.name == "weave.trace.weave_client"
    ...
    # Second publish should still use the fast path (expected_digest is non-None).
    weave.publish({"second": True}, name="http-422-second")
    client._flush()

    assert all(d is not None for d in seen_expected_digests)
...
@pytest.mark.disable_logging_error_check
def test_file_digest_mismatch_disables_fast_path(
    client: WeaveClient, fast_path: None, monkeypatch, caplog
) -> None:
    """A file digest mismatch from the server must disable the fast path,
    just like object and table mismatches do.
    """
    original_file_create = client.server.server.file_create

    def always_mismatch_file(req: tsi.FileCreateReq):
        if req.expected_digest is not None:
            raise DigestMismatchError("forced file mismatch")
        return original_file_create(req)

    monkeypatch.setattr(client.server.server, "file_create", always_mismatch_file)
    caplog.set_level(logging.WARNING, logger="weave.trace.weave_client")

    # Publish a PIL Image — this triggers file_create with an expected_digest.
    img = Image.new("RGB", (2, 2), color="red")
    weave.publish(img, name="file-mismatch-test")
    client._flush()

    assert client._client_side_digests_disabled_event.is_set(), (
        "Fast path should be disabled after a file digest mismatch"
    )
    ...
    assert len(mismatch_warnings) == 1
    assert "file://" in mismatch_warnings[0].getMessage()


@pytest.mark.disable_logging_error_check
def test_file_uploads_omit_expected_digest_after_session_disable(
    client: WeaveClient, fast_path: None, monkeypatch, caplog
) -> None:
    """After a file digest mismatch disables the fast path, subsequent file
    uploads must NOT send expected_digest.

    This is the regression test for the bug where _build_result_from_encoded
    only checked the global setting (should_enable_client_side_digests) and
    ignored the per-session _client_side_digests_disabled_event flag, so files
    remained on the fast path even after objects/tables had fallen back.
    """
    file_create_reqs: list[tsi.FileCreateReq] = []
    original_file_create = client.server.server.file_create

    call_count = 0

    def mismatch_then_record(req: tsi.FileCreateReq):
        nonlocal call_count
        call_count += 1
        file_create_reqs.append(req)
        # First call with expected_digest triggers the mismatch.
        if call_count == 1 and req.expected_digest is not None:
            raise DigestMismatchError("forced file mismatch")
        return original_file_create(req)

    monkeypatch.setattr(client.server.server, "file_create", mismatch_then_record)
    caplog.set_level(logging.WARNING, logger="weave.trace.weave_client")

    # First publish — triggers mismatch and disables fast path.
    img1 = Image.new("RGB", (2, 2), color="red")
    weave.publish(img1, name="file-disable-test-1")
    client._flush()

    assert client._client_side_digests_disabled_event.is_set()

    # Second publish — must NOT send expected_digest on file_create.
    file_create_reqs.clear()
    img2 = Image.new("RGB", (2, 2), color="blue")
    weave.publish(img2, name="file-disable-test-2")
    client._flush()

    # All file_create requests from the second publish should have
    # expected_digest=None because the session disabled the fast path.
    assert len(file_create_reqs) > 0, "Expected at least one file_create call"
    for req in file_create_reqs:
...
# ---------------------------------------------------------------------------
# Deferred execution: internal_project_id captured eagerly
# ---------------------------------------------------------------------------


def test_send_start_call_captures_internal_project_id_eagerly(
    client: WeaveClient, fast_path: None, monkeypatch
) -> None:
    """send_start_call must use the internal_project_id captured at call-creation
    time, not whatever _internal_project_id returns when the closure executes.

    Scenario: create a call while digests are enabled, then disable digests
    before the deferred closure runs.  The to_json call inside the closure
    should still receive a non-None internal_project_id.
    """
    original_executor = client.future_executor
    client.set_autoflush(False)
    client.future_executor = FutureExecutor(max_workers=1)
    ...
    captured_internal_ids: list[str | None] = []
    original_to_json = to_json

    def recording_to_json(*args, **kwargs):
        captured_internal_ids.append(kwargs.get("internal_project_id"))
        return original_to_json(*args, **kwargs)

    monkeypatch.setattr("weave.trace.weave_client.to_json", recording_to_json)

    try:
        # Block the executor so the deferred send_start_call can't run yet.
        client.future_executor.defer(block.wait)
        ...
        # Call the op — this captures internal_project_id eagerly.
        my_op.call(42)

        # Disable digests BEFORE the deferred closure executes.
        # If the closure read _internal_project_id lazily, it would get None.
        _configure_digests(client, enable=False)
        assert client._internal_project_id is None

        # Unblock and flush — the closure should use the captured (non-None) value.
        block.set()
        client._flush()

        # to_json is called for inputs (send_start_call) and outputs (send_end_call).
        # At least the start call's to_json must have received a non-None internal id.
        start_call_id = captured_internal_ids[0]
        assert start_call_id is not None, (
            "send_start_call should use eagerly captured internal_project_id, not None"
        )
    finally:
        client.future_executor = original_executor
        client.set_autoflush(True)
...
def test_send_end_call_captures_internal_project_id_eagerly(
    client: WeaveClient, fast_path: None, monkeypatch
) -> None:
    """send_end_call must use the internal_project_id captured at finish_call
    time, not whatever _internal_project_id returns when the closure executes.

    Scenario: call an op while digests are enabled, then disable digests
    before the deferred send_end_call executes.  The to_json call inside the
    closure should still receive a non-None internal_project_id.
    """
    original_executor = client.future_executor
    client.set_autoflush(False)
    client.future_executor = FutureExecutor(max_workers=1)
    ...
    try:
        ...
        # Let the start call go through normally.
        my_op.call()
        client._flush()

        # Now block the executor and install the recording hook.
        client.future_executor.defer(block.wait)
        monkeypatch.setattr("weave.trace.weave_client.to_json", recording_to_json)

        # Call the op again — send_end_call is deferred.
        my_op.call()

        # Disable digests BEFORE the deferred closures execute.
        _configure_digests(client, enable=False)
        assert client._internal_project_id is None
        ...
        # The captured ids should include calls from both send_start_call and
        # send_end_call.  All should be non-None since they were captured eagerly.
        assert len(captured_internal_ids) >= 2, (
            f"Expected at least 2 to_json calls, got {len(captured_internal_ids)}"
        )
        for i, iid in enumerate(captured_internal_ids):
            assert iid is not None, (
                f"to_json call {i} should use eagerly captured internal_project_id, not None"
            )
    finally:
        client.future_executor = original_executor
        client.set_autoflush(True)
