...
from weave.actions_worker.tasks import llm_judge, wordcount
...
def test_action_llm_judge(mock_apply_async):
    ...
    mock_publish_results = patch(
        "weave.actions_worker.tasks.publish_results_as_feedback"
    ).start()
    mock_ack_on_clickhouse = patch(
        "weave.actions_worker.tasks.ack_on_clickhouse"
    ).start()
    ...
    mock_publish_results.assert_called_once_with(
        ctx, {"llm_judge": "I'm sorry Hal, I'm afraid I can't do that."}
    )
    mock_ack_on_clickhouse.assert_called_once_with(ctx, True)
...
def test_action_wordcount(mock_apply_async):
    ...
    mock_publish_results.assert_called_once_with(ctx, {"wordcount": 4})
    mock_ack_on_clickhouse.assert_called_once_with(ctx, True)
    # Assert that the function returned the expected output
    assert out == 4
