...
@pytest.mark.vcr(
    filter_headers=["authorization", "x-api-key"], allowed_hosts=["api.wandb.ai", "api.exa.ai", "localhost"]
)
def test_search(
    ...
    if hasattr(api_response, "cost_dollars"):
        ...
        assert call.summary["usage"]["exa"]["prompt_token_cost"] == float(output.cost_dollars.total)
