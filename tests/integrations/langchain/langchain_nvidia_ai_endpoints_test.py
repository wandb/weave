...
def test_chatnvidia_quickstart(client: weave.trace.weave_client.WeaveClient) -> None:
    ...
    nvidia_client = ChatNVIDIA(api_key=api_key, model=model)
    ...
    assert op_name_from_ref(call.op_name) == "langchain.Llm.ChatNVIDIA._generate"
    assert call.started_at is not None
    assert call.started_at < call.ended_at  # type: ignore

    output = call.output
    assert output["model"] == "meta/llama-3.1-8b-instruct"
    assert output["object"] == "chat.completion"

    usage = call.usage_metadata  # type: ignore
    assert usage["input_tokens"] == 12
    assert usage["output_tokens"] == 25
    assert usage["total_tokens"] == 37

    inputs = call.inputs
    assert inputs["model"] == "meta/llama-3.1-8b-instruct"
    assert inputs["messages"] == [{"role": "human", "content": "Hello!"}]
    assert inputs["max_tokens"] == 1024
    assert inputs["temperature"] is None
    assert inputs["top_p"] is None
...
def test_chatnvidia_async_quickstart(client: weave.trace.weave_client.WeaveClient) -> None:
    api_key = os.environ.get("NVIDIA_API_KEY", "DUMMY_API_KEY")

    nvidia_client = ChatNVIDIA(api_key=api_key, model=model)

    response = nvidia_client.ainvoke("Hello!")
