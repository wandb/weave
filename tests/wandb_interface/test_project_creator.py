...
def test_ensure_project_exists_malformed_upsert_response(mock_api_with_no_project):
    """Test project creation with malformed upsert response."""
    mock_api.project.return_value = None
    mock_api.upsert_project.return_value = {"data": "something"}

    with pytest.raises(UnableToCreateProjectError):
        ensure_project_exists("test_entity", "test_project")


def test_ensure_project_exists_project_response_empty_project(mock_api_with_no_project):
    """Test project creation when project response has null project."""
    mock_api.project.return_value = {"project": None}
    mock_api.upsert_project.return_value = {
        "upsertModel": {"model": {"name": "new_project"}}
    }
...
def test_ensure_project_exists_upsert_response_null_model(mock_api_with_no_project):
    """Test project creation when upsert response has null model."""
    mock_api.project.return_value = None
    mock_api.upsert_project.return_value = {"upsertModel": {"model": None}}

    with pytest.raises(UnableToCreateProjectError):
        ensure_project_exists("test_entity", "test_project")


def test_ensure_project_exists_upsert_response_no_upsert_model(mock_api):
