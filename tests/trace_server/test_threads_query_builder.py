...
def test_clickhouse_basic_query():
    ...
    assert_clickhouse_sql(
        ...
        SELECT
            ...
            argMin(id, call_start_time) as first_call_id,     -- Call ID with earliest start_time
            argMax(id, call_end_time) as latest_call_id,   -- Call ID with latest last_updated
            quantile(0.5)(call_duration) as p50_call_duration_ms,  -- P50 of call durations in milliseconds
            quantile(0.99)(call_duration) as p99_call_duration_ms  -- P99 of call durations in milliseconds
        FROM (
...
def test_validate_and_map_sort_field():
    ...
    assert (
        _validate_and_map_sort_field("p50_call_duration_ms") == "p50_call_duration_ms"
    )
    assert (
        _validate_and_map_sort_field("p99_call_duration_ms") == "p99_call_duration_ms"
    )

    # Test invalid fields - call IDs should not be sortable since they're just identifiers
    with pytest.raises(ValueError) as exc_info:
        _validate_and_map_sort_field("first_call_id")
    assert "Unsupported sort field: first_call_id" in str(exc_info.value)

    with pytest.raises(ValueError) as exc_info:
        _validate_and_map_sort_field("latest_call_id")
    assert "Unsupported sort field: latest_call_id" in str(exc_info.value)

    with pytest.raises(ValueError) as exc_info:
        _validate_and_map_sort_field("invalid_field")
    assert "Unsupported sort field: invalid_field" in str(exc_info.value)
    assert (
        "Supported fields: ['thread_id', 'turn_count', 'start_time', 'last_updated', 'p50_call_duration_ms', 'p99_call_duration_ms']"
