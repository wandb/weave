...
def test_adapter_scopes_python_cost_hydration_to_wandb_projects() -> None:
    """Verifies the adapter wraps each request in the internal subclass and
    sets `use_python_cost_hydration` based purely on entity prefix — public
    `CallsQueryReq`/`CallReadReq` cannot carry the flag at all.
    """
    inner = MagicMock(spec=tsi.FullTraceServerInterface)
