...
def test_otel_export_recovers_after_placeholder_create_failure(
    trace_server, clickhouse_trace_server, monkeypatch
):
    """Failed writes must be retried on the next export since the op won't be in _inserted_ops."""
    ...
    # First attempt fails — op is NOT added to _inserted_ops
    ...
    # Second attempt succeeds — op is retried because it wasn't in _inserted_ops
    trace_server.otel_export(
        _create_otel_export_req(project_id, [_create_otel_span(op_name)])
    )
...
def test_otel_export_uses_placeholder_even_with_existing_real_op(
    trace_server, clickhouse_trace_server
):
    """OTel always uses the deterministic placeholder digest, even when a real op exists.

    This is by design: OTel ingest is write-only (no CH reads) for performance.
    The placeholder op version is inserted idempotently alongside the real one.
    """
    ...
    # Both the real op and the placeholder op exist as separate versions
    assert len(objs) == 2
