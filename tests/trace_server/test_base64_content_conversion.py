...
class TestStandaloneBase64Detection:
    ...
    def test_non_media_base64_strings_not_converted(self):
        ...
        for test_str in test_cases:
            # The behavior under test is that short / low-entropy base64-alphabet
            # strings round-trip unchanged. is_base64 may now reject some of
            # these via the entropy guard before the regex runs; that is the
            # same observable outcome (no conversion) reached more cheaply.
...
class TestFastRejectPaths:
    ...
    def test_low_entropy_long_string_does_not_invoke_storage(self):
        # The pathological case from the DD trace. Must short-circuit on the
        # entropy guard, never reaching Content.from_base64.
        trace_server = MagicMock()
        pathological = "x" * 50000
        result = replace_base64_with_content_objects(
            {"bait": pathological}, "test_project", trace_server
        )
