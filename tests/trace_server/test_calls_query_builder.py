...
def test_rolled_up_storage_size():
    """Test querying with rolled up storage size"""
    cq = CallsQuery(project_id="test/project", include_rolled_up_storage_size=True)
    cq.add_field("id")
    cq.add_field("total_size_bytes")
...
def test_aggregated_data_size_field():
    ...
    assert "rolled_up_cms.total_size_bytes" in sql
