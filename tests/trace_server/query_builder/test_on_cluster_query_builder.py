...
def test_query_without_cluster_unchanged() -> None:
    """Test that queries without cluster_name are unchanged (no ON CLUSTER added).

    This ensures backward compatibility when not running in distributed mode.
    """
    cq = CallsQuery(project_id="project")  # No cluster_name set
    cq.add_field("id")
