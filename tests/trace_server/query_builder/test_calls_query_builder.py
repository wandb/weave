...
def test_query_heavy_column_simple_filter_with_order_and_limit_and_mixed_query_conditions() -> (
    ...
    assert_sql(
        ...
        WITH filtered_calls AS (
            SELECT
                calls_merged.id AS id
            FROM calls_merged PREWHERE calls_merged.project_id = {pb_9:String}
    ...
    assert_sql(
        ...
        """
            ...
            LEFT JOIN (
                ...
                {pb_4:String},
                '/call/',
                calls_merged.id)) PREWHERE calls_merged.project_id = {pb_4:String}
            GROUP BY
    ...
    assert_sql(
        ...
        FROM
            calls_merged
                ...
                {pb_5:String},
                '/call/',
                calls_merged.id)) PREWHERE calls_merged.project_id = {pb_5:String}
            WHERE ((calls_merged.op_name IN {pb_0:Array(String)})
                    OR (calls_merged.op_name IS NULL))
        GROUP BY
            (calls_merged.project_id,
...
def test_query_with_simple_feedback_filter() -> None:
    ...
    assert_sql(
        ...
        FROM
            ...
            {pb_3:String},
            '/call/',
            calls_merged.id)) PREWHERE calls_merged.project_id = {pb_3:String}
        GROUP BY
            (calls_merged.project_id,
            calls_merged.id)
        HAVING
...
def test_query_with_simple_feedback_sort_and_filter() -> None:
    ...
    assert_sql(
        ...
        FROM
            ...
            {pb_6:String},
            '/call/',
            calls_merged.id)) PREWHERE calls_merged.project_id = {pb_6:String}
        GROUP BY
            (calls_merged.project_id,
            calls_merged.id)
        HAVING
...
def test_calls_query_with_predicate_filters_multiple_heavy_conditions() -> None:
    ...
    assert_sql(
        ...
        WITH filtered_calls AS (
            SELECT
                calls_merged.id AS id
            FROM calls_merged PREWHERE calls_merged.project_id = {pb_7:String}
...
def test_calls_query_with_or_between_start_and_end_fields() -> None:
    ...
    assert_sql(
        ...
        SELECT
            ...
            any(calls_merged.inputs_dump) AS inputs_dump,
            any(calls_merged.output_dump) AS output_dump
        FROM calls_merged PREWHERE calls_merged.project_id = {pb_6:String}
...
def test_calls_query_with_complex_heavy_filters() -> None:
    ...
    assert_sql(
        ...
        WITH filtered_calls AS (
            SELECT
                calls_merged.id AS id
            FROM calls_merged PREWHERE calls_merged.project_id = {pb_12:String}
                WHERE (
...
def test_storage_size_fields():
    ...
    assert_sql(
      ...
      GROUP BY id) AS storage_size_tbl ON calls_merged.id = storage_size_tbl.id PREWHERE calls_merged.project_id = {pb_0:String}
...
def test_total_storage_size():
    ...
    assert_sql(
        ...
        GROUP BY trace_id) AS rolled_up_cms
            ON calls_merged.trace_id = rolled_up_cms.trace_id PREWHERE calls_merged.project_id = {pb_0:String}
            GROUP BY (calls_merged.project_id, calls_merged.id)
        HAVING (
            ((any(calls_merged.deleted_at) IS NULL))
            AND ((NOT ((any(calls_merged.started_at) IS NULL))))
...
def test_query_with_feedback_filter_and_datetime_and_string_filter() -> None:
    ...
    assert_sql(
        ...
        WITH filtered_calls AS
            (SELECT calls_merged.id AS id
            FROM calls_merged
                            LEFT JOIN (SELECT * FROM feedback WHERE feedback.project_id = {pb_8:String} ) AS feedback ON (feedback.weave_ref = concat('weave-trace-internal:///', {pb_8:String}, '/call/', calls_merged.id)) PREWHERE calls_merged.project_id = {pb_8:String}
            ...
            HAVING (((coalesce(nullIf(JSON_VALUE(anyIf(feedback.payload_dump, feedback.feedback_type = {pb_0:String}), {pb_1:String}), 'null'), '') > coalesce(nullIf(JSON_VALUE(anyIf(feedback.payload_dump, feedback.feedback_type = {pb_0:String}), {pb_2:String}), 'null'), '')))
                ...
                AND ((NOT ((any(calls_merged.started_at) IS NULL))))))
        SELECT calls_merged.id AS id
        FROM calls_merged PREWHERE calls_merged.project_id = {pb_8:String}
            WHERE (calls_merged.id IN filtered_calls)
        GROUP BY (calls_merged.project_id,
                calls_merged.id)
        """,
...
def test_query_filter_with_escaped_dots_in_field_names() -> None:
    ...
    assert_sql(
        ...
        """
        SELECT calls_merged.id AS id
        FROM calls_merged PREWHERE calls_merged.project_id = {pb_3:String}
...
def test_calls_complete_order_by_feedback_no_aggregate() -> None:
    ...
    assert_sql(
        ...
        LEFT JOIN (
            ...
            {pb_3:String},
            '/call/',
            calls_complete.id)) PREWHERE calls_complete.project_id = {pb_3:String}
        WHERE (calls_complete.deleted_at IS NULL)
        ORDER BY
...
def test_calls_complete_filter_by_feedback_no_aggregate() -> None:
    ...
    assert_sql(
        ...
        LEFT JOIN (
            ...
            '/call/',
            calls_complete.id)) PREWHERE calls_complete.project_id = {pb_3:String}
        WHERE (((coalesce(nullIf(JSON_VALUE(if(feedback.feedback_type = {pb_0:String}, feedback.payload_dump, NULL),
...
def test_calls_complete_filter_and_order_by_feedback_no_aggregate() -> None:
    ...
    assert_sql(
        ...
        LEFT JOIN (
            ...
            {pb_6:String},
            '/call/',
            calls_complete.id)) PREWHERE calls_complete.project_id = {pb_6:String}
        WHERE (((coalesce(nullIf(JSON_VALUE(if(feedback.feedback_type = {pb_0:String}, feedback.payload_dump, NULL),
            {pb_1:String}), 'null'), '') = {pb_2:String}))
            AND ((calls_complete.deleted_at IS NULL)))
        ORDER BY
