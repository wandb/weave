...
def test_query_baseline(read_table: ReadTable, expected_table: str) -> None:
    ...
    if read_table == ReadTable.CALLS_MERGED:
        ...
        expected_query = f"""
            ...
            WHERE 1
              AND (
                ((
                    {expected_table}.deleted_at = {{pb_0:String}}
                ))
                AND
                ((
                   NOT ((
    ...
    if read_table == ReadTable.CALLS_MERGED:
        ...
        assert_sql(
            cq,
            expected_query,
            {"pb_0": "1970-01-01T00:00:00Z", "pb_1": "project"},
        )
...
def test_query_with_simple_feedback_filter_calls_complete() -> None:
    ...
    assert_sql(
        ...
        AND
            ...
            {pb_2:String}), 'null'), '')))
                AND ((calls_complete.deleted_at = {pb_3:String}))
    ...
    assert_sql(
        ...
        AND
            (((coalesce(nullIf(JSON_VALUE(calls_complete.inputs_dump, {pb_0:String}), 'null'), '') = {pb_1:String}))
                AND ((calls_complete.deleted_at = {pb_2:String}))
                    AND ((NOT ((calls_complete.started_at IS NULL)))))
        """,
        {
            "pb_0": '$."param"',
            "pb_1": "hello",
            "pb_2": "1970-01-01T00:00:00Z",
            "pb_3": '%"hello"%',
            "pb_4": "project",
        },
    )
...
def test_query_with_simple_feedback_sort_calls_complete() -> None:
    ...
    assert_sql(
        ...
        """
            ...
            WHERE 1
              AND (
                ((calls_complete.deleted_at = {pb_0:String}))
                    AND ((NOT ((calls_complete.started_at IS NULL)))))
            ORDER BY
        ...
        WHERE 1
          AND (
            ...
            (CASE
                ...
                ELSE {pb_3:String}
            END = {pb_1:String})))
            AND ((calls_complete.deleted_at = {pb_5:String}))
            AND ((NOT ((calls_complete.started_at IS NULL)))))
        """,
        {
            "pb_0": '$."status_counts"."error"',
    ...
    assert_sql(
        ...
        WHERE 1
          AND (
            ...
            AND ((calls_complete.deleted_at = {pb_1:String}))
            AND ((NOT ((calls_complete.started_at IS NULL)))))
        """,
        {
            "pb_0": "test_queue_id",
            "pb_1": "1970-01-01T00:00:00Z",
            "pb_2": "project",
        },
    )
...
def test_stats_query_calls_complete_flat_count() -> None:
    ...
    assert_stats_sql(
        ...
        WHERE 1
          AND (calls_complete.deleted_at = {pb_0:String})
        """,
        {
            "pb_0": "1970-01-01T00:00:00Z",
            "pb_1": "project",
