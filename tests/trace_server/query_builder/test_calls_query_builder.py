    ...
    if read_table == ReadTable.CALLS_MERGED:
        expected_query = f"""
            ...
            HAVING (
                ...
                ((
                   NOT ((
                  any({expected_table}.op_name) IS NULL
                   ))
                ))
            )
        """
    ...
    assert_sql(
        ...
        WITH filtered_calls AS (
            ...
            HAVING (
                ((any(calls_merged.deleted_at) IS NULL))
            AND
                ((NOT ((any(calls_merged.op_name) IS NULL))))
            )
            ORDER BY any(calls_merged.started_at) DESC
            LIMIT 10
        )
    ...
    assert_sql(
        ...
        filtered_calls AS (
            ...
            WHERE (calls_merged.id IN filter_candidate_ids)
                ...
                AND (ifNull(calls_merged.trace_id, '') = {pb_1:String} OR calls_merged.trace_id IS NULL)
            ...
            HAVING (
                ...
                AND ((coalesce(nullIf(JSON_VALUE(any(calls_merged.inputs_dump), {pb_7:String}), 'null'), '') = {pb_8:String}))
                AND ((any(calls_merged.wb_user_id) = {pb_9:String}))
    ...
    assert_sql(
        ...
        filtered_calls AS (
            ...
            HAVING (
                ...
                AND ((any(calls_merged.wb_user_id) = {pb_4:String}))
                AND ((any(calls_merged.deleted_at) IS NULL))
                AND ((NOT ((any(calls_merged.op_name) IS NULL))))
            )
        )
...
def test_calls_query_with_predicate_filters_multiple_heavy_conditions() -> None:
    ...
    # Heavy conditions spanning start-only (inputs) and end-only (output) fields
    # cannot be ANDed in the strict candidate CTE (no single un-merged row has
    # both), so no bloom candidate is emitted; the OR-IS-NULL outer path handles it.
    assert_sql(
        cq,
        """
        WITH filtered_calls AS (
            ...
            WHERE ((ifNull(calls_merged.inputs_dump, '') LIKE {pb_5:String} OR calls_merged.inputs_dump IS NULL)
                     AND (ifNull(calls_merged.output_dump, '') LIKE {pb_6:String} OR calls_merged.output_dump IS NULL))
            GROUP BY (calls_merged.project_id, calls_merged.id)
            HAVING (
...
def test_calls_query_with_or_between_start_and_end_fields() -> None:
    ...
    # Heavy conditions spanning start-only (inputs) and end-only (output) fields
    # skip the bloom candidate CTE (the strict per-row form is unsafe across
    # parts), falling back to the OR-IS-NULL outer path.
    assert_sql(
        cq,
        """
        SELECT
...
def test_calls_query_with_complex_heavy_filters() -> None:
    ...
    # The nested OR puts an end-only field (output) inside an AND with start-only
    # fields (inputs), so no single un-merged row can satisfy the strict candidate;
    # the bloom candidate is skipped in favor of the OR-IS-NULL outer path.
    assert_sql(
        cq,
        """
        WITH filtered_calls AS (
            ...
            WHERE ((ifNull(calls_merged.inputs_dump, '') LIKE {pb_9:String} OR calls_merged.inputs_dump IS NULL)
                AND ((ifNull(calls_merged.output_dump, '') LIKE {pb_10:String} OR calls_merged.output_dump IS NULL)
                    OR (lower(ifNull(calls_merged.inputs_dump, '')) LIKE {pb_11:String} OR calls_merged.inputs_dump IS NULL)))
            GROUP BY (calls_merged.project_id, calls_merged.id)
            HAVING (
                ((coalesce(nullIf(JSON_VALUE(any(calls_merged.inputs_dump), {pb_0:String}), 'null'), '') = {pb_1:String}))
                AND
...
def test_calls_query_with_like_optimization_calls_complete() -> None:
    ...
    assert_sql(
        ...
        WHERE (ifNull(calls_complete.inputs_dump, '') LIKE {pb_3:String})
        AND
...
def test_calls_query_with_like_optimization_contains_calls_complete() -> None:
    ...
    assert_sql(
        ...
        FROM calls_complete
        PREWHERE calls_complete.project_id = {pb_4:String}
        WHERE (lower(ifNull(calls_complete.inputs_dump, '')) LIKE {pb_3:String})
        AND
...
def test_calls_query_with_like_optimization_in_calls_complete() -> None:
    ...
    assert_sql(
        ...
        WHERE ((ifNull(calls_complete.inputs_dump, '') LIKE {pb_4:String} OR ifNull(calls_complete.inputs_dump, '') LIKE {pb_5:String}))
        AND
...
def test_calls_query_with_combined_like_optimizations_and_op_filter() -> None:
    ...
    assert_sql(
        ...
        filtered_calls AS (
            ...
            GROUP BY (calls_merged.project_id, calls_merged.id)
            HAVING (
                ((coalesce(nullIf(JSON_VALUE(any(calls_merged.attributes_dump), {pb_6:String}), 'null'), '') = {pb_7:String}))
                AND
                (positionCaseInsensitive(coalesce(nullIf(JSON_VALUE(any(calls_merged.inputs_dump), {pb_8:String}), 'null'), ''), {pb_9:String}) > 0)
                AND
                ((coalesce(nullIf(JSON_VALUE(any(calls_merged.attributes_dump), {pb_10:String}), 'null'), '') IN ({pb_11:String},{pb_12:String})))
                AND ((any(calls_merged.deleted_at) IS NULL))
                AND ((NOT ((any(calls_merged.op_name) IS NULL))))
            )
        )
        ...
        """,
        {
            ...
            "pb_11": "0.7",
            "pb_12": "0.8",
            "pb_13": ["llm/openai", "llm/anthropic"],
        },
    )
...
def test_literal_inferred_casts_cover_feedback_and_mixed_numeric_in() -> None:
    ...
    assert_sql(
        ...
        WITH filter_candidate_ids AS (
            ...
            WHERE ((ifNull(calls_merged.inputs_dump, '') LIKE {pb_0:String}
                    OR ifNull(calls_merged.inputs_dump, '') LIKE {pb_1:String}
                    OR ifNull(calls_merged.inputs_dump, '') LIKE {pb_2:String})))
        SELECT calls_merged.id AS id
        FROM calls_merged
        PREWHERE calls_merged.project_id = {pb_3:String}
        WHERE (calls_merged.id IN filter_candidate_ids)
        ...
        HAVING (((toFloat64OrNull(coalesce(nullIf(JSON_VALUE(any(calls_merged.inputs_dump),
            {pb_4:String}), 'null'), '')) IN ({pb_5:Int64},{pb_6:Float64})))
            AND ((any(calls_merged.deleted_at) IS NULL))
            AND ((NOT ((any(calls_merged.op_name) IS NULL)))))
        """,
        {
...
def test_whole_number_float_eq_emits_int_form_like_prefilter() -> None:
    ...
    assert_sql(
        ...
        GROUP BY (calls_merged.project_id, calls_merged.id)
        HAVING (
            ((toFloat64OrNull(coalesce(nullIf(JSON_VALUE(any(calls_merged.inputs_dump), {pb_3:String}), 'null'), '')) = {pb_4:Float64}))
            AND ((any(calls_merged.deleted_at) IS NULL))
            AND ((NOT ((any(calls_merged.op_name) IS NULL))))
        )
        """,
...
def test_bool_eq_emits_numeric_form_like_prefilter() -> None:
    ...
    assert_sql(
        ...
        GROUP BY (calls_merged.project_id, calls_merged.id)
        HAVING (
            ((multiIf(coalesce(nullIf(JSON_VALUE(any(calls_merged.inputs_dump), {pb_3:String}), 'null'), '') = 'true', 1, coalesce(nullIf(JSON_VALUE(any(calls_merged.inputs_dump), {pb_3:String}), 'null'), '') = 'false', 0, toUInt8OrNull(coalesce(nullIf(JSON_VALUE(any(calls_merged.inputs_dump), {pb_3:String}), 'null'), ''))) = {pb_4:Bool}))
            AND ((any(calls_merged.deleted_at) IS NULL))
            AND ((NOT ((any(calls_merged.op_name) IS NULL))))
        )
        """,
    ...
    if read_table == ReadTable.CALLS_MERGED:
        assert_sql(
            ...
            HAVING (
                ((any({expected_table}.deleted_at) IS NULL))
                AND
                ((NOT ((any({expected_table}.op_name) IS NULL))))
            )
            """,
            {
                "pb_0": '$."status_counts"."error"',
...
def test_query_with_feedback_filter_and_datetime_and_string_filter() -> None:
    ...
    assert_sql(
        ...
        filtered_calls AS
            (SELECT calls_merged.id AS id
            FROM calls_merged
                            LEFT JOIN (SELECT * FROM feedback WHERE feedback.project_id = {pb_2:String} ) AS feedback ON (feedback.weave_ref = concat('weave-trace-internal:///', {pb_2:String}, '/call/', calls_merged.id))
            PREWHERE calls_merged.project_id = {pb_2:String}
            WHERE (calls_merged.id IN filter_candidate_ids)
            ...
            GROUP BY (calls_merged.project_id,
                        calls_merged.id)
            HAVING (((coalesce(nullIf(JSON_VALUE(anyIf(feedback.payload_dump, feedback.feedback_type = {pb_3:String}), {pb_4:String}), 'null'), '') > coalesce(nullIf(JSON_VALUE(anyIf(feedback.payload_dump, feedback.feedback_type = {pb_3:String}), {pb_5:String}), 'null'), '')))
                AND ((any(calls_merged.started_at) > {pb_6:String}))
                AND ((coalesce(nullIf(JSON_VALUE(any(calls_merged.inputs_dump), {pb_7:String}), 'null'), '') = {pb_8:String}))
                AND ((any(calls_merged.deleted_at) IS NULL))
                AND ((NOT ((any(calls_merged.op_name) IS NULL))))))
        SELECT calls_merged.id AS id
        ...
        """,
        {
            ...
            "pb_4": '$."output"."expected"',
            "pb_5": '$."output"."found"',
            "pb_6": "2024-03-01 00:00:00.000000",
            "pb_7": '$."message"',
            "pb_8": "hello",
        },
    )
...
def test_trace_id_filter_in():
    ...
    assert_sql(
        ...
        PREWHERE calls_merged.project_id = {pb_1:String}
        WHERE (calls_merged.id IN filter_candidate_ids)
            AND (ifNull(calls_merged.trace_id, '') IN {pb_2:Array(String)}
...
def test_trace_id_filter_calls_complete_no_candidate_cte() -> None:
    ...
    cq = CallsQuery(project_id="project", read_table=ReadTable.CALLS_COMPLETE)
    cq.add_field("id")
    cq.hardcoded_filter = HardCodedFilter(filter={"trace_ids": ["111111111111"]})
    assert_sql(
        cq,
        """
        SELECT
...
def test_filter_candidate_ids_cte_matches_user_example() -> None:
    """A heavy LIKE filter on calls_merged emits a filter_candidate_ids CTE
    that uses the strict `ifNull(<dump>, '')` form (no OR-IS-NULL) so that
    ClickHouse can use the ngram bloom filter index. The CTE is a WHERE-only
    id superset (no ORDER BY/LIMIT); the outer filter_query keeps the
    OR-IS-NULL form for unmerged-call-part correctness and owns ORDER BY/LIMIT.
    """
    cq = CallsQuery(project_id="project")
    cq.add_field("id")
    ...
    cq.add_condition(
        tsi_query.EqOperation.model_validate(
            {
                "$eq": [
                    {"$getField": "inputs.record.unique_id"},
                    {"$literal": "847d9162-7249-47c7-ab36-2ba4e821d153"},
                ]
            }
        )
    )
    ...
    assert_sql(
        ...
        """,
        {
            "pb_0": '%"847d9162-7249-47c7-ab36-2ba4e821d153"%',
            "pb_1": "project",
            "pb_2": '$."record"."unique_id"',
            "pb_3": "847d9162-7249-47c7-ab36-2ba4e821d153",
        },
    )
...
def test_all_optimization_filters():
    ...
    assert_sql(
        ...
        PREWHERE calls_merged.project_id = {pb_1:String}
        WHERE (calls_merged.id IN filter_candidate_ids)
            AND (calls_merged.parent_id IN {pb_13:Array(String)}
                OR calls_merged.parent_id IS NULL)
            AND ((calls_merged.op_name IN {pb_7:Array(String)})
                OR (calls_merged.op_name IS NULL))
            AND (ifNull(calls_merged.trace_id, '') IN {pb_8:Array(String)}
                OR calls_merged.trace_id IS NULL)
            AND (calls_merged.thread_id IN {pb_9:Array(String)}
                OR calls_merged.thread_id IS NULL)
            AND (calls_merged.turn_id IN {pb_10:Array(String)}
                OR calls_merged.turn_id IS NULL)
            AND (((hasAny(calls_merged.input_refs, {pb_11:Array(String)})
                OR length(calls_merged.input_refs) = 0)
                AND (hasAny(calls_merged.output_refs, {pb_12:Array(String)})
                    OR length(calls_merged.output_refs) = 0)))
        GROUP BY (calls_merged.project_id, calls_merged.id)
        HAVING (((any(calls_merged.deleted_at) IS NULL))
            AND ((NOT ((any(calls_merged.op_name) IS NULL))))
            AND (((hasAny(array_concat_agg(calls_merged.input_refs), {pb_2:Array(String)}))
                AND (hasAny(array_concat_agg(calls_merged.output_refs), {pb_3:Array(String)}))
            AND (any(calls_merged.parent_id) IN {pb_4:Array(String)})
              AND (any(calls_merged.thread_id) IN {pb_5:Array(String)})
              AND (any(calls_merged.turn_id) IN {pb_6:Array(String)}))))
        """,
        {
            "pb_0": ["111111111111", "222222222222"],
            "pb_1": "project",
            ...
            "pb_5": ["thread_333", "thread_444"],
            "pb_6": ["turn_555", "turn_666"],
            "pb_7": [
                "weave-trace-internal:///222222222222",
                "weave-trace-internal:///111111111111",
            ],
            "pb_8": ["111111111111", "222222222222"],
            "pb_9": ["thread_333", "thread_444"],
            "pb_10": ["turn_555", "turn_666"],
            "pb_11": ["weave-trace-internal:///222222222222%"],
            "pb_12": ["weave-trace-internal:///111111111111%"],
            "pb_13": ["111111111111", "222222222222"],
        },
    )
...
def test_query_filter_with_escaped_dots_in_field_names() -> None:
    ...
    assert_sql(
        ...
        GROUP BY (calls_merged.project_id,
                calls_merged.id)
        HAVING (((toInt64OrNull(coalesce(nullIf(JSON_VALUE(any(calls_merged.output_dump), {pb_2:String}), 'null'), '')) = {pb_3:Int64}))
                AND ((any(calls_merged.deleted_at) IS NULL))
                AND ((NOT ((any(calls_merged.op_name) IS NULL)))))
        """,
        {
