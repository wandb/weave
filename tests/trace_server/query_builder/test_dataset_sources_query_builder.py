...
- ``deterministic_link_id`` is a stable UUIDv5 over the logical key
  ...
  same logical key -> same id; different keys -> different ids.
- The read queries (forward + reverse) dedup via ``GROUP BY`` + ``argMax``
  and NEVER use ``FINAL`` (see membership_pattern.md: FINAL is catastrophically
  slow at scale).
...
def test_deterministic_link_id_distinct_for_different_keys() -> None:
    ...
    variants = [
        ...
        {**base, "source_kind": "span"},
        {**base, "source_id": "call_456"},
    ]
