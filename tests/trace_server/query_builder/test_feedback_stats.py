...
from weave.trace_server.clickhouse_trace_server_batched import _parse_window_stat_col
...
class TestJsonPathHelpers:
    ...
    def test_extraction_numeric(self):
        ...
        assert "toFloat64OrNull" in sql
        assert "JSONExtractRaw" in sql
...
class TestTriggerRefWhereClause:
    def test_exact_match(self):
        ...
        assert sql.startswith("trigger_ref =")

    def test_prefix_match(self):
        pb = ParamBuilder()
        sql = trigger_ref_where_clause("my-monitor:*", pb)
        assert sql.startswith("startsWith(")
        # The prefix stored should be "my-monitor" (without ":*")
        params = pb.get_params()
        assert any("my-monitor" == v for v in params.values())
...
class TestBuildFeedbackStatsQuery:
    def test_returns_sql_with_all_buckets_cte(self):
        req = _make_req()
        pb = ParamBuilder()
        result = build_feedback_stats_query(req, pb)
        assert "all_buckets" in result.sql
        assert "aggregated_data" in result.sql
    ...
    def test_feedback_type_filter_added(self):
        req = _make_req(feedback_type="wandb.runnable.my-scorer")
        pb = ParamBuilder()
        result = build_feedback_stats_query(req, pb)
        assert "feedback_type" in result.sql

    def test_trigger_ref_exact_filter_added(self):
        req = _make_req(trigger_ref="weave:///entity/project/op/my-op:v1")
        pb = ParamBuilder()
        result = build_feedback_stats_query(req, pb)
        assert "trigger_ref" in result.sql

    def test_trigger_ref_prefix_filter_added(self):
        req = _make_req(trigger_ref="weave:///entity/project/op/my-op:*")
        pb = ParamBuilder()
        result = build_feedback_stats_query(req, pb)
        assert "startsWith" in result.sql
    ...
    def test_boolean_metric_uses_countif(self):
        req = _make_req(
            metrics=[
                FeedbackMetricSpec(
                    json_path="passed",
...
class TestBuildFeedbackStatsWindowQuery:
    ...
    def test_sql_has_no_group_by_bucket(self):
        req = _make_req()
        pb = ParamBuilder()
        result = build_feedback_stats_window_query(req, pb)
        assert result is not None
        # Window query must not bucket by time
        assert "all_buckets" not in result.sql
        assert "GROUP BY bucket" not in result.sql

    def test_same_filters_as_bucket_query(self):
