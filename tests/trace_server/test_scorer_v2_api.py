"""Tests for Scorer API endpoints.

Tests verify that the Scorer API correctly creates, reads, lists, and deletes scorer objects.
"""
...
"""
    ...
    read_res = trace_server.object_interface.scorer_read(read_req)
...
def test_scorer_read_not_found(trace_server):
    ...
    with pytest.raises(NotFoundError):
        trace_server.object_interface.scorer_read(read_req)
...
def test_scorer_list_basic(trace_server):
    ...
    for name in scorer_names:
        ...
        trace_server.object_interface.scorer_create(create_req)

    # List all scorers
    list_req = oi.ScorerListReq(project_id=project_id)
    scorers = list(trace_server.object_interface.scorer_list(list_req))
...
def test_scorer_list_with_limit(trace_server):
    ...
    # List with a limit
    list_req = oi.ScorerListReq(project_id=project_id, limit=3)
    scorers = list(trace_server.object_interface.scorer_list(list_req))
...
def test_scorer_list_with_offset(trace_server):
    ...
    # List with an offset
    list_req = oi.ScorerListReq(project_id=project_id, offset=2)
    scorers = list(trace_server.object_interface.scorer_list(list_req))
...
def test_scorer_list_with_limit_and_offset(trace_server):
    ...
    # First page
    list_req1 = oi.ScorerListReq(project_id=project_id, limit=3, offset=0)
    scorers1 = list(trace_server.object_interface.scorer_list(list_req1))
    assert len(scorers1) == 3

    # Second page
    list_req2 = oi.ScorerListReq(project_id=project_id, limit=3, offset=3)
    scorers2 = list(trace_server.object_interface.scorer_list(list_req2))
...
def test_scorer_delete_single_version(trace_server):
    ...
    delete_res = trace_server.object_interface.scorer_delete(delete_req)
...
def test_scorer_delete_not_found(trace_server):
    ...
    with pytest.raises(NotFoundError):
        trace_server.object_interface.scorer_delete(delete_req)
