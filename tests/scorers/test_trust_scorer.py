...
from weave.scorers.moderation_scorer import TOXICITY_TOTAL_THRESHOLD, TOXICITY_CATEGORY_THRESHOLD
...
class DummyScorer:
    def score(self, output: str, query: str):
        return WeaveScorerResult(passed=True, metadata={"dummy": True}, extras={"score": 1})
...
def test_filter_inputs_for_scorer(trust_scorer):
    dummy = DummyScorer()
    inputs = {"output": "test_output", "query": "test_query", "extra": "should_be_filtered"}
...
def test_score_with_logic(trust_scorer):
    ...
    class DummyCriticalScorer:
        def score(self, **kwargs):
            return WeaveScorerResult(passed=False, metadata={"score": 0.8}, extras={"score": 0.8})
    
    class DummyAdvisoryScorer:
        def score(self, **kwargs):
            return WeaveScorerResult(passed=False, metadata={"score": 0.4}, extras={"score": 0.4})
