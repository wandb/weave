...
class TestOTelSpanEmission:
    ...
    def test_session_agent_identity_propagates_to_turn_span(
        self, otel_spans: InMemorySpanExporter
    ) -> None:
        # agent_id/description/version set once on the Session land on every
    ...
    def test_turn_overrides_session_agent_identity(
        self, otel_spans: InMemorySpanExporter
    ) -> None:
        # A turn served by a different agent (multi-agent routing) or after a
        # version bump overrides the session default via turn.record(...).
        with Session(
            agent_name="bot",
            session_id="sess-ident-override",
            agent_id="agent-7",
            agent_version="v3",
        ) as s:
            with s.start_turn() as turn:
...
class TestLogSession:
    ...
    def test_agent_identity_defaults_applied(
        ...
        # Session-level identity defaults fill in turns that leave a field empty;
        # a turn's own value wins, per-field.
        log_session(
            session_id="sess-ident-defaults",
            agent_name="bot",
