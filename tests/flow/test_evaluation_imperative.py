...
def test_log_example_with_scorer_objects(client):
    """Test log_example with different scorer types (strings, dicts, Scorer objects)."""
    ...
    # Test with mixed scorer types
    # Note: log_example currently only supports string keys in the scores dict
    ev.log_example(
        inputs={"text": "hello"},
        output="world",
        scores={
    ...
    assert predict_and_score_call is not None
    assert predict_and_score_call.output["scores"]["string_scorer"] == 0.5
    assert predict_and_score_call.output["scores"]["another_scorer"] == 0.8
...
def test_log_example_with_none_values(client):
    """Test log_example with None as output or score values."""
    ev = EvaluationLogger()

    # Log example with None output and None score
    ev.log_example(
        inputs={"q": "unanswerable"},
        output=None,
        scores={"correctness": None, "quality": 0.5},
    )
    ...
    assert predict_and_score_call is not None
    assert predict_and_score_call.output["output"] is None
    assert predict_and_score_call.output["scores"]["correctness"] is None
    assert predict_and_score_call.output["scores"]["quality"] == 0.5


def test_log_example_with_complex_score_types(client):
    """Test log_example with complex score values (dicts with value/reason)."""
    ev = EvaluationLogger()

    # Log example with complex score structure
    ev.log_example(
        inputs={"text": "input"},
        output="output",
        scores={
            "simple": 0.8,
            "with_reason": {"value": 0.9, "reason": "Well formatted"},
            "bool_score": True,
        },
    )
    ...
    assert predict_and_score_call is not None
    assert predict_and_score_call.output["scores"]["simple"] == 0.8
    assert predict_and_score_call.output["scores"]["with_reason"] == {
        "value": 0.9,
        "reason": "Well formatted",
    }
    assert predict_and_score_call.output["scores"]["bool_score"] is True


def test_log_example_returns_score_logger(client):
    """Test that log_example returns a ScoreLogger instance."""
    ev = EvaluationLogger()

    # log_example should return a ScoreLogger
    result = ev.log_example(
        inputs={"q": "test"},
        output="answer",
        scores={"score": 1.0},
    )

    # Verify it's a ScoreLogger (has the expected attributes)
    assert hasattr(result, "predict_call")
    assert hasattr(result, "predict_and_score_call")
    assert hasattr(result, "log_score")
