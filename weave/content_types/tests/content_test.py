...
def main_content_test():
    try:
        ...
        print(
            f"--- Testing with MIME Detector: {type(_current_mime_detector).__name__} ---"
        )
    except ImportError:
        print(
            "--- Testing (MIME detector type not printed due to potential import context issues when run standalone) ---"
        )
    ...
    try:
        ...
        print(
            f"Stored as UTF-8 (BOM stripped), re-decoded: '{redecoded_content_bom}' - Content MATCHES."
        )
        ...
        pt_utf8_bom_obj.export_metadata(
            os.path.join(temp_dir, "utf8_bom_doc_meta.json")
        )

    except Exception as e:
        print(f"PlainText (UTF-8 BOM) Error: {e}")
    ...
    try:
        ...
        print(f"PlainText (UTF-8) Error: {e}")

    print("\n--- Testing Json from_data (string input) ---")
    json_string_data = '{"status": "ok", "message": "你好世界"}'  # Contains non-ASCII
    try:
        json_from_str_obj = Json.from_data(
            json_string_data, mime_type="application/json"
        )
        ...
        print("String input content matches stored UTF-8 bytes.")
        assert json_from_str_obj.get_content_as_string() == json_string_data
        json_from_str_obj.export_metadata(
            os.path.join(temp_dir, "json_from_string_meta.json")
        )
    except Exception as e:
        print(f"Json (from_data string) Error: {e}")
...
def main_media_test():
    ...
    test_cases = [
        ...
        (
            Python,
            "script.py",
            ["text/x-python", "application/x-python-code", "text/python"],
        ),
        (JavaScript, "app.js", ["application/javascript", "text/javascript"]),
        (
            TypeScript,
            "module.ts",
            ["text/x-typescript", "application/typescript", "text/typescript"],
        ),  # Our added types should take precedence
    ...
    for media_cls, filename, expected_mime in test_cases:
        ...
        try:
            ...
            if isinstance(expected_mime, list):
                assert (
                    media_obj.mime_type in expected_mime
                ), f"Expected MIME in {expected_mime}, got {media_obj.mime_type}"
            else:
                assert (
                    media_obj.mime_type == expected_mime
                ), f"Expected MIME {expected_mime}, got {media_obj.mime_type}"

            if hasattr(media_obj, "get_content_as_string") and media_cls not in [
                Audio,
                Video,
                Image,
                Pdf,
            ...
            exported_path = media_obj.export(
                os.path.join(temp_dir, f"exported_{filename.split('.')[0]}")
            )
    ...
    try:
        ...
        print(
            f"Incorrectly loaded unhandled type as PlainText: {txt_obj}"
...
if __name__ == "__main__":
    ...
    from weave.content_types.mime_types import (
        _current_mime_detector,  # Relative import for when running as a module
    )
