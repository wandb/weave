...
import mimetypes
from typing import Dict, List, Literal, Protocol, TypeVar, get_args
...
try:
    ...
    try:
        ...
        print(
            f"Warning: python-magic is installed, but libmagic seems to be missing or broken: {e}. Falling back to standard mimetypes."
        )
        MAGIC_AVAILABLE = False
except ImportError:
    MAGIC_AVAILABLE = False
    print(
        "Info: python-magic not found. Falling back to standard mimetypes library for MIME detection."
    )
...
def _initialize_mimetypes():
    ...
    mimetypes.add_type(
        "text/yaml", ".yml", strict=True
    )  # Ensure .yml is also text/yaml
...
def get_mime_detector_instance() -> MimeDetector:
    ...
    if MAGIC_AVAILABLE:
        try:
            ...
            print(
                f"Warning: Failed to instantiate PythonMagicMimeDetector ({e}). Falling back to standard mimetypes."
            )
