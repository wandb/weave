...
__all__ = ["V2CallEndParams", "End", "EndSummary", "EndSummaryUsage"]


class V2CallEndParams(TypedDict, total=False):
    entity: Required[str]

    end: Required[End]
    """Deprecated alias.

    `started_at` now lives on the parent `EndedCallSchemaForInsert`; prefer that.
    Kept so external SDK pins on the `WithStartedAt` name keep importing. Remove
    once all in-tree callers migrate.
    """
...
class End(TypedDict, total=False):
    ...
    `started_at` now lives on the parent
