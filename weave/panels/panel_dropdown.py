...
class Dropdown(panel.Panel):
    id = "Dropdown"
    config: typing.Optional[DropdownConfig] = dataclasses.field(default_factory=DropdownConfig)
    ...
    def __init__(self, input_node=graph.VoidNode(), vars=None, config=None, **options) -> None:
        ...
        if isinstance(self.input_node, weave.graph.VoidNode):
            ...
            self.input_node = weave_internal.const([], weave.types.List(choices_type.object_type))
