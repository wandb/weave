...
def custom_dd_patch():
    ...
    def execute(self, document, *args, **kwargs):
        with tracer.trace(
            ...
            engine_trace.set_pii_tag(
                span, "document", wandb_graphql.print_ast(document), ""
            )
            engine_trace.set_pii_tag(
                span, "variable_values", kwargs.get("variable_values", {}), ""
            )
...
def execute():
    ...
    if root_span is not None:
        engine_trace.set_pii_tag(
            root_span, "request_size", len(req_bytes), len(req_bytes)
        )
    ...
    if root_span is not None:
        engine_trace.set_pii_metric(
            root_span,
            "node_count",
            len(response_payload["data"]),
            len(response_payload["data"]),
        )
        engine_trace.set_pii_metric(
            root_span,
