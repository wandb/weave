...
# that someone doesn't introduce auto-importing loading weave.old_weave.ops or weave.old_weave.panels
...
# introduce loading weave.old_weave.ops or weave.old_weave.panels when importing weave.
newly_added_modules = set(sys.modules.keys()) - pre_init_modules
ops_modules = []
panels_modules = []
for module_name in newly_added_modules:
    if module_name.startswith("weave.old_weave.ops"):
...
if ops_modules or panels_modules:
    ...
    raise errors.WeaveInternalError(
        "importing weave should not import weave.old_weave.ops or weave.old_weave.panels, but the following modules were imported: "
