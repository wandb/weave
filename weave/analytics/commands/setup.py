...
def setup(
    ...
    if current_wandb_key:
        console.print(f"[green]✓[/green] WANDB_API_KEY: [dim]{'*' * 8}...{current_wandb_key[-4:]}[/dim]")
    else:
        console.print("[yellow]⚠[/yellow] WANDB_API_KEY not found")
        wandb_api_key = click.prompt(
            "  Enter your W&B API key (from https://wandb.ai/authorize)",
            hide_input=True,
        )
        config["WANDB_API_KEY"] = wandb_api_key
        console.print("[green]✓[/green] WANDB_API_KEY saved")
    ...
    if current_llm_key:
        console.print(f"[green]✓[/green] {env_var}: [dim]{'*' * 8}...{current_llm_key[-4:]}[/dim]")
    else:
        console.print(f"[yellow]⚠[/yellow] {env_var} not found")
    ...
    if configure_debug:
        ...
        console.print("[green]✓[/green] Debug configuration saved")
...
[bold]Settings:[/bold]
  ...
  • LLM Provider: [cyan]{detected_provider}[/cyan]"""
    ...
    if config.get("DEBUG_WEAVE_ENTITY"):
        config_info += f"""
  • Debug Entity: [cyan]{config.get('DEBUG_WEAVE_ENTITY')}[/cyan]
  • Debug Project: [cyan]{config.get('DEBUG_WEAVE_PROJECT')}[/cyan]"""

    console.print(Panel(config_info, title="[green]✓ Setup Complete[/green]", border_style="green"))
