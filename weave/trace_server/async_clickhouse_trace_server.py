...
Two mechanisms live here:
  ...
  - `_run_on_ch_executor` hands a *blocking* driver call to a thread pool. The
    ...
    whole round-trip.
  - `ClickHouseTraceServer._query_async` and friends go through
    `AsyncClickHouseTransport`, which speaks HTTP over aiohttp. No pool slot is
    held while the server works, so concurrency stops being bounded by pool width.
"""
