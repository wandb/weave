...
class SqliteTraceServer(tsi.FullTraceServerInterface):
    ...
    def setup_tables(self) -> None:
        ...
        cursor.execute(
            ...
            ON dataset_sources (
                project_id, dataset_object_id, row_digest, source_kind,
    ...
    #
    # CONVERSATIONS ARE ASSERTED (D2/D3/D4): a conversation is a first-class
    # source kind with no backing table to validate against, so conversation
    # links skip existence validation and store no cached source fields (display
    # name '', started_at = now). Write-layer invariants (D4): conversation links
    # MUST carry source_trace_id='' (a conversation spans traces; conversation_id
    # is self-sufficient identity), and call links MUST carry a non-empty
    # source_trace_id (their identity is (trace_id, id); '' would collapse
    # distinct edges under the logical key).
    def dataset_sources_link(
        ...
        # D4 write-layer invariants: enforce the source_trace_id contract per
        # kind before any write so a bad link can never collide across kinds.
        for _rd, source, _meta in flat:
            ...
            elif source.source_trace_id == "":
                raise InvalidRequest(
                    "dataset_sources_link: "
                    f"source_kind={source.source_kind.value!r} links require a "
                    "non-empty source_trace_id (identity is (trace_id, id))."
                )

        conn, cursor = get_conn_cursor(self.db_path)
        with self.lock:
            # Validate CALL existence + fetch cached display fields. Conversation
            # links are asserted (no backing table) and skip validation.
            call_ids = sorted(
                {
                    source.source_id
                    for _rd, source, _meta in flat
            ...
            for row_digest, source, meta in flat:
                ...
                if source.source_kind == tsi.SourceKind.CONVERSATION:
                    # Asserted link: no cached source fields. started_at must be a
                    # valid timestamp (read hydration parses it), so use now.
                    display_name, started_at = "", now_iso
                else:
