...
class SqliteTraceServer(tsi.TraceServerInterface):
    ...
    def op_read(self, req: tsi.OpReadReq) -> tsi.OpReadRes:
        """Read an op object by delegating to obj_read with op filtering."""
        ...
        return tsi.OpReadRes(op_obj=result.obj)

    def ops_query(self, req: tsi.OpQueryReq) -> tsi.OpQueryRes:
        """Query op objects by delegating to objs_query with op filtering."""
        obj_query_req = tsi.ObjQueryReq(
            project_id=req.project_id,
            filter=tsi.ObjectVersionFilter(
                is_op=True,
                object_ids=req.filter.object_ids if req.filter else None,
                base_object_classes=["Op"] if req.filter else None,
            ),
            limit=req.limit,
            offset=req.offset,
            metadata_only=req.metadata_only,
        ...
        return tsi.OpQueryRes(op_objs=result.objs)
    ...
    def actions_execute_batch(
        ...
        """Execute actions batch - SQLite doesn't support remote actions execution."""
    ...
    def evaluation_log_start(
        ...
        call_id = generate_id()
        return tsi.EvaluationLogStartRes(evaluate_call_id=call_id)
    ...
    def evaluation_log_prediction(
        ...
        call_id = generate_id()
        return tsi.EvaluationLogPredictionRes(predict_call_id=call_id)
