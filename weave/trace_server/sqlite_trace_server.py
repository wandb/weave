    ...
    def _eval_results_grouped_rows(
        ...
        for pas_call in predict_and_score_calls:
            ...
            if eval_call_id is None:
                continue
            example = pas_call.inputs.get("example") if isinstance(pas_call.inputs, dict) else None
            ...
            predict_call = self._select_predict_call(trial_children, pas_call.inputs.get("model"))
        ...
        if req.include_raw_data_rows and req.resolve_row_refs:
            ...
            # full object → TableRef → row traversal.  Refs stored in the DB
            # use the weave-trace-internal:/// scheme.
            ...
            if dataset_ref_by_digest:
                try:
                    ...
                    for row_digest, val in zip(
                        dataset_ref_by_digest.keys(), refs_res.vals
                    ):
