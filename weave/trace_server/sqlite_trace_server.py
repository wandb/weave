...
class SqliteTraceServer(tsi.TraceServerInterface):
    ...
    def calls_query(self, req: tsi.CallsQueryReq) -> tsi.CallsQueryRes:
        ...
        select_columns = [
            ...
            if key not in ["storage_size_bytes", "total_size_bytes"]
        ]
        if req.columns:
        ...
        join_clause = ""
        if req.include_rolled_up_storage_size:
            select_columns.append(
                """
                CASE
                    WHEN calls.parent_id IS NULL THEN trace_stats.total_size_bytes
                    ELSE NULL
                END
            """
            )
            select_columns_names.append("total_size_bytes")

            join_clause += """
                LEFT JOIN (SELECT
                    calls.trace_id as tid,
                    sum(COALESCE(length(attributes),0) + COALESCE(length(inputs),0) + COALESCE(length(output),0) + COALESCE(length(summary),0)) as total_size_bytes
                FROM calls
