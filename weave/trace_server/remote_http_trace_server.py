...
class RemoteHTTPTraceServer(tsi.TraceServerInterface):
    ...
    def __init__(self, trace_server_url:str, should_batch:bool=True):
        ...
        if self.should_batch:
            self.call_buffer = InMemAutoFlushingBuffer(MAX_FLUSH_COUNT, MAX_FLUSH_AGE, self._flush_calls)
    ...
    def _flush_calls(self, to_flush):
        ...
        r = requests.post(self.trace_server_url + "/call/upsert_batch", data=json.dumps({"batch": to_flush}))
        r.raise_for_status()
