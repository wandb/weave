...
def make_queues_query(
    ...
    *,
    ...
    if name is not None:
        ...
        where_clauses.append(f"lowerUTF8(name) LIKE lowerUTF8({{{name_param}: String}})")
...
def make_queue_create_query(
    ...
    Args:
        ...
        claim_timeout_seconds: Claim expiration timeout
        due_date: Optional deadline for completing annotations
    ...
    claim_timeout_param = pb.add_param(claim_timeout_seconds)
    due_date_param = pb.add_param(due_date)
    ...
    INSERT INTO annotation_queues (
        ...
        {{{claim_timeout_param}: UInt32}},
        {{{due_date_param}: Nullable(DateTime64(3))}},
...
def make_queues_stats_query(
    ...
    - completed_items: Count of items with 'completed' or 'skipped' status in annotator_queue_items_progress per queue
    ...
    # The annotator_queue_items_progress table has one row per queue_item_id when an annotator
    # completes or skips an item (in shared pool mode)
    query = f"""
