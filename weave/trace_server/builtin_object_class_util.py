...
def get_base_object_class_name(val: Any) -> Optional[str]:
...
def process_incoming_object(
    ...
    2. There are 2 ways to specify the base object class:
        a. The `req_builtin_object_class` argument.
    ...
    4. if the object contains a base object class inside the payload, then we simply validate
        the object against the base object class (if a match is found in BUILTIN_OBJECT_CLASS_REGISTRY)
    5. If the object does not have a base object class and a requested base object class is
        provided, we require a match in BUILTIN_OBJECT_CLASS_REGISTRY and validate the object against
        the requested base object class. Finally, we set the correct feilds.
    """
    if not isinstance(val, dict):
        if req_builtin_object_class is not None:
            raise ValueError(
                "set_builtin_object_class cannot be provided for non-dict objects"
            )
        return val, None

    dict_val = val.copy()
    val_class_name = get_object_class_name(dict_val)

    if (
        val_class_name != None
        and req_builtin_object_class != None
        and val_class_name != req_builtin_object_class
    ):
        raise ValueError(
            f"set_builtin_object_class must match _class_name: {req_builtin_object_class} != {val_class_name}"
        )

    if val_class_name is not None:
...
def dump_object_with_correct_hierarchy(val: BaseModel) -> dict:
    cls = val.__class__
...
def _general_dump(val: Any) -> Any:
    if isinstance(val, BaseModel):
        return dump_object_with_correct_hierarchy(val)
