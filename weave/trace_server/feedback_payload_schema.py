...
def _infer_value_type(
    types_seen: set[type],
) -> Literal["boolean", "numeric", "categorical"]:
...
def build_feedback_payload_sample_query(
    ...
    """Build parameterized ClickHouse SQL to fetch sample payload_dump rows.

    Schema discovery works by fetching a representative sample of payload rows
    and inspecting their structure in Python. Rather than scanning every row,
    this query groups by trigger_ref and takes the most-recent payload per ref
    (via argMax), so each distinct monitor/source contributes exactly one
    payload to the sample. The number of sampled rows is capped by
    ``req.sample_limit`` (default 2000, max 5000), which is enough to discover
    the full schema for virtually all real-world projects while keeping the
    query fast.

    Uses the same filters as feedback_stats (project_id, created_at range,
    feedback_type, trigger_ref).

    Args:
        req: Schema discovery request with filters and sample limit.
        pb: Parameter builder for safe SQL parameterization.

    Returns:
        A ``(sql, params)`` tuple ready to pass to ClickHouse.
    """
