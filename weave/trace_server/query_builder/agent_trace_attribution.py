...
def attributed_spans_source(
    ...
    *,
    ...
    `scope_fallback_to_base`, when true, restricts the fallback rollup to the
    `trace_id`s of `base_relation` (`trace_id IN (SELECT trace_id FROM
    base_relation)`). Since the fallback is consumed only via the `trace_id`
    join, scoping it to the join's trace_ids removes only rows the join would
    discard, so attributed values are unchanged. Used by the page-prefetch
    two-pass list read to scope the rollup to the page's traces.

    `base_relation` is interpolated as raw SQL and must be a trusted, internal
    fragment (a relation name / a literal subquery), never user input;
    user-derived values still flow through `pb` params.
    ...
    if scope_fallback_to_base:
        fallback_conds.append(f"trace_id IN (SELECT trace_id FROM {base_relation})")
