...
class FileStorageClient(ABC):
    ...
    def store(self, uri: FileStorageURI, data: bytes) -> None:
        """Store data at the specified URI."""
        pass
...
def get_storage_client(uri: FileStorageURI) -> FileStorageClient:
    """Factory method to create appropriate storage client based on URI type."""
...
def store_in_bucket(file_storage_uri: FileStorageURI, data: bytes) -> None:
    ...
    try:
        client = get_storage_client(file_storage_uri)
