...
from weave.trace_server.base64_content_conversion import ContentWriter
...
# Signature of the server's `_query_async`; takes (sql, params), returns QueryResult.
QueryParams: TypeAlias = dict[str, Any]
ClickHouseRow: TypeAlias = dict[str, Any]
QueryFn = Callable[[str, QueryParams], Awaitable["QueryResult"]]

#: Signature of the server's feedback query, awaited.
FeedbackQueryFn = Callable[[FeedbackQueryReq], Awaitable[FeedbackQueryRes]]
...
class AgentQueryHandler:
    ...
    Takes a `query_fn` (typically the server's `_query_async` method) so queries
    ...
    async def spans_query(self, req: AgentSpansQueryReq) -> AgentSpansQueryRes:
        ...
        """
        total, rows = await self._run_paginated(
        ...
        if req.group_distributions:
            await self._hydrate_group_distributions(req, groups, aliases[0])
        if _is_conversation_grouping(req.group_by):
            await self._hydrate_conversation_previews(req, groups, aliases[0])
        return AgentSpansQueryRes(groups=groups, total_count=total)

    async def _hydrate_conversation_previews(
        self,
        ...
        try:
            rows = _rows_as_dicts(await self._query(sql, pb.get_params()))
        except Exception:
    ...
    async def conversation_spans(
        ...
        feedback_by_conv = await self._fetch_conversation_spans_feedback(
    ...
    async def _fetch_conversation_spans_feedback(
        self,
        project_id: str,
        ...
        try:
            grouped = await self._fetch_agent_feedback(
    ...
    async def spans_stats(self, req: AgentSpanStatsReq) -> AgentSpanStatsRes:
        """Return chart-ready aggregations over spans."""
        pb = ParamBuilder(PARAM_NAMESPACE)
        query = build_agent_span_stats_query(req, pb)
        result = await self._query(query.sql, query.parameters)
    ...
    async def _hydrate_group_distributions(
        self,
        req: AgentSpansQueryReq,
        groups: list[AgentSpanGroupRow],
        group_alias: str,
        ...
        total_counts = {
            ...
            for row in _rows_as_dicts(await self._query(counts_sql, pb.get_params()))
        }
        ...
        async def numeric_rows() -> list[ClickHouseRow]:
        ...
        async def categorical_rows() -> list[ClickHouseRow]:
    ...
    async def agents_query(self, req: AgentsQueryReq) -> AgentsQueryRes:
        ...
        if req.include_costs and agents:
            cost_map = await self._grouped_cost_map(
    ...
    async def _grouped_cost_map(
        ...
        *,
        ...
        res = await self.spans_query(cost_req)
        out: dict[str, float] = {}
    ...
    async def search_messages(self, req: AgentSearchReq) -> AgentSearchRes:
        """Full-text search across message content."""
        rows = await self._run_message_search_query(req)
    ...
    async def conversation_chat(
        ...
        total_turns, rows = await self._run_paginated(
            make_conversation_chat_turns_count_query,
            make_conversation_chat_spans_query,
            req,
        )
    ...
    async def _fetch_agent_feedback(
        self,
        project_id: str,
        ...
        feedback = await self._feedback_query(req)
    ...
    async def _run_trace_detail_query(
        self, project_id: str, trace_id: str
    ) -> list[ClickHouseRow]:
...
class AgentWriteHandler:
    ...
    _trace_server: ContentWriter | None = None
