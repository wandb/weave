...
class Attribute:
    ...
    standardised on. ``additional_aliases`` is for legacy / historical /
    parallel forms that the server still needs to recognise so old data
    and clients-in-the-field keep parsing. The catalog is a recognizer
    registry; the OTel spec is the source of truth, not this file.
    """
    ...
    additional_aliases: tuple[str, ...] = ()  # legacy / pre-standardisation forms
