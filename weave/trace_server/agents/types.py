"""Pydantic request/response types for the Weave Agents observability API.

These types are the shared interface between the FastAPI handlers in
`services/weave-trace`, the ClickHouse handler in
`weave/trace_server/clickhouse_trace_server_batched.py`, the in-memory fake,
and every SDK binding. The same shape goes in on every backend, the same
shape comes out.

Domain model:

1. **Agent** — identified by `agent_name`. The container at the top of the
   Agents view.
2. **Agent version** — identified by `(agent_name, agent_version)`. Lets a
   user diff behavior across releases of the same agent.
3. **Span** — one OTel GenAI span: agent invocation, LLM call, tool
   execution, compaction, or handoff. The atomic unit of recorded
   activity.
4. **Turn** — one trace's span tree rendered as a linear chat timeline.
   Roughly "one user message and the agent's complete response."
5. **Conversation** — turns sharing a `conversation_id`, presented as
   ordered multi-turn chat.

Request/response pairs follow a `*Req` → `*Res` naming convention:

* `agent_agents_query` — `AgentsQueryReq` → `AgentsQueryRes`
* `agent_versions_query` — `AgentVersionsQueryReq` → `AgentVersionsQueryRes`
* `agent_spans_query` — `AgentSpansQueryReq` → `AgentSpansQueryRes`
  (list mode and group mode share one endpoint)
...
* `genai_otel_export` — `GenAIOTelExportReq` → `GenAIOTelExportRes`

See https://docs.wandb.ai/weave/guides/tracking/trace-agents for the
...
class AgentSpanValueRef(BaseModel):
    """Pointer to one value pulled off a span row.

    The primitive that every higher-level spec (`AgentSpanMeasureSpec`,
    `AgentSpanStatsMetricSpec`, `AgentSpanGroupDistributionSpec`,
    `AgentSpanStatsNumericBucketSpec`) uses to name "the thing to read."
    The `source` chooses where the value lives and `key` chooses which
    one:

    1. `field` — a semconv field (`agent.name`) or direct column
       (`agent_name`); resolved through the server-side allowlist.
    2. `derived` — a query-time metric from
       `AGENT_SPAN_STATS_DERIVED_VALUE_TYPES` (for example,
    ...
    3. `custom_attrs_string` / `custom_attrs_int` /
       ...
       inside the matching typed custom attribute Map column.
    ...
    Raises:
        ValueError: When `source="derived"` but `key` isn't a recognized
            derived metric.
    """
...
class AgentSpanMeasureSpec(BaseModel):
    """Aggregate roll-up of one span value into a single number per group.

    Pair an `AgentSpanValueRef` with an aggregation function. Used by
    `AgentSpansQueryReq.measures` (per-group measures returned in
    `AgentSpanGroupRow.metrics`) and by `AgentSpanGroupFilter.measure`
    (range-filtering groups on an aggregated value).

    The aggregation must be legal for `value_type` — the allowed set is
    in `_ALLOWED_AGGS_BY_TYPE`. `count` is the only aggregation that may
    omit `value`.
    ...
    Attributes:
        ...
        aggregation: Which roll-up to apply.
        value: Value to aggregate. Required for every aggregation
            except `count`.
        value_type: Declared type of `value`. Used to validate the
            aggregation choice. Optional when the aggregation is
            `count` or when `value.source="derived"` (the derived metric
            already pins the type).
        filter: Optional Mongo-style `Query` that further restricts
            which spans contribute to this measure. Applied on top of
    ...
    Raises:
        ValueError: When `value` is missing for a non-`count`
            ...
            metric's known type, or when the aggregation isn't legal
            for `value_type`.
    """
...
class AgentSpanStatsMetricSpec(BaseModel):
    """Series specification for the `agent_spans_stats` chart endpoint.
    ...
    Note:
        ...
        validator picks a sensible default: `sum` for `number`,
        `count` for everything else. Don't rely on this in new code —
        pass at least one aggregation explicitly.
...
class AgentSpanStatsColumn(BaseModel):
    """Schema entry telling the client how to read one cell in a stats row.

    `AgentSpanStatsRes.rows` is a list of untyped `dict` rows so the wire
    format stays compact. This column list, returned alongside, is what
    pins down the type and meaning of each key in those dicts.
    ...
    Attributes:
        ...
        role: Provenance of the column. `time` for the time bucket
            ...
            for a group-by ref, `metric` for one aggregation/percentile
            of a metric.
...
class AgentSpanGroupFilter(BaseModel):
    """Keep only span groups whose aggregated measure falls in a range.

    Example: "agents whose error count is between 5 and 100" or
    "conversations whose first span happened after yesterday." Used on
    `AgentSpansQueryReq.group_filters` (post-grouping filter) and
    `AgentSpanStatsReq.group_filters` (post-bucket filter).

    At least one of `min` or `max` must be set. When both are set, they
    must share a type and `max` must be greater than or equal to `min`.
...
class AgentSpanStatsTimeBucketSpec(BaseModel):
    """Time-series bucketing for the `agent_spans_stats` endpoint.

    Default branch of the `AgentSpanStatsBucketSpec` discriminated union.
    Bucket width comes from `AgentSpanStatsReq.granularity` (seconds);
    when granularity is `None`, the server picks a sensible width based
    on the date range. Use `AgentSpanStatsNumericBucketSpec` for
...
class AgentSpanStatsNumericBucketSpec(BaseModel):
    """Histogram bucketing for the `agent_spans_stats` endpoint.

    Numeric branch of `AgentSpanStatsBucketSpec`. Two modes:

    1. **Per-span histogram** — set `value`, leave `group_by`/`measure`
       empty. Each span's `value` is placed in one bucket.
    2. **Grouped-measure histogram** — set `group_by` and `measure`,
       leave `value` empty. Each *group's* aggregated measure is placed
       in one bucket.

    The two modes are mutually exclusive. `bins` divides `[min, max]`
    into equal-width buckets; `min`/`max` default to the data range
    when omitted.

    Attributes:
        alias: Output column name for the bucket boundary.
        bins: Bucket count, 1 to 200. Defaults to 24.
        min: Lower bound of the histogram. `None` to use the data
            minimum.
        max: Upper bound of the histogram. `None` to use the data
            maximum.
    ...
    Raises:
        ValueError: When the two modes overlap or both are empty,
            ...
            non-numeric value/measure is referenced, or when
            `max < min`.
    """
...
class AgentSpanStatsReq(BaseModel):
    """Chart-ready aggregation request for the `agent_spans_stats` endpoint.

    The pipeline runs in this order: filter spans (`query` +
    `started_at` window) → bucket (`bucket_by`, default time buckets at
    `granularity` seconds) → group within bucket (`group_by`) →
    compute metrics → drop groups failing `group_filters`.

    Pair with `AgentSpanStatsRes`.

    Note:
        Grouped requests cap the window at `MAX_AGENT_STATS_RANGE_DAYS`.
        Unfiltered, ungrouped requests are exempt — they're used for
        cheap top-line counts.

    Attributes:
        project_id: Target project in `entity/project` form.
        query: Optional Mongo-style filter. Field names resolve through
            the GenAI semantic conventions, direct span columns, or
            typed custom attribute keys.
        ...
        group_limit: Maximum groups returned per bucket, 1 to
            `MAX_AGENT_STATS_GROUP_LIMIT`.
        bucket_by: How to bucket. `None` is equivalent to
            `AgentSpanStatsTimeBucketSpec()`.
        group_filters: Range filters over aggregated measures. Only
            valid for time buckets and grouped numeric buckets.

    Raises:
        ValueError: When `metrics` are missing without a numeric bucket
            spec, when metric aliases collide, when `end < start`, when
            a grouped request exceeds `MAX_AGENT_STATS_RANGE_DAYS`, or
            when `group_by`, `metrics`, and `bucket_by` combine
            illegally.
    """
...
class AgentSpanStatsRes(BaseModel):
    """Chart-ready response for `agent_spans_stats`.

    Read `columns` first to get the schema, then index into each `rows`
    entry by `AgentSpanStatsColumn.name`. The `start`, `end`,
    `granularity`, and `timezone` fields echo what the server actually
    applied so clients can render axes without re-deriving them. Empty
    buckets are omitted.
...
class AgentSpanSchema(BaseModel):
    """Normalized agent span row, the output unit of `agent_spans_query`.

    One OTel GenAI span, flattened into Weave's column layout. Same shape
    regardless of how the span was produced: `weave.start_session()` /
    `start_turn()` / `start_llm()` / `start_tool()`, a raw OTel exporter,
    ...
    Note:
        ...
        matched," so the UI can distinguish "unpriced" from "free."
    ...
    Attributes:
        ...
        span_name: OTel span name (e.g. `chat gpt-4o`,
            `execute_tool search`).
        span_kind: OTel span kind: `Server`, `Client`, `Internal`, etc.
        ...
        operation_name: GenAI `gen_ai.operation.name`
            (`invoke_agent`, `chat`, `execute_tool`, etc.).
        provider_name: GenAI `gen_ai.provider.name`
        ...
        agent_version: Agent version string for cross-release grouping.
        ...
        reasoning_tokens: Reasoning tokens (reasoning-capable models).
        cache_creation_input_tokens: Tokens written to a provider
            prompt cache.
        cache_read_input_tokens: Tokens read from a provider prompt
            cache.
        input_cost_usd: Query-time input-token cost. Opt-in via
            `include_costs`. See module note on `None` semantics.
        output_cost_usd: Query-time output-token cost. Opt-in.
        cache_read_cost_usd: Query-time cost of cached prompt reads.
            Opt-in.
        cache_creation_cost_usd: Query-time cost of cache writes.
            Opt-in.
        total_cost_usd: Sum of the per-direction cost fields above.
            Opt-in.
        reasoning_content: Reasoning text captured from a reasoning
            model.
        conversation_id: GenAI `gen_ai.conversation.id`. The key that
            joins spans into multi-turn conversations.
        ...
        tool_call_id: Tool-use call ID, links an LLM's tool call to
            its `execute_tool` span.
        tool_description: Tool description at call time.
        ...
        error_type: Exception type or provider error code for failed
            spans.
        request_temperature: Sampling temperature.
        ...
        raw_span_dump: Full OTel JSON payload. Opt-in via
            `include_details`.
    """
...
class AgentSortBy(BaseModel):
    ...
    Passed as an ordered list to `AgentsQueryReq.sort_by`,
    `AgentVersionsQueryReq.sort_by`, and `AgentSpansQueryReq.sort_by`.
    Steps are applied left to right; allowed `field` values are validated
    per endpoint server-side.

    Attributes:
        field: Sortable field name.
        direction: `desc` (default) sorts newest/largest first; `asc`
            sorts oldest/smallest first.
    """
...
class AgentGroupByRef(BaseModel):
    ...
    Used on `AgentSpansQueryReq.group_by`, `AgentSpanStatsReq.group_by`,
    ...
    1. **Field / column** (`field`, `column`) — an allowlisted semconv
       field (`agent.name`) or direct span column (`agent_name`).
       `column` is the legacy alias and behaves the same as `field`.
    2. **Custom attribute** (`custom_attrs_string`, `custom_attrs_int`,
       `custom_attrs_float`, `custom_attrs_bool`) — a user-defined key
...
def group_by_ref_alias(ref: AgentGroupByRef) -> str:
    """Resolve the output alias the server will use for a group-by ref.

    Single source of truth for the alias resolution rules. Both request
    validation and SQL projection call this so the validator's reserved
    name check sees the same key that ends up in
    `AgentSpanGroupRow.group_keys`.
    ...
    Returns:
        Explicit `ref.alias` when set; the semconv-resolved column name
        for `field`/`column` sources; the raw `ref.key` otherwise.
    """
...
class AgentSpanGroupDistributionSpec(BaseModel):
    """Custom attribute distribution to attach to each returned span group.

    Pass on `AgentSpansQueryReq.group_distributions`. For each group in
    the response, the server adds an `AgentSpanGroupDistributionItem`
    under `AgentSpanGroupRow.distributions[alias]` summarizing the
    ...
    Raises:
        ValueError: When `value.source` isn't a custom attribute
            source.
    """
...
class AgentSpanGroupDistributionBin(BaseModel):
    ...
    Attributes:
        index: 0-based bucket position. Clients render bins in `index`
            order, so this doubles as a stable sort key.
        min: Lower bound, inclusive.
        max: Upper bound, exclusive (inclusive on the final bin).
...
class AgentSpanGroupDistributionItem(BaseModel):
    """One distribution result inside an `AgentSpanGroupRow`.

    Returned in `AgentSpanGroupRow.distributions` keyed by the
    requesting `AgentSpanGroupDistributionSpec.alias`. Numeric attribute
    requests populate `bins`; categorical attribute requests populate
    `values`. The four `*_count` fields always sum to `total_count`.

    Attributes:
        alias: Echoes the requesting spec's `alias`.
        source: The custom attribute source that produced the
            distribution.
        ...
        other_count: Spans whose values fell outside `bins` or below
            the `top_n` cutoff.
        bins: Histogram bins. Populated for numeric distributions only.
...
class AgentConversationMessagePreview(BaseModel):
    """Length-capped message snippet attached to conversation group rows.

    Populated on `AgentSpanGroupRow.first_message` and
    `AgentSpanGroupRow.last_message` when grouping by `conversation_id`.
    Lets the conversations table render teaser text without an extra
    per-row hydration query.
...
class AgentSpanGroupRow(BaseModel):
    ...
    bundle (`span_count`, `total_*_tokens`, `*_seen`, etc.) is always
    present; `metrics` and `distributions` are populated only when the
    request asked for the corresponding spec.

    Note:
        `first_message`/`last_message` are populated only when grouping
        by `conversation_id`. The cost fields use `None` (not `0`) to
        mean "no priced model matched in this group."
    ...
    Attributes:
        ...
        first_seen: Earliest `started_at`. UTC.
        last_seen: Latest `started_at`. UTC.
        first_message: Preview of the earliest renderable message.
            Conversation grouping only.
        last_message: Preview of the latest renderable message.
            Conversation grouping only.
...
class AgentSpansQueryReq(BaseModel):
    ...
    One endpoint, two output shapes:

    1. **List mode** — `group_by` empty/omitted. Returns one
       `AgentSpanSchema` per matched span in `AgentSpansQueryRes.spans`.
       Use this for the spans table and trace drill-downs.
    2. **Group mode** — `group_by` set. Returns one `AgentSpanGroupRow`
       per group in `AgentSpansQueryRes.groups`, with optional
       `measures` and `distributions`. Use this for the agents,
       versions, and conversations lists (group by `conversation_id`).

    Note:
        `group_by` is the discriminator. Setting any group-only field
        (`measures`, `group_filters`, `group_distributions`) without
        also setting `group_by` is a validation error. `include_details`
        and `custom_attr_columns` are list-mode only.

    Attributes:
        project_id: Target project in `entity/project` form.
        query: Optional Mongo-style filter. Field names resolve through
            GenAI semantic conventions, direct span columns, or typed
            custom attribute keys.
        group_by: Refs to group by. Triggers group mode when non-empty.
        ...
        include_details: Include large fields (messages, tool payloads,
            `raw_span_dump`) on each span row. List mode only;
            intended for single-trace fetches, not list views.
        include_costs: Compute query-time USD costs by joining each
            span's model against `llm_token_prices`.
        sort_by: Sort steps. Allowed fields are validated server-side.
        limit: Max rows, 0 to `MAX_AGENT_QUERY_LIMIT`.
        offset: Rows to skip.
        started_after: Filters `started_at >= start`.
        started_before: Filters `started_at < end`.

    Raises:
        ValueError: When group-only fields are set without `group_by`,
            when `custom_attr_columns` or `include_details` is combined
            with `group_by`, when `group_distributions` has more than
            one `group_by` ref, when `custom_attr_columns` aren't
            custom-attr sources, or when measure/distribution aliases
...
class AgentCustomAttrsSchemaReq(BaseModel):
    ...
    Run this before constructing filters, group-bys, or distributions
    that target user-defined attributes — it tells the client which
    `(source, key, value_type)` triples actually exist in the data.
...
class AgentCustomAttrSchemaItem(BaseModel):
    ...
    Attributes:
        ...
        span_count: Spans in the discovery scope that carried the key.
    """
...
class AgentCustomAttrsSchemaRes(BaseModel):
    ...
    and `AgentSpanStatsReq` via the matching `custom_attrs_*` source
    on `AgentSpanValueRef`/`AgentGroupByRef`.
...
class AgentSearchReq(BaseModel):
    ...
    1. **Full-text search** — set `query` to a substring. Filters narrow
       the scope (e.g. "only assistant messages from agent X in the last
       week").
    2. **Structured retrieval** — leave `query` empty. Use the filters
       alone (e.g. set `trace_id` to fetch every message in a trace).
       Set `truncate_content=False` to receive full bodies.

    Note:
        Results are grouped by conversation in the response shape, not
        by trace. One conversation can contain matches from multiple
        traces.
    ...
    Attributes:
        ...
        truncate_content: `True` (default) returns previews;
            `False` returns full message bodies. Flip to `False` for
            structured retrieval, not for ad-hoc search UIs (payloads
            get large).
        started_after: Filters `started_at >= start`.
        started_before: Filters `started_at < end`.
...
class AgentChatAssistantMessage(BaseModel):
    ...
    Carries one assistant response, plus the LLM-call metadata
    (model, tokens, costs, status) the chat-view assembler folded in
    from the underlying span.

    Note:
        The `*_cost_usd` fields use `None` (not `0`) to mean "no priced
        model matched." For aggregated agent turns, cost fields are
        summed across the agent's whole subtree.
...
class AgentChatAgentHandoff(BaseModel):
    ...
    Reserved for handoff semantics in multi-agent frameworks. No fields
    yet — present so the timeline model is forward-compatible with
    handoff-aware UIs.
    """
...
class AgentChatMessage(BaseModel):
    ...
    `AgentChatMessage`. Each event is one of six discriminated kinds
    (see `AgentChatMessageType`); the payload field whose name matches
    `type` is populated, and all other payload fields must be `None`.

    Common header fields live at the top so headers and filters can be
    rendered without unwrapping the payload.

    Note:
        Exactly one payload field must be set, and it must match
        `type`. The validator rejects mismatched or missing payloads,
        which prevents drift between the discriminator and the data.
    ...
    Attributes:
        ...
        feedback: Feedback rows attached to this event. Populated only
            when the parent request set `include_feedback=True`.

    Raises:
        ValueError: When the payload field matching `type` is missing
            or any other payload field is also set.
    """
...
class AgentTraceChatReq(BaseModel):
    ...
    Attributes:
        project_id: Target project in `entity/project` form.
        trace_id: OTel trace ID for the turn.
...
class AgentTraceChatRes(BaseModel):
    ...
    duration, cost); `messages` is the ordered chat timeline. The same
    shape is reused inside `AgentConversationChatRes.turns`, so one
    client-side renderer covers both single-turn and per-turn views.
    ...
    Attributes:
        ...
        total_cost_usd: Sum of `total_cost_usd` across every span in
            the trace. `None` (not `0`) signals no priced model
            matched.
...
class AgentConversationChatReq(BaseModel):
    ...
    Returns an ordered page of turns sharing one `conversation_id`. The
    conversation id is `gen_ai.conversation.id` on the producing spans.
...
class AgentConversationChatRes(BaseModel):
    ...
    `turns` is an ordered list of per-turn `AgentTraceChatRes` rows so
    the client can reuse single-turn rendering for each one. Conversation
    totals (cost, feedback) sit on the parent response.

    Note:
        One turn corresponds to one trace, not one `invoke_agent` span.
        A turn can contain zero, one, or many agent invocations.
    ...
    Attributes:
        ...
        total_cost_usd: Sum of `total_cost_usd` across the returned
            turns. `None` (not `0`) signals no priced model matched.
...
class AgentSchema(BaseModel):
    ...
    versions and conversations. Entry point row for the Agents view.
...
class AgentsQueryReq(BaseModel):
    """Top-level agent listing request for `agent_agents_query`.
    ...
    Attributes:
        ...
        sort_by: Sort steps. Allowed fields are validated
            server-side.
        limit: Max rows, 0 to `MAX_AGENT_QUERY_LIMIT`.
        offset: Rows to skip.
...
class AgentVersionSchema(AgentSchema):
    ...
    `agent_version`; everything else has the same meaning.
...
class AgentVersionsQueryReq(BaseModel):
    ...
    Run after `agent_agents_query` to compare behavior across releases
    of one agent.
...
class AgentVersionsQueryRes(BaseModel):
    ...
    Attributes:
        versions: One `AgentVersionSchema` per matched
...
class GenAIOTelExportReq(BaseModel):
    ...
    The only write-side endpoint in this module. Sits at the start of
    the workflow: spans flow in here from the SDK or a raw OTel
    exporter, land in the agent spans table, and refresh the
    ...
    Note:
        ...
        the raw protobuf payload. Set
        `arbitrary_types_allowed=True` because Pydantic doesn't have
        a built-in schema for the OTel proto type.
