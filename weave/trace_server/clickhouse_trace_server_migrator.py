...
class ClickHouseTraceServerMigrator:
    ...
    def __init__(
        ...
        # run sync on class init
        asyncio.create_task(self._initialize_migration_db())
    ...
    async def apply_migrations(
        ...
        status = await self._get_migration_status(target_db)
        ...
        if len(migrations_to_apply) == 0:
            ...
            if should_insert_costs(status["curr_version"], target_version):
                await insert_costs(self.ch_client, target_db)
            return
        logger.info(f"Migrations to apply: {migrations_to_apply}")
        if status["curr_version"] == 0:
            await self.ch_client.command(self._create_db_sql(target_db))
        for target_version, migration_file in migrations_to_apply:
            await self._apply_migration(target_db, target_version, migration_file)
        if should_insert_costs(status["curr_version"], target_version):
            await insert_costs(self.ch_client, target_db)

    async def _initialize_migration_db(self) -> None:
        await self.ch_client.command(self._create_db_sql("db_management"))
        create_table_sql = """
            ...
            ORDER BY (db_name)
        """
        await self.ch_client.command(self._format_replicated_sql(create_table_sql))

    async def _get_migration_status(self, db_name: str) -> dict:
        ...
        """
        res = await self.ch_client.query(query)
    ...
    async def _execute_migration_command(self, target_db: str, command: str) -> None:
        ...
        await self.ch_client.command(self._format_replicated_sql(command))
    ...
    async def _apply_migration(
        ...
        await self._update_migration_status(target_db, target_version, is_start=True)

        # Execute each command in the migration
        migration_sub_commands = migration_sql.split(";")
        for command in migration_sub_commands:
            await self._execute_migration_command(target_db, command)

        # Mark migration as fully applied
        await self._update_migration_status(target_db, target_version, is_start=False)
