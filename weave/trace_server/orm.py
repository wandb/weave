...
class Column:
    ...
    def __init__(self, name: str, type: ColumnType, nullable: bool = False, db_name: str = None):
        self.name = name
...
class Table:
    ...
    def insert(self, row: typing.Optional[Row] = None):
        ins = Insert(self)
        if row:
            ins.row(row)
        return ins
...
class Select:
    ...
    def project_id(self, project_id: str) -> 'Select':
    ...
    def where(self, query: tsi.Query) -> 'Select':
        self._query = query
        return self
    ...
    def prepare(self):
        ...
        if self.action == 'SELECT':
            ...
            internal_fields = [_transform_external_field_to_internal_field(f, self.all_columns, self.json_columns, param_builder=self.param_builder)[0] for f in fieldnames]
        ...
        # print(' in prepare')
        # print(self._where)
        # sql += self._where.to_sql(self.db_names)

        # if self._order_by:
        #     sql += 'ORDER BY ' + ', '.join(f"{field_expr(o.field, self.db_names)} {o.direction}" for o in self._order_by) + '\n'
            ...
            for clause in self._order_by:
                ...
                for cast, direct in options:
                    ...
                    (
                        ...
                        field, self.all_columns, self.json_columns, cast, param_builder=self.param_builder
                    )
