...
from weave.trace_server.calls_query_builder.optimization_builder import (
    ...
    SortableDatetimeOptimizationProcessor,
    apply_processor,
...
CTE_INPUT_MATCHING_IDS = "input_matching_ids"
...
class CallsQuery(BaseModel):
    ...
    # When set, the heavy WHERE filter is replaced by AND id IN {_pre_filter_cte_name}.
    # See _try_build_pre_filter_cte_sql for details.
    _pre_filter_cte_name: str | None = PrivateAttr(default=None)
    ...
    def as_sql(self, pb: ParamBuilder, table_alias: str | None = None) -> str:
        ...
        if use_filter_cte:
            ...
            # For heavy field text searches on calls_merged, build a pre-filter CTE
            # that finds matching call IDs using strict LIKE (no OR IS NULL). This
            # avoids scanning millions of NULL-inputs partial rows before GROUP BY.
            pre_filter_cte_sql = filter_query._try_build_pre_filter_cte_sql(
                pb, table_alias_resolved, self.expand_columns
            )
            if pre_filter_cte_sql is not None:
                ctes.add_cte(CTE_INPUT_MATCHING_IDS, pre_filter_cte_sql)
                filter_query._pre_filter_cte_name = CTE_INPUT_MATCHING_IDS
    ...
    def _try_build_pre_filter_cte_sql(
        self,
        pb: ParamBuilder,
        table_alias: str,
        expand_columns: list[str] | None,
    ) -> str | None:
        """Build a pre-filter CTE SQL for heavy field text search optimization.

        For calls_merged, the normal LIKE WHERE optimization adds "OR field IS NULL"
        to avoid filtering out partial rows of an AggregatingMergeTree call. On
        high-traffic projects this causes expensive GROUP BY over millions of NULL
        rows that ultimately fail the HAVING filter.

        This CTE finds matching call IDs using strict LIKE (no IS NULL). The
        filtered_calls CTE then filters to those IDs, pulling all partial rows for
        correct HAVING evaluation (including deleted_at, ended_at checks).

        Returns the CTE SQL string, or None if no heavy field text search is present.
        """
        if self.read_table != ReadTable.CALLS_MERGED:
            return None
        ...
        if not non_obj_conditions:
            return None

        and_op = tsi_query.AndOperation(
            **{"$and": [c.operand for c in non_obj_conditions]}
        )

        # Compute strict heavy filter (no IS NULL) to find call IDs with matching data.
        # ParamBuilder deduplicates identical param values, so the LIKE param slot is
        # shared with the one later generated by the full optimization path.
        strict_processor = HeavyFieldOptimizationProcessor(
            pb, table_alias, use_null_check=False
        )
        strict_result = apply_processor(strict_processor, and_op)
        strict_sql = strict_processor.finalize_sql(strict_result)

        if not strict_sql:
            return None  # No heavy field text search; pre-filter not needed

        # Include sortable_datetime for efficient time-range pruning
        sortable_dt_processor = SortableDatetimeOptimizationProcessor(pb, table_alias)
        sortable_dt_result = apply_processor(sortable_dt_processor, and_op)
        sortable_dt_sql = sortable_dt_processor.finalize_sql(sortable_dt_result) or ""

        # Include trace_roots_only (parent_id IS NULL) filter if set
        ...
        pre_filter_where = WhereFilters(
            sortable_datetime=sortable_dt_sql,
            trace_roots_only=trace_roots_sql,
            heavy_filter=strict_sql,
        )
        where_sql = pre_filter_where.to_sql()
        where_stripped = re.sub(r"^\s*AND\s+", "", where_sql)
        where_clause = f"WHERE {where_stripped}" if where_stripped else ""

        return f"""SELECT {table_alias}.id
        FROM {table_alias}
    ...
    def _build_where_clause_optimizations(
        ...
        # If a pre-filter CTE is set, it replaces the heavy filter with an efficient
        # ID-based filter, avoiding the expensive OR IS NULL condition on high-traffic
        # projects where ~50% of partial rows have NULL inputs/output/attributes.
        if self._pre_filter_cte_name is not None:
            heavy_filter = ""
            id_subquery = f"AND ({table_alias}.id IN {self._pre_filter_cte_name})"
        else:
