...
def _extract_call_usage(
    ...
    for model_name, usage in usage_map.items():
        ...
        cache_write_input_tokens = _safe_int(
        ...
        if include_costs:
            ...
            if isinstance(cost_entry, dict):
                ...
                cache_write_cost = _safe_float(
                    cost_entry.get("cache_write_input_tokens_total_cost")
                )
        ...
        usage_obj = tsi.LLMAggregatedUsage(
            ...
            cache_write_input_tokens_total_cost=cache_write_cost,
        )
    ...
    for model_name, usage in source.items():
        ...
        if existing is None:
            ...
            if include_costs:
                ...
                existing.cache_write_input_tokens_total_cost = (
                    existing.cache_write_input_tokens_total_cost or 0.0
                ) + (usage.cache_write_input_tokens_total_cost or 0.0)
...
def _has_usage(usage: tsi.LLMAggregatedUsage, include_costs: bool) -> bool:
    ...
    if usage.requests or has_tokens:
        return True
    if include_costs:
        return bool(
            (usage.prompt_tokens_total_cost or 0.0)
            or (usage.completion_tokens_total_cost or 0.0)
            or (usage.cache_read_input_tokens_total_cost or 0.0)
            or (usage.cache_write_input_tokens_total_cost or 0.0)
        )
    return False
