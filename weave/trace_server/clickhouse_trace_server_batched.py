...
class ClickHouseTraceServer(tsi.FullTraceServerInterface):
    ...
    def completions_create(
        ...
        # Make a copy of inputs for the API call without prompt_ref and template_vars
    ...
    def completions_create_stream(
        ...
        if prompt_ref:
            try:
                ...
                logger.error(f"Failed to resolve prompt_ref: {e}", exc_info=True)

                # Yield error as single chunk then stop.
                def _single_error_iter(err: Exception) -> Iterator[dict[str, str]]:
                    yield {"error": f"Failed to resolve prompt_ref: {err!s}"}
        ...
        # Make a copy for the API call without prompt_ref and template_vars
        api_inputs = req.inputs.model_copy()
        if hasattr(api_inputs, "prompt_ref"):
            api_inputs.prompt_ref = None
        if hasattr(api_inputs, "template_vars"):
