...
class ClickHouseTraceServer(tsi.TraceServerInterface):
    ...
    def _find_descendants(self, project_id: str, ancestor_ids: list[str]) -> set[str]:
        ...
        if approach == "bottom_up":
            logger.debug(
                f"Using bottom-up approach for {len(ancestor_ids)} ancestors (forced by env)"
            )
            return self._find_descendants_bottom_up(project_id, ancestor_ids)
        elif approach == "top_down":
            logger.debug(
                f"Using top-down approach for {len(ancestor_ids)} ancestors (forced by env)"
            )
                ...
                for future in done:
                    try:
                        ...
                        if future == bottom_up_future:
                            logger.debug(
                                f"Bottom-up approach won for {len(ancestor_ids)} ancestors"
                            )
                        else:
                            logger.debug(
            ...
            raise ValueError(
                f"Invalid WEAVE_FIND_DESCENDANTS_APPROACH: {approach}. Must be 'race', 'bottom_up', or 'top_down'"
            )

    def _find_descendants_bottom_up(
        self, project_id: str, ancestor_ids: list[str]
    ) -> set[str]:
        ...
        while current_level:
            ...
            req = tsi.CallsQueryReq(
                ...
                filter=tsi.CallsFilter(parent_ids=list(current_level)),
    ...
    def _find_descendants_top_down(
        self, project_id: str, ancestor_ids: list[str]
    ) -> set[str]:
        ...
        req = tsi.CallsQueryReq(
            project_id=project_id,
            filter=tsi.CallsFilter(call_ids=list(ancestor_ids)),
            columns=["id", "trace_id"],
        )
