...
class ClickHouseTraceServer(tsi.TraceServerInterface):
    ...
    def op_create(self, req: tsi.OpCreateReq) -> tsi.OpCreateRes:
        ...
        # Use retry logic to handle race condition where object may not be immediately available
        ...
        # Construct the op_ref using internal reference format
        # ops are stored as objects, not in the /op/ namespace
        op_ref = ri.InternalObjectRef(
            project_id=req.project_id,
            name=req.name,
    ...
    def evaluation_create(
        ...
        # Construct the evaluation reference using internal reference format
    ...
    def dataset_create(self, req: tsi.DatasetCreateReq) -> tsi.DatasetCreateRes:
        ...
        # Construct table ref using internal reference format
        ...
        # Construct the dataset reference using internal reference format
    ...
    def dataset_get(self, req: tsi.DatasetGetReq) -> tsi.DatasetGetRes:
        ...
        # Parse the table reference and query the table to get actual rows
        rows = []
        table_ref_str = val.get("rows")
        if table_ref_str and isinstance(table_ref_str, str):
            # Parse the internal table ref
            table_ref = ri.parse_internal_uri(table_ref_str)
            if isinstance(table_ref, ri.InternalTableRef):
                # Query the table to get the actual rows
                table_query_req = tsi.TableQueryReq(
                    project_id=req.project_id,
                    digest=table_ref.digest,
                )
                table_query_res = self.table_query(table_query_req)
    ...
    def scorer_create(self, req: tsi.ScorerCreateReq) -> tsi.ScorerCreateRes:
        ...
        # Construct the scorer reference using internal reference format
