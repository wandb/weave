...
class ClickHouseTraceServer(tsi.FullTraceServerInterface):
    ...
    def dataset_sources_link(
        ...
        keeps relink idempotent. See membership_pattern.md.
        """
        ...
        # Detect any missing sources -> fail fast, no partial writes.
        ...
        prior_state: dict[tuple[str, str, str], dict[str, Any]] = {}
        ...
        for row_digest, source, meta in flat:
            ...
            logical_key = (row_digest, kind, source.source_id)
            created: bool | None
            created_at = now
            if req.include_created_status:
                ...
                # and created_at is refreshed to now (see membership_pattern.md).
                created = None
    ...
    def _fetch_dataset_link_state(
        ...
        """Collapsed current link state keyed by (row_digest, kind, source_id)."""
        logical_keys = sorted(
        ...
        for row in result.named_results():
            out[row["row_digest"], row["source_kind"], row["source_id"]] = row
        return out
