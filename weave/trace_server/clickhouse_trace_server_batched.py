...
class ClickHouseTraceServer(tsi.TraceServerInterface):
    ...
    def op_create_v2(self, req: tsi.OpCreateV2Req) -> tsi.OpCreateV2Res:
        ...
        source_code = (
            req.source_code or object_creation_utils.get_placeholder_op_source()
        )
    ...
    def op_read_v2(self, req: tsi.OpReadV2Req) -> tsi.OpReadV2Res:
        ...
        name = obj.object_id  # Default to object_id for ops
        code_ref = ""
        ...
        if object_creation_utils.OP_SOURCE_FILE_NAME in files:
            ...
            # Return a reference to the file instead of reading it
            code_ref = f"weave-trace-internal:///{req.project_id}/file/{file_digest}"
    ...
    def op_read_eager_v2(self, req: tsi.OpReadV2Req) -> tsi.OpReadEagerV2Res:
        """Get a specific op object with resolved code.

        This is similar to op_read_v2 but makes the additional API call to
        resolve the code reference and return the actual source code.
        """
        # Use the base op_read_v2 to get metadata and code_ref
        base_result = self.op_read_v2(req)

        # Now resolve the code_ref to get the actual code
        code = ""
        if base_result.code_ref:
            # Extract file digest from the code_ref format:
            # weave-trace-internal:///{project_id}/file/{file_digest}
            try:
                # Parse the internal reference
                if base_result.code_ref.startswith("weave-trace-internal:///"):
                    parts = base_result.code_ref.split("/")
                    # Format: weave-trace-internal:///{project_id}/file/{digest}
                    # After splitting by /, we get: ['weave-trace-internal:', '', '', project_id, 'file', digest]
        ...
        return tsi.OpReadEagerV2Res(
            object_id=base_result.object_id,
            digest=base_result.digest,
            version_index=base_result.version_index,
    ...
    def op_list_v2(self, req: tsi.OpListV2Req) -> Iterator[tsi.OpReadV2Res]:
        ...
        for obj in objs:
            ...
            try:
                ...
                pass  # If parsing fails, leave code_ref empty
    ...
    def dataset_read_eager_v2(
        ...
        """
        # Use the base dataset_read_v2 to get metadata and rows_ref
        base_result = self.dataset_read_v2(req)

        # Now resolve the rows_ref to get the actual rows
        rows: list[dict[str, Any]] = []
        if base_result.rows:
            try:
                # Parse the rows reference (should be a table reference)
                parsed_ref = ri.parse_internal_uri(base_result.rows)
    ...
    def scorer_create_v2(self, req: tsi.ScorerCreateV2Req) -> tsi.ScorerCreateV2Res:
        ...
        summarize_op_req = tsi.OpCreateV2Req(
            ...
            source_code=object_creation_utils.get_default_summarize_op_source(),
        )
    ...
    def scorer_list_v2(
        self, req: tsi.ScorerListV2Req
    ) -> Iterator[tsi.ScorerItemMetadataV2]:
        ...
        # Yield back a descriptive metadata object for each scorer
        for obj in obj_res.objs:
            name = None
            description = None
            ...
            yield tsi.ScorerItemMetadataV2(
                object_id=obj.object_id,
                digest=obj.digest,
                version_index=obj.version_index,
                created_at=obj.created_at,
    ...
    def evaluation_list_v2(
    ...
    ) -> Iterator[tsi.EvaluationItemMetadataV2]:
        ...
        for obj in result.objs:
            ...
            yield tsi.EvaluationItemMetadataV2(
                object_id=obj.object_id,
                digest=obj.digest,
                version_index=obj.version_index,
                created_at=obj.created_at,
