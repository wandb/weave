...
class ClickHouseTraceServer(tsi.FullTraceServerInterface):
    ...
    def dataset_sources_link(
        ...
        for row_digest, source, meta in flat:
            ...
            if source.source_kind == tsi.SourceKind.CALL:
                display_name, started_at = call_fields[source.source_id][:2]
            elif source.source_kind == tsi.SourceKind.SPAN:
                key = (source.source_trace_id, source.source_id)
                display_name, started_at = span_fields[key]
            else:
                # Conversation: asserted link, no cached source fields.
                display_name, started_at = "", now
            ...
            logical_key = (row_digest, kind, source.source_id, source.source_trace_id)
            created: bool | None
            created_at = now
            if req.include_created_status:
