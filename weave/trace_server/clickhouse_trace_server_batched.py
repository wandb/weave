...
class ClickHouseTraceServer(tsi.FullTraceServerInterface):
    ...
    def _assert_obj_version_exists(
        ...
        """Raise NotFoundError if the object version doesn't exist."""
        query = """
            SELECT 1
    ...
    def _set_alias(
        ...
        """Internal helper to insert an alias row."""
    ...
    def obj_remove_alias(self, req: tsi.ObjRemoveAliasReq) -> tsi.ObjRemoveAliasRes:
        ...
        ch_alias = AliasCHInsertable(
            ...
            object_id=req.object_id,
            alias=req.alias,
            digest="",  # digest doesn't matter for delete; ORDER BY key is (project_id, object_id, alias)
            wb_user_id=req.wb_user_id or "",
            deleted_at=now,
        )
        self._insert(
    ...
    def _maybe_resolve_alias(
        ...
        if is_version:
            return None
        # If it looks like a sha256 hash (64 hex chars), it's a real digest
        if len(digest) == 64:
            try:
                int(digest, 16)
            except ValueError:
