...
def _fetch_feedback_rows(
    ...
    _, cursor = _sqlite_ts.get_conn_cursor(server.db_path)
...
def sqlite_feedback_stats(
    ...
    for created_at_str, payload_str in rows:
        ...
        # Count every row, not just rows with a non-null value for some metric,
        # to match ClickHouse's count() which counts all rows in the bucket.
