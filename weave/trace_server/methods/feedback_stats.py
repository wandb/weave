...
def feedback_stats(
    server: ClickHouseTraceServer, req: tsi.FeedbackStatsReq
) -> tsi.FeedbackStatsRes:
    """Run ClickHouse-backed feedback stats queries for a request.

    Args:
        server: ClickHouse trace server instance.
    ...
    if not req.metrics:
        # No metrics requested — return an empty response with a default 1h
        # granularity (3600s) as a sensible placeholder.
...
def feedback_payload_schema(
    server: ClickHouseTraceServer, req: tsi.FeedbackPayloadSchemaReq
