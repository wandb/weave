...
def _json_path_to_extraction_sql(
    ...
    if value_type == "boolean":
        # JSONExtractRaw returns the raw JSON token, so JSON `true` comes back
        # as the string 'true' and `false` as 'false' — single-quote comparison
        # is intentional here (ClickHouse has no toBoolOrNull equivalent).
