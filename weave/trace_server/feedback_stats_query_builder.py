...
def _json_path_to_raw_expr(json_path: str, col: str = "payload_dump") -> str:
    """Build chained JSONExtractRaw SQL for a dot path."""
...
def _json_path_to_extraction_sql(
    ...
    if value_type == "boolean":
        # ClickHouse has no toBoolOrNull; JSONExtractRaw returns "true"/"false" strings
        return f"if({raw} = 'true', 1, if({raw} = 'false', 0, NULL))"
    if value_type == "categorical":
        return f"JSONExtractString({col}, '{json_path}')"
    return f"toFloat64OrNull({raw})"
...
def _build_query_context(
    ...
    start_param = pb.add_param(start.replace(tzinfo=datetime.timezone.utc).timestamp())
    end_param = pb.add_param(end.replace(tzinfo=datetime.timezone.utc).timestamp())
...
def build_feedback_stats_query(
    ...
    bucket_expr = (
        f"toStartOfInterval(created_at, INTERVAL {granularity_seconds} SECOND, "
          ...
          toUInt64(
            ceil(
              (toUnixTimestamp(toDateTime({param_slot(ep, "Float64")}, {param_slot(tp, "String")})) -
               toUnixTimestamp(toStartOfInterval(toDateTime({param_slot(sp, "Float64")}, {param_slot(tp, "String")}), {all_buckets_interval}, {param_slot(tp, "String")})))
...
@dataclass
class FeedbackStatsWindowQueryResult:
    """Result of window-level feedback stats query (aggregated over the full time range)."""
...
def build_feedback_stats_window_query(
    req: FeedbackStatsReq,
    pb: ParamBuilder,
) -> FeedbackStatsWindowQueryResult | None:
    ...
    return FeedbackStatsWindowQueryResult(
        sql=safely_format_sql(raw_sql, logger),
        columns=columns,
        parameters=pb.get_params(),
    )
