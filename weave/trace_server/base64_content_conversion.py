...
# Reject strings whose first 32 chars have fewer than 3 distinct characters.
# Real base64 of binary content has uniformly high entropy in any short
# window; pathological inputs like 'x' * 50000 collapse to one unique char
# and would otherwise pass the regex and force a full Content.from_base64
# decode + libmagic probe. See DD trace 6a0c7d99...975ce4fb10f7c246 (454ms leaf).
...
def is_base64(value: str) -> bool:
    ...
    A cheap entropy floor runs before the regex to short-circuit pathological
    low-entropy inputs (e.g. "x" * 50000) that otherwise pass the alphabet
    check and trigger a wasted Content.from_base64 decode + libmagic probe.
