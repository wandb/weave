...
def cmd_run(args: argparse.Namespace) -> None:
    ...
    try:
        logger.info(
            f"Starting backfill for database={args.database}, version={args.version}"
        )
        if args.max_batches:
...
def cmd_pause(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        logger.exception("✗ Failed to pause backfill")
        sys.exit(1)
...
def cmd_resume(args: argparse.Namespace) -> None:
    ...
    try:
        logger.info(
            f"Resuming backfill for database={args.database}, version={args.version}"
        )
        if args.max_batches:
            logger.info(f"Limited to {args.max_batches} batches for testing")
        ...
        logger.exception("✗ Failed to resume backfill")
        sys.exit(1)
    except Exception as e:
        logger.exception("✗ Unexpected error")
        sys.exit(1)
    ...
    try:
        if args.all:
            ...
            if not args.database or not args.version:
                print(
                    "✗ Error: --database and --version are required when not using --all"
                )
                sys.exit(1)

            backfill_id = f"{args.database}_{args.version}"
            result = manager.ch_client.command(
                f"DELETE FROM db_management.backfills WHERE backfill_id = '{backfill_id}'"
            )
            ...
            if not status.get("found"):
                ...
                print(
                    f"  Warning: {backfill_id} still exists (deletion may have failed)"
                )

    except Exception as e:
        logger.exception("✗ Failed to clear checkpoint")
        sys.exit(1)
...
Environment Variables:
  ...
  CLICKHOUSE_PASSWORD    Password (default: empty)
    ...
    list_parser.add_argument(
        "--status",
        help="Filter by status (pending, running, completed, failed, paused)",
    )
    ...
    run_parser.add_argument(
        "--version", type=int, required=True, help="Backfill version number"
    )
    run_parser.add_argument(
        "--max-batches", type=int, help="Maximum batches to run (for testing)"
    )
    ...
    clear_group.add_argument(
        "--database", help="Target database name (use with --version)"
    )
    clear_group.add_argument(
        "--all", action="store_true", help="Clear all backfill checkpoints"
    )
    clear_parser.add_argument(
        "--version", type=int, help="Backfill version number (use with --database)"
    )
