...
class ClickHouseBackfillManager:
        ...
        for file in backfill_dir.iterdir():
            if file.suffix == ".meta" and ".backfill" in file.name:
                ...
                if not sql_file.exists():
                    raise BackfillError(f"Missing SQL file for backfill version {version}: {sql_file}")
    ...
    def _validate_metadata(self, metadata: dict, version: int) -> None:
        ...
        for field in required:
            if field not in metadata:
                raise BackfillError(f"Missing required field '{field}' in backfill metadata for version {version}")
        
        if metadata["migration_version"] != version:
            raise BackfillError(f"Metadata version mismatch: expected {version}, got {metadata['migration_version']}")
        ...
        if not isinstance(metadata["checkpoint_columns"], list) or len(metadata["checkpoint_columns"]) == 0:
        ...
        if "cutover_timestamp" in metadata.get("custom_fields", []):
            ...
            if "cutover_timestamp" in metadata:
                ...
                try:
                    ...
                    logger.error(f"Error calculating cutover timestamp: {e}")
                    raise
    ...
    def _render_sql(self, template: str, checkpoint: dict, metadata: dict, db_name: str) -> str:
        """
        ...
        sql = sql.replace("{batch_size}", str(checkpoint.get("current_batch_size", metadata["batch_size"])))
        ...
        for key, value in checkpoint.items():
            if key not in ["current_batch_size", "rows_processed", "batches_completed", "started_at", "last_updated"]:
                if key not in metadata["checkpoint_columns"]:
                    placeholder = f"{{{key}}}"
                    sql = sql.replace(placeholder, str(value))
    ...
    def _advance_checkpoint_by_window(self, checkpoint: dict, current_time: str) -> None:
        """
    ...
    def _execute_batch(self, sql: str, checkpoint: dict, metadata: dict) -> dict:
        ...
        for attempt in range(max_retries):
            try:
                ...
                return {"rows_affected": rows_affected, "success": True, "error": None, "execution_time": batch_execution_time}
                ...
                if "memory" in error_msg or "too many" in error_msg:
                    ...
                    if "sortable_datetime" in metadata["checkpoint_columns"]:
                        current_window_minutes = checkpoint.get("current_window_minutes", 60)  # Default 60 minutes
                        ...
                        logger.warning(f"OOM on attempt {attempt + 1}, reducing time window to {new_window_minutes} minutes")
                        ...
                        batch_execution_time = time.time() - batch_start_time
                        return {"rows_affected": 0, "success": False, "error": "RENDER_SQL", "retry": True, "execution_time": batch_execution_time}
                    else:
                        ...
                        logger.warning(f"OOM on attempt {attempt + 1}, reducing batch size to {batch_size}")
                ...
                if attempt == max_retries - 1:
                    batch_execution_time = time.time() - batch_start_time
                    return {"rows_affected": 0, "success": False, "error": str(e), "execution_time": batch_execution_time}
                
                time.sleep(2 ** attempt)
        
        batch_execution_time = time.time() - batch_start_time
        return {"rows_affected": 0, "success": False, "error": "Max retries exceeded", "execution_time": batch_execution_time}
    ...
    def _get_last_inserted_position(self, db_name: str, metadata: dict) -> dict:
        ...
        """
        target_table = metadata.get("target_table", metadata.get("description", "").split()[-1])
        ...
        if not result.result_rows:
            return {col: ("1970-01-01 00:00:00" if col == "sortable_datetime" else "") for col in checkpoint_cols}
        
        row = result.result_rows[0]
        return {col: (row[i] if row[i] is not None else ("1970-01-01 00:00:00" if col == "sortable_datetime" else "")) for i, col in enumerate(checkpoint_cols)}

    def _register_backfill(self, backfill_id: str, db_name: str, version: int, metadata: dict, checkpoint: dict) -> None:
        """
        ...
        self.ch_client.insert(
            ...
            data=[[backfill_id, version, db_name, "pending", checkpoint_json, 0, None, now, None, None]],
    ...
    def _update_status(self, backfill_id: str, status: str, error: Optional[str] = None) -> None:
        """
    ...
    def run_backfill(
        ...
        if not checkpoint:
            ...
            self._register_backfill(backfill_id, database, version, metadata, checkpoint)
        else:
            ...
            while True:
                ...
                if not result["success"]:
                    if result.get("retry") and result["error"] == "RENDER_SQL":
                        ...
                        raise BackfillError(f"Batch execution failed: {result['error']}")
                ...
                if "sortable_datetime" in metadata["checkpoint_columns"]:
                    ...
                    if max_result.result_rows and max_result.result_rows[0][0]:
                        ...
                        if current_time_dt >= max_time:
                            ...
                            total_wall_time = time.time() - checkpoint.get("backfill_start_time", time.time())
                            total_time_str = f"{total_wall_time:.2f}s"
                            final_batch_size = checkpoint.get("current_batch_size", metadata["batch_size"])
                            logger.info(f"Backfill {backfill_id} completed! Reached end of available data. Total time: {total_time_str}, final batch_size: {final_batch_size}")
                            self._print_status_table()
                            break
                    
                    # Determine how to advance the checkpoint
                    logger.debug(f"Batch result: rows_affected={result['rows_affected']}, current_time={current_time}")
                    if result["rows_affected"] > 0:
                        # We processed rows - check if we hit the batch limit
                        batch_size = checkpoint.get("current_batch_size", metadata["batch_size"])
                        logger.debug(f"Checking batch limit: rows_affected={result['rows_affected']}, batch_size={batch_size}")
                        ...
                        if result["rows_affected"] >= batch_size:
                            ...
                            logger.info(f"Hit batch limit! rows_affected={result['rows_affected']}, batch_size={batch_size}")
                            window_minutes = checkpoint.get("current_window_minutes", 60)
                            ...
                            """
                            try:
                                last_started_result = self.ch_client.query(last_started_query)
                                logger.debug(f"Last started query result: {last_started_result.result_rows}")
                                if last_started_result.result_rows and last_started_result.result_rows[0][0]:
                                    last_started_at = last_started_result.result_rows[0][0]
                                    last_started_at_str = last_started_at.strftime("%Y-%m-%d %H:%M:%S")
                                    logger.info(f"Hit batch limit, advancing from {current_time} to last started_at: {last_started_at_str}")
                                    checkpoint["sortable_datetime"] = last_started_at_str
                                    ...
                                    logger.warning("Hit batch limit but no started_at found, falling back to window advancement")
                                    self._advance_checkpoint_by_window(checkpoint, current_time)
                            except Exception as e:
                                logger.warning(f"Failed to get last started_at: {e}, falling back to window advancement")
                                self._advance_checkpoint_by_window(checkpoint, current_time)
                        else:
                            # We didn't hit the batch limit - advance by full window
                            logger.debug(f"Didn't hit batch limit (rows_affected={result['rows_affected']} < batch_size={batch_size}), advancing by full window")
                            self._advance_checkpoint_by_window(checkpoint, current_time)
                    else:
                    ...
                    if result["rows_affected"] == 0:
                        ...
                        total_time_str = f"{total_wall_time:.2f}s"
                        final_batch_size = checkpoint.get("current_batch_size", metadata["batch_size"])
                        logger.info(f"Backfill {backfill_id} completed! Total time: {total_time_str}, final batch_size: {final_batch_size}")
                        self._print_status_table()
                        break
                ...
                total_time_str = f"{total_wall_time:.2f}s"
                current_batch_size = checkpoint.get("current_batch_size", metadata["batch_size"])
    ...
    def _print_status_table(self) -> None:
        ...
        # Print header
        logger.info("")
        logger.info("Version    Database        Status       Rows Processed     Started              Updated")
        logger.info("=" * 100)
        
        # Print each backfill
        for backfill in backfills:
            started = backfill.get("started_at", "").strftime("%Y-%m-%d %H:%M:%S") if backfill.get("started_at") else ""
            updated = backfill.get("updated_at", "").strftime("%Y-%m-%d %H:%M:%S") if backfill.get("updated_at") else ""
            
            logger.info(f"{backfill['migration_version']:<10} {backfill['db_name']:<15} {backfill['status']:<12} {backfill['rows_processed']:<17} {started:<19} {updated}")
    ...
    def resume_backfill(self, database: str, version: int, max_batches: Optional[int] = None) -> None:
        """
        ...
        if status["status"] not in ["paused", "failed"]:
            raise BackfillError(f"Cannot resume backfill with status: {status['status']}")
        
        logger.info(f"Resuming backfill {database}_{version} from checkpoint")
        self.run_backfill(database, version, max_batches)

    def register_pending_backfills(self, db_name: str, migration_versions: list[int]) -> None:
        """
        ...
        for version in migration_versions:
            ...
            if metadata.get("run_on_startup", False):
                logger.info(f"Running backfill {backfill_id} inline (run_on_startup=true)")
                self.run_backfill(db_name, version)
            else:
                logger.info(f"Backfill {backfill_id} registered. Run manually: manager.run_backfill(database='{db_name}', version={version})")
