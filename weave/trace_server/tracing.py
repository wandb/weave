...
Why a new module instead of editing `datadog.py`:
  ...
  - Existing public symbols in `datadog.py` (`record_db_insert`,
    ...
    surface moves here.
  - Keeps the PR-7a diff scoped to additions, so reviewers can see the new
    decorator contract without scanning for what changed in `datadog.py`.
...
This module mirrors the call-site surface of `wandb/core`'s
`services/weave-trace/src/tracing/decorator.py` during the dual-pipeline
window.
"""
