...
class AnnotationQueueSchema(BaseModel):
    ...
    due_date: datetime.datetime | None = None  # Optional deadline for completing annotations
    created_at: datetime.datetime
...
class AnnotationQueueCreateReq(BaseModelStrict):
    ...
    description: str | None = Field(
        default=None, examples=["Review calls with exceptions"]
    )
    ...
    claim_timeout_seconds: int | None = Field(
        default=1800, description="Duration in seconds before a claimed item expires"
    )
    due_date: datetime.datetime | None = Field(
        default=None,
        description="Deadline for completing annotations in this queue",
    )
    wb_user_id: str | None = Field(None, description=WB_USER_ID_DESCRIPTION)
