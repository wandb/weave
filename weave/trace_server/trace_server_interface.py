...
class OpReadMetadataV2(BaseModel):
    """Base metadata model for Op read operations."""
...
class OpReadV2Res(OpReadMetadataV2):
    """Response model for reading an Op object with a code reference.

    Returns a reference to the code file rather than the actual code content.
    """

    code_ref: str = Field(..., description="Reference to the op code file")


class OpReadEagerV2Res(OpReadMetadataV2):
    """Response model for eagerly reading an Op object with resolved code.

    Returns the actual source code instead of just a reference to it.
    """
...
class ScorerItemMetadataV2(BaseModel):
    object_id: str = Field(..., description="The scorer ID")
    digest: str = Field(..., description="The digest of the scorer")
    version_index: int = Field(..., description="The version index of the object")
    created_at: datetime.datetime = Field(
...
class ScorerReadV2Res(ScorerItemMetadataV2):
    score_op_ref: str = Field(..., description="The Scorer.score op reference")
...
class EvaluationRunStartV2Body(BaseModel):
    evaluation_ref: str = Field(
...
class EvaluationRunStartV2Req(EvaluationRunStartV2Body):
    project_id: str = Field(..., description="Project ID (e.g., 'entity/project')")


class EvaluationRunStartV2Res(BaseModel):
...
class EvaluationRunFilterV2(BaseModelStrict):
    evaluation_refs: Optional[list[str]] = Field(
        default=None,
        description="Filter by evaluation references (weave:// URIs)",
        examples=[["weave:///entity/project/object/Evaluation:abc123"]],
    )
    model_refs: Optional[list[str]] = Field(
        default=None,
...
class EvaluationRunListV2Req(BaseModel):
    ...
    filter: Optional[EvaluationRunFilterV2] = Field(
        default=None,
        description="Filter criteria for evaluation runs",
    )
    limit: Optional[int] = Field(
...
class EvaluationRunDeleteV2Req(BaseModelStrict):
    project_id: str = Field(
        ..., description="The `entity/project` where this evaluation run exists"
    )
...
class TraceServerInterface(Protocol):
    ...
    def evaluation_run_start_v2(
        self, req: EvaluationRunStartV2Req
    ) -> EvaluationRunStartV2Res: ...
