...
class CallReadReq(BaseModel):
    ...
    include_rolled_up_storage_size: Optional[bool] = False
...
class CallsQueryReq(BaseModel):
    ...
    include_rolled_up_storage_size: Optional[bool] = Field(
        default=False,
        description="Beta, subject to change. If true, the response will"
        " include the rolled up storage size for each root call only.",
    )
