...
# ---------------------------------------------------------------------------
# Agent structured conversation ingest
# ---------------------------------------------------------------------------
...
class AgentStructuredToolCall(BaseModel):
...
class AgentStructuredTurn(BaseModel):
    """One user-agent interaction within a conversation.

    Each turn maps to a unique ``trace_id`` in the underlying span storage.
    """

    messages: list[AgentStructuredMessage]
    tool_calls: list[AgentStructuredToolCall] = Field(default_factory=list)
    agent_name: str = ""
    agent_version: str = ""
    model: str = ""
    input_tokens: int = 0
...
class AgentConversationIngestReq(BaseModel):
    ...
    agent_name: str = ""
    agent_version: str = ""
    turns: list[AgentStructuredTurn]


class AgentConversationIngestRes(BaseModel):
...
# ---------------------------------------------------------------------------
# ---------------------------------------------------------------------------
# Agent traces (from genai_traces MV)
# ---------------------------------------------------------------------------


class AgentTraceSchema(BaseModel):
    """Aggregated per-trace stats from the genai_traces MV."""
...
class AgentVersionSchema(BaseModel):
    """Aggregated per-version stats from the genai_agent_versions MV."""
