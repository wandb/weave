CREATE TABLE calls_complete2 (
    -- used for deduping/choosing the most recent row on primary key
    -- must be monotonically increasing, callers should use timestamp
    version         UInt8,
...
PARTITION BY toStartOfWeekYYYYMMDD(started_at)
ORDER BY (project_id, started_at, id)
SETTINGS
    -- Required for lightweight updates
    enable_block_number_column=1,
    enable_block_offset_column=1,
    min_age_to_force_merge_seconds=86400, -- 1 day
