...
def ref_stable_json_digest(json_val: Any) -> str:
    ...
    return _json_digest(_map_val(json_val, mapper))
...
def _stabilize_ref_str(ref: str) -> str:
    if ref.startswith(external_ref_prefix):
        # Cut off the entity/project/ part of the ref
        ref = ref[len(external_ref_prefix) :]
        parts = ref.split("/")
        if len(parts) < 3:
            raise ValueError(f"Invalid URI: {ref}")
        _, _, stable_content_address = parts[:3]
        return _make_stabilized_ref_str(stable_content_address)
    elif ref.startswith(internal_ref_prefix):
        # Cut off the project_id/ part of the ref
        ref = ref[len(internal_ref_prefix) :]
        parts = ref.split("/")
        if len(parts) < 2:
            raise ValueError(f"Invalid URI: {ref}")
        _, stable_content_address = parts[:2]
        return _make_stabilized_ref_str(stable_content_address)
    return ref
