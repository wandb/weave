...
def wait_for_database_engine(
    ...
    *context* is optional human-readable text (e.g. the SQL that created the
    ...
    """
    attempt_count = 0
    delay_seconds = ENGINE_DISCOVERY_INITIAL_DELAY_SECONDS
    start_time = time.monotonic()

    while True:
        attempt_count += 1
        engine = get_database_engine(ch_client, db_name)
        ...
        delay_seconds = min(delay_seconds * 2, ENGINE_DISCOVERY_MAX_DELAY_SECONDS)

    context_clause = ""
    if context is not None:
        context_clause = f" after executing `{context}`"

    raise EngineDiscoveryError(
        f"Could not determine database engine for `{db_name}` after "
        f"{attempt_count} attempts over "
