...
#         return f"""weave.legacy.codify.load_type({obj_type_name}, {d})"""
#     return None
...
def _otc_using_storage_fallback(obj: typing.Any) -> str:
    return f"""weave.legacy.codify.load({storage.to_python(obj)})"""
...
def _module_name_corrections(qualified_name: str) -> str:
    if qualified_name == "weave.legacy.ops_primitives.file_local":
        return "weave.legacy.ops"
    ...
    if isinstance(node, graph.VoidNode):
        ...
        if node.from_op.name == "dict":
            ...
            if len(node.from_op.inputs) > 0:
                args += ","
            return f"weave.legacy.ops_primitives.dict.dict_({args})"
            ...
            if len(node.from_op.inputs) > 0:
                args += ","
            return f"weave.legacy.ops_primitives.list_.make_list({args})"
