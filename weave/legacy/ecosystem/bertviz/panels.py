...
@weave.op()
def head_view(attention: huggingface.ModelOutputAttention) -> weave.legacy.ops.Html:
    ...
    return weave.legacy.ops.Html(html.data)
...
class BertvizHeadView(weave.Panel):
    ...
    @weave.op()
    def render(self) -> weave.legacy.panels.PanelHtml:
        ...
        return weave.legacy.panels.PanelHtml(html)


@weave.op()
def model_view(attention: huggingface.ModelOutputAttention) -> weave.legacy.ops.Html:
...
class BertvizModelView(weave.Panel):
    ...
    @weave.op()
    def model_view_panel_render(self) -> weave.legacy.panels.PanelHtml:
