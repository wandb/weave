...
import weave
import weave.legacy
from weave.legacy import val_const
...
class MetricsBankPanel(weave.Panel):
    ...
    @weave.op()
    def render(self) -> weave.legacy.panels.Each:
        input = typing.cast(MetricsBankInput, self.input_node)

        baseline = input["baseline"]
        candidate = input["candidate"]

        joined = weave.legacy.ops.join_all(
            weave.legacy.ops.make_list(l0=baseline, l1=candidate),
        ...
        metrics = weave.legacy.ops.difference(
            joined_keys, [val_const.const("scenario_id")]
        )

        # TODO: broken
        return weave.legacy.panels.Each(
            metrics,
            render=lambda metric_name: weave.legacy.panels.Group(
                items={
                    "title": metric_name,
