...
import weave
from weave.legacy.ecosystem import huggingface as hf
from weave.legacy.ecosystem import xgboost as weave_xgb
...
class ShapValues:
    ...
    def summary_plot(self):
        ...
        plt.savefig(path)
        plt.close()
        return weave.legacy.ops.LocalFile(path)
...
@weave.op()
def shap_plot_text(shap_values: shap.Explanation) -> weave.legacy.ops.Html:
...
class ShapPlotText(weave.Panel):
    ...
    def render(self) -> weave.legacy.panels.PanelHtml:
        return weave.legacy.panels.PanelHtml(shap_plot_text(self.input_node))
