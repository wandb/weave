...
import weave
from weave.legacy import panel, panel_util
...
def single_distribution(
    ...
    binned = input_node.groupby(lambda v: round(v * 10) / 10).map(  # type: ignore
        lambda group: weave.legacy.ops.dict_(value=group.key(), count=group.count())
    )
...
class AdderConfig:
    operand: weave.Node[int] = dataclasses.field(
        default_factory=lambda: weave.legacy.graph.ConstNode(weave.types.Int(), 10)
    )
...
def adder_config(
    ...
    return weave.legacy.panels.LabeledItem(
        label="operand",
        item=weave.legacy.panels.Slider(
            config=weave.legacy.panels.SliderConfig(
                weave.legacy.ops.execute(config.operand)
            )
        ),
    )
...
def adder(
    ...
    input_val = typing.cast(int, input_node)
    config = adder_default_config(config)
    return weave.legacy.panels.LabeledItem(label="output", item=input_val + config.operand)  # type: ignore
