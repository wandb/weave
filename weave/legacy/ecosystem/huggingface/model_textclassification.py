...
import weave
from weave.legacy.ecosystem.huggingface import hfmodel
...
class ClassificationResultPanel(weave.Panel):
    ...
    def render(self) -> weave.legacy.panels.Table:
        from weave.legacy.ecosystem.huggingface.huggingface_models import huggingface

        return weave.legacy.panels.Table(
            self.input_node,
            columns=[
                lambda result_row: weave.legacy.panels.WeaveLink(
...
class FullTextClassificationResultPanel(weave.Panel):
    ...
    def render(self) -> weave.legacy.panels.Group:
        ...
        return weave.legacy.panels.Group(
            ...
            items={
                "input": weave.legacy.panels.LabeledItem(
                    label="input", item=output.model_input
                ),
                "output": weave.legacy.panels.LabeledItem(
                    label="output",
                    item=weave.legacy.panels.Plot(
