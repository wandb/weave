...
import weave
from weave.legacy.ecosystem import pytorch
from weave.legacy.ecosystem.huggingface import (
    hfmodel,
...
class HuggingfaceModelsPanel(weave.Panel):
    ...
    def render(self) -> weave.legacy.panels.Table:
        return weave.legacy.panels.Table(
            self.input_node,
            columns=[
                lambda model_row: weave.legacy.panels.WeaveLink(
                    model_row.id(),
        ...
        return weave.legacy.panels.Card(
            ...
            content=[
                weave.legacy.panels.CardTab(
                    name="Model Card",
                    content=weave.legacy.panels.PanelMarkdown(model.readme()),  # type: ignore
                ),
                weave.legacy.panels.CardTab(
                    name="Metadata",
                    content=weave.legacy.panels.Group(
                        items={
                            "id": weave.legacy.panels.LabeledItem(
                                item=model.id(), label="ID"
                            ),
                            "pipeline_tag": weave.legacy.panels.LabeledItem(
                ...
                #         weave.legacy.ops.used_by(model, model.call.op_name()),
                #         columns=[
...
class HuggingfacePackagePanel(weave.Panel):
    ...
    def render(self) -> weave.legacy.panels.Card:
        ...
        return weave.legacy.panels.Card(
            ...
            content=[
                weave.legacy.panels.CardTab(name="Models", content=pack.models()),  # type: ignore
            ],
        )
