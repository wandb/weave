...
class TimeSeriesConfig:
    ...
    ] = dataclasses.field(default_factory=lambda: weave.legacy.graph.VoidNode())
    mark: weave.Node[str] = dataclasses.field(
        default_factory=lambda: weave.legacy.graph.ConstNode(
...
def first_column_of_type(
...
) -> typing.Tuple[weave.legacy.graph.ConstNode, weave.legacy.graph.ConstNode]:
    ...
    if weave.types.List().assign_type(node_type):
        ...
        # return weave_internal.define_fn(
        #     {"input_node": node_type}, weave.legacy.graph.VoidNode()
        # ), weave_internal.define_fn({"item": object_type}, lambda _: weave.legacy.graph.VoidNode())
    raise ValueError(
...
class TimeSeries(weave.Panel):
    ...
    def __init__(self, input_node, vars=None, config=None, **options):
        ...
        if self.config is None:
            ...
            for attr in ["x", "min_x", "max_x", "label", "mark", "agg", "axis_labels"]:
                if attr in options:
                    value = options[attr]
                    if not isinstance(value, weave.legacy.graph.Node):
                    ...
                    value = weave_internal.define_fn(
                        ...
                        lambda item: weave.legacy.graph.VoidNode(),
                    )
    ...
    def render_config(self) -> weave.legacy.panels.Group:
        ...
        return weave.legacy.panels.Group(
            items={
                "x": weave.legacy.panels.LabeledItem(
                    label="x", item=weave.legacy.panels.FunctionEditor(config.x)
                ),
                "label": weave.legacy.panels.LabeledItem(
                    label="label",
                    item=weave.legacy.panels.FunctionEditor(config.label),
                ),
                "min_x": weave.legacy.panels.LabeledItem(
                    label="min_x",
                    item=weave.legacy.panels.FunctionEditor(config.min_x),
                ),
                "max_x": weave.legacy.panels.LabeledItem(
                    label="max_x",
                    item=weave.legacy.panels.FunctionEditor(config.max_x),
                ),
                "agg": weave.legacy.panels.LabeledItem(
                    label="agg", item=weave.legacy.panels.FunctionEditor(config.agg)
                ),
                "mark": weave.legacy.panels.LabeledItem(
                    label="mark",
                    item=weave.legacy.panels.ObjectPicker(
                        ...
                        config=weave.legacy.panels.ObjectPickerConfig(
                            choice=config.mark
                        ),
                    ),
                ),
    ...
    def render(self) -> weave_plotly.PanelPlotly:
        ...
        bin_size_index = TIME_SERIES_BIN_SIZES_SEC_NODE.map(  # type: ignore
            lambda x: (
                ...
                / weave.legacy.ops.make_list(a=x, b=exact_bin_size).min()
            )
        ...
        def bin_fn(item):
            ...
            return weave.legacy.ops.dict_(**group_items)
        ...
        binned = (
            ...
            .map(
                lambda group: weave.legacy.ops.dict_(
            ...
            .sort(
                lambda item: weave.legacy.ops.make_list(a=item["x"]["start"]),
                ["asc"],
            )
        )

        default_labels = weave.legacy.ops.dict_(
