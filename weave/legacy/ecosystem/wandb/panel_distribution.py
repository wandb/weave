...
class Distribution(weave.Panel):
    ...
    def render_config(self) -> weave.legacy.panels.Group:
        ...
        return weave.legacy.panels.Group(
            items={
                "value_fn": weave.legacy.panels.LabeledItem(
                    label="value",
                    item=weave.legacy.panels.FunctionEditor(config.value_fn),
                ),
                "label_fn": weave.legacy.panels.LabeledItem(
                    label="label",
                    item=weave.legacy.panels.FunctionEditor(config.label_fn),
                ),
                "bin_size": weave.legacy.panels.LabeledItem(
                    label="bin_size",
                    ...
                    # owner.
                    item=weave.legacy.panels.Slider(config.bin_size.execute()),  # type: ignore
                ),
            }
        )
    ...
    def render(self) -> weave_plotly.PanelPlotly:
        ...
        if not weave.types.union(
            ...
            return weave.legacy.panels.PanelString("Invalid value_fn")  # type: ignore
...
def distribution_panel_plot_render(
    ...
    unnested = weave.legacy.ops.unnest(input_node)
    bin_size = weave.legacy.ops.execute(config.bin_size)
    ...
    def bin_func(item):
        ...
        res = weave.legacy.ops.dict_(**group_items)
        return res
