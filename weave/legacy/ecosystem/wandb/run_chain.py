...
import weave
from weave.legacy import compile, op_def, ops_arrow
from weave.legacy.ecosystem.wandb import gql_artifact_dag
...
class RunChain:
    ...
    def _history_node(self):
        with op_def.no_refine():
            proj = weave.legacy.ops.project(self.entity_name, self.project_name)
            history_nodes = []
            ...
            history_node = weave.legacy.ops.List.concat(
...
def run_chain(run_path: str) -> RunChain:
    with op_def.no_refine():
        ...
        proj = weave.legacy.ops.project(entity, project)
        run = proj.run(run_id)
