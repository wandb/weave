    ...
    return weave.legacy.panels.Group(
        items={
            "x_fn": weave.legacy.panels.LabeledItem(
                label="x",
                item=weave.legacy.panels.FunctionEditor(
                    config=weave.legacy.panels.FunctionEditorConfig(config.x_fn)
                ),
            ),
            "y_fn": weave.legacy.panels.LabeledItem(
                label="y",
                item=weave.legacy.panels.FunctionEditor(
                    config=weave.legacy.panels.FunctionEditorConfig(config.y_fn)
                ),
            ),
            "color_fn": weave.legacy.panels.LabeledItem(
                label="color",
                item=weave.legacy.panels.FunctionEditor(
                    config=weave.legacy.panels.FunctionEditorConfig(config.color_fn)
                ),
            ),
        }
    )
