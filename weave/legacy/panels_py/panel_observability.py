...
def observability(
    ...
    now = weave.legacy.ops.datetime_now()

    dashboard = weave.legacy.panels.Group(
        layoutMode="grid",
    ...
    display_states = weave.legacy.ops.dict_(
        **{
            "running": "running",
            "failed": "failed",
            "crashed": "crashed",
    ...
    window_start = varbar.add(
        "window_start",
        weave.legacy.ops.from_number(now - seven_days_in_seconds),
        hidden=True,
    )
    window_end = varbar.add(
        "window_end",
        weave.legacy.ops.from_number(now),
        hidden=True,
    )
    ...
    window_data = source_data.filter(
        lambda row: weave.legacy.ops.Boolean.bool_and(
    ...
    colors_node = weave.legacy.ops.dict_(
        **{
    ...
    queued_time_data = varbar.add(
        ...
        filtered_window_data.filter(
            lambda row: weave.legacy.ops.Boolean.bool_or(
                row["state"] == "queued",
                row["state"] == "pending",
            ),
        ),
    ...
    is_start_stop_state = weave_internal.define_fn(
        ...
        lambda row: weave.legacy.ops.Boolean.bool_or(
            row["state"] == "running",
            weave.legacy.ops.Boolean.bool_or(
                row["state"] == "finished",
                weave.legacy.ops.Boolean.bool_or(
                    row["state"] == "crashed",
                    row["state"] == "failed",
    ...
    state_transitions_plot = panels.Plot(
        ...
        x=lambda row: row[timestamp_col_name].bin(
            weave.legacy.ops.timestamp_bins_nice(bin_range, num_buckets)
        ),
        x_title="Time",
        y=lambda row: row.count(),
        y_title="Count of transitions by state",
        label=lambda row: row["state"],
        tooltip=lambda row: weave.legacy.ops.dict_(
            **{
                "State": row["state"][0],
                "Count": row.count(),
                "Job (s)": weave.legacy.ops.join_to_str(row["job"].unique(), ","),
            }
        ),
        color_title="state",
    ...
    runs_grouped = weave.legacy.ops.List.groupby(
        queued_time_data, lambda row: row["trace_id"]
    )
    runs_mapped = weave.legacy.ops.List.map(
        runs_grouped,
        lambda row: weave.legacy.ops.dict_(
            **{
                ...
                "duration": weave.legacy.ops.Number.__truediv__(
                    weave.legacy.ops.Number.__mul__(
                        weave.legacy.ops.timedelta_total_seconds(
                            weave.legacy.ops.datetime_sub(
    ...
    queued_time_plot = panels.Plot(
        ...
        tooltip=lambda row: weave.legacy.ops.dict_(
            **{
                "Job (s)": weave.legacy.ops.join_to_str(row["job"].unique(), ","),
                "User (s)": weave.legacy.ops.join_to_str(
                    row["entity_name"].unique(), ","
                ),
                "Project (s)": weave.legacy.ops.join_to_str(
    ...
    start_stop_states = varbar.add(
        "start_stop_states",
        weave.legacy.ops.List.filter(filtered_window_data, is_start_stop_state),
        hidden=True,
    )
    ...
    latest_runs_plot = panels.Plot(
        ...
        tooltip=lambda row: weave.legacy.ops.dict_(
            **{
                ...
                "Run ID": row["run_id"][-1],
                "Status": row["state"][-1],
                "Duration (m)": weave.legacy.ops.Number.__truediv__(
                    weave.legacy.ops.Number.__mul__(
                        weave.legacy.ops.timedelta_total_seconds(
                            weave.legacy.ops.datetime_sub(
                                row[timestamp_col_name].max(),
    ...
    jobs_table.add_column(
        lambda row: weave.legacy.ops.run_ops.str_run_link(
    ...
    jobs_table.add_column(
        lambda row: weave.legacy.ops.cond(
            weave.legacy.ops.dict_(
                a=row["state"][-1] != "running",
                b=row["state"][-1] == "running",
            ),
            weave.legacy.ops.dict_(
                a=weave.legacy.ops.Number.__truediv__(
                    weave.legacy.ops.timedelta_total_seconds(
                        weave.legacy.ops.datetime_sub(
                            row[timestamp_col_name].max(),
                            row[timestamp_col_name].min(),
                ...
                b=weave.legacy.ops.Number.__truediv__(
                    weave.legacy.ops.timedelta_total_seconds(
                        weave.legacy.ops.datetime_sub(
                            weave.legacy.ops.from_number(
                                weave.legacy.ops.datetime_now()
                            ),
                            row[timestamp_col_name].min(),
                        )
                    ),
    ...
    jobs_table.add_column(
        lambda row: weave.legacy.ops.cond(
            ...
            weave.legacy.ops.dict_(
                ...
                b=weave.legacy.ops.make_const_node(types.NoneType(), None),
            ),
        ),
        "Avg. CPU %",
    )
    ...
    metric_plot_data = weave.legacy.ops.List.groupby(
