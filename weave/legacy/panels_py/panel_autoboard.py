...
def timeseries(
    ...
    if weave.types.optional(weave.types.Timestamp()).assign_type(x_axis_type):
        x_title = ""
        bin_fn = weave.legacy.ops.timestamp_bins_nice
    elif weave.types.optional(weave.types.Number()).assign_type(x_axis_type):
        x_title = x_axis_key
        bin_fn = weave.legacy.ops.numbers_bins_equal
    else:
...
def auto_panels(
    ...
    metrics = weave.legacy.panels.Group(
        layoutMode=weave.legacy.panels.GroupLayoutFlow(2, 3),
        items={"panel%s" % i: panel for i, panel in enumerate(metric_panels)},
    )
    categoricals = weave.legacy.panels.Group(
        layoutMode=weave.legacy.panels.GroupLayoutFlow(2, 3),
    ...
    control_items = [
        ...
        weave.legacy.panels.GroupPanel(
            lambda data: weave.legacy.ops.make_list(
                a=data[x_axis].min(), b=data[x_axis].max()
            ),
            id="data_range",
            hidden=True,
        ),
        weave.legacy.panels.GroupPanel(None, id="zoom_range", hidden=True),
        weave.legacy.panels.GroupPanel(
        ...
        weave.legacy.panels.GroupPanel(
            lambda data, zoom_range: weave.legacy.panels.DateRange(
...
class AutoBoard(weave.Panel):
    ...
    def render(self) -> weave.legacy.panels.Group:
