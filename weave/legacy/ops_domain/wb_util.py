...
from weave.legacy.ops_domain.run_history import history_op_common
from weave.legacy.wandb_interface import wandb_stream_table
from weave.legacy.runfiles_wandb import WandbRunFiles, WeaveWBRunFilesURI
...
def _process_run_dict_item(val, run_path: typing.Optional[RunPath] = None):
    if isinstance(val, dict) and "_type" in val:
        ...
        if val["_type"] == "image-file" and run_path is not None:
            from weave.legacy.ops_domain import ImageArtifactFileRef
        ...
        if val["_type"] == "audio-file" and run_path is not None:
            from weave.legacy.ops_domain import AudioArtifactFileRef
        ...
        if val["_type"] == "bokeh-file" and run_path is not None:
            from weave.legacy.ops_domain import BokehArtifactFileRef
        ...
        if val["_type"] == "video-file" and run_path is not None:
            from weave.legacy.ops_domain import VideoArtifactFileRef
        ...
        if val["_type"] == "object3D-file" and run_path is not None:
            from weave.legacy.ops_domain import Object3DArtifactFileRef
        ...
        if val["_type"] == "molecule-file" and run_path is not None:
            from weave.legacy.ops_domain import MoleculeArtifactFileRef
        ...
        if val["_type"] == "html-file" and run_path is not None:
            from weave.legacy.ops_domain import HtmlArtifactFileRef
        ...
        if val["_type"] == "wb_trace_tree":
            from weave.legacy.ops_domain.trace_tree import WBTraceTree
