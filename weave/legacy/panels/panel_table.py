...
from weave.legacy.panels.panel_query import Query
...
class ColumnDef(typing.TypedDict):
    columnName: str
    columnSelectFunction: weave.legacy.graph.Node
...
class Table(panel.Panel, codifiable_value_mixin.CodifiableValueMixin):
    ...
    def to_code(self) -> typing.Optional[str]:
        ...
        return f"""weave.legacy.panels.panel_table.Table({codify.object_to_code_no_format(self.input_node)}, {param_str})"""
...
def _get_pinned_node(self: typing.Union[Table, Query], data_or_rows_node: Node) -> Node:
    if self.config is None:
        return weave.legacy.ops.make_list()
...
def _get_rows_node(self: Table, apply_sort: bool = True) -> Node:
    ...
    if (
        ...
        data_node = weave.legacy.ops.List.filter(
            data_node,
    ...
    if self.config and self.config.tableState.groupBy:
        ...
        data_node = weave.legacy.ops.List.groupby(
            data_node,
    ...
    # Apply Selection
    data_node = weave.legacy.ops.List.map(
        data_node,
        lambda row, index: weave.legacy.ops.dict_(
            **{
                col_def["columnName"]: (
    ...
    if self.config and self.config.tableState.sort and apply_sort:
        ...
        data_node = weave.legacy.ops.List.sort(
            data_node,
            lambda row: weave.legacy.ops.make_list(
                **{
