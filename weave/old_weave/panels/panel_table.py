...
def _get_pinned_node(self: typing.Union[Table, Query], data_or_rows_node: Node) -> Node:
    if self.config is None:
        return weave.old_weave.ops.make_list()
...
def _get_rows_node(self: Table, apply_sort: bool = True) -> Node:
    ...
    if (
        ...
        data_node = weave.old_weave.ops.List.filter(
            data_node,
    ...
    if self.config and self.config.tableState.groupBy:
        ...
        data_node = weave.old_weave.ops.List.groupby(
            data_node,
    ...
    # Apply Selection
    data_node = weave.old_weave.ops.List.map(
        data_node,
        lambda row, index: weave.old_weave.ops.dict_(
            **{
                col_def["columnName"]: (
    ...
    if self.config and self.config.tableState.sort and apply_sort:
        ...
        data_node = weave.old_weave.ops.List.sort(
            data_node,
            lambda row: weave.old_weave.ops.make_list(
                **{
