...
def observability(
    ...
    now = weave.old_weave.ops.datetime_now()
    ...
    display_states = weave.old_weave.ops.dict_(
        **{
            "running": "running",
            "failed": "failed",
            "crashed": "crashed",
    ...
    source_data = input_node.map(
        lambda row: weave.old_weave.ops.dict_(
            **{
    ...
    window_start = varbar.add(
        "window_start",
        weave.old_weave.ops.from_number(now - seven_days_in_seconds),
        hidden=True,
    )
    window_end = varbar.add(
        "window_end",
        weave.old_weave.ops.from_number(now),
        hidden=True,
    )
    ...
    colors_node = weave.old_weave.ops.dict_(
        **{
    ...
    queued_time_data = varbar.add(
        ...
        filtered_window_data.filter(
            lambda row: weave.old_weave.ops.Boolean.bool_or(
                row["state"] == "queued",
                row["state"] == "pending",
            ),
        ),
    ...
    is_start_stop_state = weave_internal.define_fn(
        ...
        lambda row: weave.old_weave.ops.Boolean.bool_or(
            row["state"] == "running",
            weave.old_weave.ops.Boolean.bool_or(
                row["state"] == "finished",
                weave.old_weave.ops.Boolean.bool_or(
                    row["state"] == "crashed",
                    row["state"] == "failed",
    ...
    state_transitions_plot = panels.Plot(
        ...
        x=lambda row: row[timestamp_col_name].bin(
            weave.old_weave.ops.timestamp_bins_nice(bin_range, num_buckets)
        ),
        x_title="Time",
        y=lambda row: row.count(),
        y_title="Count of transitions by state",
        label=lambda row: row["state"],
        tooltip=lambda row: weave.old_weave.ops.dict_(
            **{
                "State": row["state"][0],
                "Count": row.count(),
                "Job (s)": weave.old_weave.ops.join_to_str(row["job"].unique(), ","),
            }
        ),
        color_title="state",
    ...
    runs_grouped = weave.old_weave.ops.List.groupby(
        queued_time_data, lambda row: row["trace_id"]
    )
    runs_mapped = weave.old_weave.ops.List.map(
        runs_grouped,
        lambda row: weave.old_weave.ops.dict_(
            **{
                ...
                "duration": weave.old_weave.ops.Number.__truediv__(
                    weave.old_weave.ops.Number.__mul__(
                        weave.old_weave.ops.timedelta_total_seconds(
                            weave.old_weave.ops.datetime_sub(
    ...
    runs_mapped_filtered = varbar.add(
        ...
        weave.old_weave.ops.List.filter(
            runs_mapped,
    ...
    queued_time_plot = panels.Plot(
        ...
        tooltip=lambda row: weave.old_weave.ops.dict_(
            **{
                "Job (s)": weave.old_weave.ops.join_to_str(row["job"].unique(), ","),
                "User (s)": weave.old_weave.ops.join_to_str(
                    row["entity_name"].unique(), ","
                ),
                "Project (s)": weave.old_weave.ops.join_to_str(
    ...
    start_stop_states = varbar.add(
        "start_stop_states",
        weave.old_weave.ops.List.filter(filtered_window_data, is_start_stop_state),
        hidden=True,
    )
    ...
    latest_runs_plot = panels.Plot(
        ...
        tooltip=lambda row: weave.old_weave.ops.dict_(
            **{
                ...
                "Run ID": row["run_id"][-1],
                "Status": row["state"][-1],
                "Duration (m)": weave.old_weave.ops.Number.__truediv__(
                    weave.old_weave.ops.Number.__mul__(
                        weave.old_weave.ops.timedelta_total_seconds(
                            weave.old_weave.ops.datetime_sub(
                                row[timestamp_col_name].max(),
    ...
    jobs_table.add_column(
        lambda row: weave.old_weave.ops.run_ops.str_run_link(
    ...
    jobs_table.add_column(
        lambda row: weave.old_weave.ops.cond(
            weave.old_weave.ops.dict_(
                a=row["state"][-1] != "running",
                b=row["state"][-1] == "running",
            ),
            weave.old_weave.ops.dict_(
                a=weave.old_weave.ops.Number.__truediv__(
                    weave.old_weave.ops.timedelta_total_seconds(
                        weave.old_weave.ops.datetime_sub(
                            row[timestamp_col_name].max(),
                            row[timestamp_col_name].min(),
                ...
                b=weave.old_weave.ops.Number.__truediv__(
                    weave.old_weave.ops.timedelta_total_seconds(
                        weave.old_weave.ops.datetime_sub(
                            weave.old_weave.ops.from_number(
                                weave.old_weave.ops.datetime_now()
                            ),
                            row[timestamp_col_name].min(),
                        )
                    ),
    ...
    jobs_table.add_column(
        lambda row: weave.old_weave.ops.cond(
            ...
            weave.old_weave.ops.dict_(
                ...
                b=weave.old_weave.ops.make_const_node(types.NoneType(), None),
            ),
        ),
        "Avg. CPU %",
    )
    ...
    metric_plot_data = weave.old_weave.ops.List.groupby(
