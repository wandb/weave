...
class RemoteHTTPTraceServer(tsi.TraceServerInterface):
    ...
    def op_read_v2(
        ...
        entity, project = req.project_id.split("/", 1)
        url = f"/v2/{entity}/{project}/ops/{req.object_id}/{req.digest}"
    ...
    def op_read_eager_v2(
        ...
        req = cast(tsi.OpReadV2Req, req)
        entity, project = req.project_id.split("/", 1)
        url = f"/v2/{entity}/{project}/ops/{req.object_id}/{req.digest}/eager"
        r = self._get_request_executor(url)
        return tsi.OpReadEagerV2Res.model_validate(r.json())
    ...
    def dataset_read_v2(
        ...
        entity, project = req.project_id.split("/", 1)
        url = f"/v2/{entity}/{project}/datasets/{req.object_id}/{req.digest}"
        r = self._get_request_executor(url)
    ...
    def dataset_read_eager_v2(
        ...
        req = cast(tsi.DatasetReadV2Req, req)
        entity, project = req.project_id.split("/", 1)
        url = f"/v2/{entity}/{project}/datasets/{req.object_id}/{req.digest}/eager"
        r = self._get_request_executor(url)
        return tsi.DatasetReadEagerV2Res.model_validate(r.json())
    ...
    def scorer_create_v2(
        ...
        entity, project = req.project_id.split("/", 1)
        url = f"/v2/{entity}/{project}/scorer"
    ...
    def scorer_read_v2(
        ...
        entity, project = req.project_id.split("/", 1)
        url = f"/v2/{entity}/{project}/scorer/{req.object_id}/{req.digest}"
        r = self._get_request_executor(url)
    ...
    def scorer_list_v2(
        ...
        for line in r.iter_lines():
            if line:
                yield tsi.ScorerItemMetadataV2.model_validate_json(line)
    ...
    def scorer_delete_v2(
        ...
        entity, project = req.project_id.split("/", 1)
        url = f"/v2/{entity}/{project}/scorer/{req.object_id}"
        # Build query params
        params = {}
        if req.digests:
            params["digests"] = req.digests
    ...
    def evaluation_create_v2(
        ...
        entity, project = req.project_id.split("/", 1)
        url = f"/v2/{entity}/{project}/evaluation"
    ...
    def evaluation_read_v2(
        ...
        entity, project = req.project_id.split("/", 1)
        url = f"/v2/{entity}/{project}/evaluation/{req.object_id}/{req.digest}"
        r = self._get_request_executor(url)
    ...
    def evaluation_list_v2(
        ...
        for line in r.iter_lines():
            if line:
                yield tsi.EvaluationItemMetadataV2.model_validate_json(line)
    ...
    def evaluation_delete_v2(
        ...
        entity, project = req.project_id.split("/", 1)
        url = f"/v2/{entity}/{project}/evaluation/{req.object_id}"
        # Build query params
        params = {}
        if req.digests:
            params["digests"] = req.digests
    ...
    def evaluation_run_start(
        ...
        if isinstance(req, dict):
            req = tsi.EvaluationRunStartReq.model_validate(req)
        req = cast(tsi.EvaluationRunStartReq, req)
        entity, project = req.project_id.split("/", 1)
        url = f"/v2/{entity}/{project}/evaluation_runs/start"
        r = self._post_request_executor(url, req.model_dump(mode="json"))
        return tsi.EvaluationRunStartRes.model_validate(r.json())
    ...
    def evaluation_run_log_prediction(
        ...
        if isinstance(req, dict):
            req = tsi.EvaluationRunLogPredictionReq.model_validate(req)
        req = cast(tsi.EvaluationRunLogPredictionReq, req)
        entity, project = req.project_id.split("/", 1)
        url = f"/v2/{entity}/{project}/evaluation_runs/log_prediction"
        r = self._post_request_executor(url, req.model_dump(mode="json"))
        return tsi.EvaluationRunLogPredictionRes.model_validate(r.json())

    def evaluation_run_log_score(
        self, req: Union[tsi.EvaluationRunLogScoreReq, dict[str, Any]]
    ) -> tsi.EvaluationRunLogScoreRes:
        if isinstance(req, dict):
            req = tsi.EvaluationRunLogScoreReq.model_validate(req)
        req = cast(tsi.EvaluationRunLogScoreReq, req)
        entity, project = req.project_id.split("/", 1)
        url = f"/v2/{entity}/{project}/evaluation_runs/log_score"
        r = self._post_request_executor(url, req.model_dump(mode="json"))
        return tsi.EvaluationRunLogScoreRes.model_validate(r.json())
    ...
    def evaluation_run_finish(
        ...
        r = self._post_request_executor(url, req.model_dump(mode="json"))
        return tsi.EvaluationRunFinishRes.model_validate(r.json())
