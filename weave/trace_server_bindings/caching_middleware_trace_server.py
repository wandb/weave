...
from typing import Any, Callable, TypedDict, TypeVar, Awaitable
...
class CachingMiddlewareTraceServer(tsi.TraceServerInterface):
    ...
    async def _with_cache_pydantic(
        self,
        func: Callable[[TReq], Awaitable[TRes]],
        req: TReq,
        res_type: type[TRes],
    ...
    async def obj_read(self, req: tsi.ObjReadReq) -> tsi.ObjReadRes:
        if not digest_is_cacheable(req.digest):
            return await self._next_trace_server.obj_read(req)
        return await self._with_cache_pydantic(
    ...
    async def obj_delete(self, req: tsi.ObjDeleteReq) -> tsi.ObjDeleteRes:
        ...
        if req.digests:
            ...
            self._safe_cache_delete_prefix(cache_key_prefix)
        return await self._next_trace_server.obj_delete(req)

    async def table_query(self, req: tsi.TableQueryReq) -> tsi.TableQueryRes:
        if not digest_is_cacheable(req.digest):
            return await self._next_trace_server.table_query(req)
        return await self._with_cache_pydantic(
    ...
    async def table_query_stream(
        ...
        # Not caching streaming queries for now
        async for row in self._next_trace_server.table_query_stream(req):
            yield row
    ...
    async def table_query_stats(
        ...
        if not digest_is_cacheable(req.digest):
            return await self._next_trace_server.table_query_stats(req)
        return await self._with_cache_pydantic(
    ...
    async def table_query_stats_batch(
        ...
        if any(not digest_is_cacheable(digest) for digest in req.digests or []):
            return await self._next_trace_server.table_query_stats_batch(req)
        return await self._with_cache_pydantic(
    ...
    async def refs_read_batch(self, req: tsi.RefsReadBatchReq) -> tsi.RefsReadBatchRes:
        ...
        if needed_refs:
            ...
            needed_results = await self._next_trace_server.refs_read_batch(new_req)
    ...
    async def table_update(self, req: tsi.TableUpdateReq) -> tsi.TableUpdateRes:
        return await self._next_trace_server.table_update(req)

    async def feedback_create(
        self, req: tsi.FeedbackCreateReq
    ) -> tsi.FeedbackCreateRes:
        return await self._next_trace_server.feedback_create(req)

    async def feedback_query(self, req: tsi.FeedbackQueryReq) -> tsi.FeedbackQueryRes:
        return await self._next_trace_server.feedback_query(req)

    async def feedback_purge(self, req: tsi.FeedbackPurgeReq) -> tsi.FeedbackPurgeRes:
        return await self._next_trace_server.feedback_purge(req)

    async def feedback_replace(
        self, req: tsi.FeedbackReplaceReq
    ) -> tsi.FeedbackReplaceRes:
        return await self._next_trace_server.feedback_replace(req)

    # Action API
    async def actions_execute_batch(
        self, req: tsi.ActionsExecuteBatchReq
    ) -> tsi.ActionsExecuteBatchRes:
        return await self._next_trace_server.actions_execute_batch(req)

    # Execute LLM API
    async def completions_create(
        self, req: tsi.CompletionsCreateReq
    ) -> tsi.CompletionsCreateRes:
        return await self._next_trace_server.completions_create(req)

    async def project_stats(self, req: tsi.ProjectStatsReq) -> tsi.ProjectStatsRes:
        return await self._next_trace_server.project_stats(req)
