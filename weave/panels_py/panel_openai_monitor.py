...
def cost(row: dispatch.RuntimeOutputNode) -> dispatch.RuntimeOutputNode:
    return row["metrics.total_tokens"] * 0.0015e-3
...
def board(
    ...
    overview_tab.add(
        ...
        panel_autoboard.timeseries(
            ...
            bin_domain_node=bin_range,
            x_axis_key="timestamp",
            y_expr=lambda row: row["metrics.latency_s"].avg(),
    ...
    requests_table.add_column(lambda row: row["metrics.prompt_tokens"], "Prompt Tokens")
    requests_table.add_column(
        lambda row: row["metrics.completion_tokens"], "Completion Tokens"
    )
    requests_table.add_column(lambda row: row["metrics.total_tokens"], "Total Tokens")
    requests_table.add_column(lambda row: row["metrics.latency_s"], "Latency")
