...
def test_timestamp_bins_fixed():
    bin_fn = weave.ops.timestamp_bins_fixed(60)
...
def test_timestamp_bins_nice():
    ...
    bin_fn = weave.ops.timestamp_bins_nice([start_ts, stop_ts], 100)
...
def test_timestamp_bin_vector():
    ...
    ts_node = weave.ops.to_weave_arrow([ts, ts + timedelta(seconds=90)])
