...
def test_stream_logging(user_by_api_key_in_env):
    ...
    hist_node = weave.ops.project(user_by_api_key_in_env.username, "stream-tables").run("test_table").history3()

    exp_type = weave_types.TypedDict({"a": weave.types.optional(weave_types.List(weave_types.Int()))})
    nested_type = hist_node.type.value.object_type.property_types["nested"]
    assert exp_type.assign_type(nested_type)
    assert weave.use(hist_node["hello"]).to_pylist_tagged() == [f"world_{i}" for i in range(10)]
    assert weave.use(hist_node["index"]).to_pylist_tagged() == [i for i in range(10)]
    assert weave.use(hist_node["nested"]).to_pylist_tagged() == [{"a": [i]} for i in range(10)]
...
def test_stream_logging_image(user_by_api_key_in_env):
    ...
    hist_node = weave.ops.project(user_by_api_key_in_env.username, "stream-tables").run("test_table-8").history2()
...
def test_multi_writers_sequential(user_by_api_key_in_env):
    ...
    def do_asserts():
        hist_node = weave.ops.project(user_by_api_key_in_env.username, "stream-tables").run("test_table").history2()
...
def test_multi_writers_parallel(user_by_api_key_in_env):
    ...
    hist_node = weave.ops.project(entity_name, "stream-tables").run(table_name).history2()
