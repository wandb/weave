...
# @pytest.fixture(scope="function")
# # @pytest.fixture()
# def patched_wandb_init(user, base_url, test_settings, request):
...
#         for key in ("user", "test_settings", "request", "base_url"):
#             kwargs.pop(key, None)
#         # merge settings from request with test_settings
#         request_settings = kwargs.pop("settings", dict()) or dict()
#         kwargs["name"] = kwargs.pop("name", request.node.name) or request.node.name
#         # request_settings = request_settings or {}
#         # request_settings['base_url'] = base_url
