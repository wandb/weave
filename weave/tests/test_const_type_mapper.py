...
def test_const_type_mapper():
    node = weave_internal.make_const_node(_TestType.WeaveType(weave.types.List(weave.types.Number())), None)
    fn_node = weave_internal.define_fn({"row": weave.types.Number()}, lambda row: row + 1)
