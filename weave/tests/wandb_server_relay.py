...
if TYPE_CHECKING:
    ...
    class Resolver(TypedDict):
        name: ResolverName
        resolver: Callable[[Any, Any], Optional[Dict[str, Any]]]
...
class Context:
    ...
    def __init__(self) -> None:
        ...
        self._entries: defaultdict = defaultdict(dict)
...
class QueryResolver:
    ...
    def resolve_upload_files(
        ...
        if query:
            ...
            kwargs_split = kwargs.get("path")
            assert isinstance(kwargs_split, str)
            name = kwargs_split.split("/")[2]
            files = {
                file_name: [
                    {
                        "content": [
    ...
    def resolve(
        ...
        **kwargs: Any,
        ...
        for resolver in self.resolvers:
            resolver_resolver = resolver.get("resolver")
            if resolver_resolver is not None:
                result = resolver_resolver(request_data, response_data, **kwargs)
...
class RelayServer:
    def __init__(
        ...
        self.app.add_url_rule(
            rule="/graphql",
            endpoint="graphql",
            view_func=self.graphql,  # type: ignore
            methods=["POST"],
        )
        self.app.add_url_rule(
            rule="/files/<path:path>",
            endpoint="files",
            view_func=self.file_stream,  # type: ignore
            methods=["POST"],
        )
        self.app.add_url_rule(
            rule="/storage",
            endpoint="storage",
            view_func=self.storage,  # type: ignore
            methods=["PUT", "GET"],
        )
        self.app.add_url_rule(
            rule="/storage/<path:path>",
            endpoint="storage_file",
            view_func=self.storage_file,  # type: ignore
            methods=["PUT", "GET"],
        )
        if control:
            self.app.add_url_rule(
                rule="/_control",
                endpoint="_control",
                view_func=self.control,  # type: ignore
                methods=["POST"],
            )
        ...
        **kwargs: Any,
        ...
        if isinstance(request_data, dict):
            snooped_context = self.resolver.resolve(
                request_data, response_data, **kwargs
            )
...
BASE_URL = f"http://localhost:{LOCAL_BASE_PORT}"
