...
def generate_schemas() -> None:
    """
    Generate JSON schemas for all registered base objects in BUILTIN_OBJECT_CLASS_REGISTRY.
    ...
    CompositeModel = create_model(
        "CompositeBaseObject",
        **{name: (cls, ...) for name, cls in BUILTIN_OBJECT_CLASS_REGISTRY.items()},
    )
    ...
    print(f"Generated schema for {len(BUILTIN_OBJECT_CLASS_REGISTRY)} objects")
