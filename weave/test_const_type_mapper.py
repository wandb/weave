...
@weave.type()
class TestType(typing.Generic[MetricRows]):
    metrics: MetricRows
...
def test_const_type_mapper():
    node = weave_internal.make_const_node(
        TestType.WeaveType(weave.types.List(weave.types.Number())), None
    )
    fn_node = weave.define_fn({"row": weave.types.Number()}, lambda row: row + 1)
