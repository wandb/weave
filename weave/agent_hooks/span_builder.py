...
class SpanBuilder:
    ...
    def __init__(
        ...
        # Track image digests already uploaded to avoid re-attaching the same
        # image on every turn (Claude Code's transcript replays full history).
        self._uploaded_image_digests: set[str] = set()
    ...
    def _open_turn(self, conv: _ConvState, ev: AgentHookEvent) -> _TurnState:
        ...
        workspace = conv.workspace or (
            ev.workspace_roots[0] if ev.workspace_roots else ""
        )
    ...
    def _close_turn(
        self, conv: _ConvState, stop_status: str = "", loop_count: int = 0
    ) -> None:
    ...
    def _upload_attachment(self, attachment: dict) -> dict | None:
        ...
        try:
            ...
            resp = requests.post(
                url,
                files={
                    "file": (p.name, io.BytesIO(content), "application/octet-stream")
                },
        ...
        for raw_line in reversed(lines):
            ...
            for block in content:
                ...
                try:
                    ...
                    continue
                # Skip images already uploaded in a previous turn
                digest = hashlib.sha256(image_bytes).hexdigest()[:40]
                if digest in self._uploaded_image_digests:
                    continue
                ...
                if ref:
                    refs.append(ref)
                    self._uploaded_image_digests.add(digest)
                idx += 1
    ...
    def _on_user_prompt(self, ev: AgentHookEvent) -> None:
        ...
        if not conv.conversation_name and ev.prompt_text and ev.source != "cursor":
    ...
    def _on_tool_use_start(self, ev: AgentHookEvent) -> None:
        ...
        span, ctx = self._start_span(
            f"execute_tool {ev.tool_name}", turn.agent_ctx, attrs
        )
    ...
    def _on_tool_use_end(self, ev: AgentHookEvent) -> None:
        ...
        if ev.event_kind == "tool_use_failed":
            ...
            span.set_status(
                StatusCode.ERROR, ev.tool_error or ev.failure_type or "tool failed"
            )
        span.end()
    ...
    def _on_mcp_call(self, ev: AgentHookEvent) -> None:
        ...
        if turn is None:
            return
        tool_label = (
            f"{ev.mcp_server}/{ev.tool_name}" if ev.mcp_server else ev.tool_name
        )
        span, _ = self._start_span(
            ...
            turn.agent_ctx,
            {
                ...
                "gen_ai.tool.call.arguments": json.dumps(ev.tool_input)
                if ev.tool_input
                else "",
                "gen_ai.tool.call.result": ev.tool_output[:_TOOL_OUTPUT_TRUNCATE],
    ...
    def _on_subagent_stop(self, ev: AgentHookEvent) -> None:
        ...
        if ev.subagent_summary:
            span.set_attribute(
                "gen_ai.output.messages",
                json.dumps([{"role": "assistant", "content": ev.subagent_summary}]),
            )
        span.set_attribute("weave.subagent.status", ev.subagent_status)
        span.set_attribute(
            "weave.subagent.tool_call_count", ev.subagent_tool_call_count
        )
        if ev.subagent_message_count:
            span.set_attribute(
                "weave.subagent.message_count", ev.subagent_message_count
            )
        ...
        if ev.agent_transcript_path:
            span.set_attribute(
                "weave.subagent.transcript_path", ev.agent_transcript_path
            )
    ...
    def _on_stop_failure(self, ev: AgentHookEvent) -> None:
        ...
        if conv.current_turn and conv.current_turn.agent_span:
            ...
            if ev.stop_error_details:
                span.set_attribute(
                    "weave.turn.stop_error_details", ev.stop_error_details
                )
            from opentelemetry.trace import StatusCode
