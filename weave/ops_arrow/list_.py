...
class ArrowTableGroupByType(types.Type):
    ...
    def save_instance(self, obj, artifact, name):
        ...
        table = rewrite_weavelist_refs(
            obj._table, obj.object_type.object_type, obj._artifact
        )
        d = {
            "_table": table,
            "_groups": obj._groups,
...
class ArrowTableGroupBy:
    object_type: ArrowTableGroupResultType

    def __init__(self, _table, _groups, _group_keys, object_type, artifact):
        self._table = _table
    ...
    @op(
        output_type=lambda input_types: ArrowWeaveListType(
            input_types["self"].object_type._key
        )
    )
    def groupkey(self):
        return ArrowWeaveList(
            self._groups.column("group_key").combine_chunks(),
            self.object_type._key,
            self._artifact,
        )
    ...
    def map(self, map_fn):
        # TODO: should this actually do a `_apply_fn_node`?
...
class ArrowWeaveList(typing.Generic[ArrowWeaveListObjectTypeVar]):
    ...
    def groupby(self, group_by_fn):
        ...
        return ArrowTableGroupBy(
            ...
            agged,
            original_col_names,
            ArrowTableGroupResultType(self.object_type, group_table_object_type),
            self._artifact,
        )
...
def _apply_fn_node(
    awl: typing.Union[ArrowWeaveList, ArrowTableGroupBy], fn: graph.OutputNode
) -> ArrowWeaveList:
    ...
    if isinstance(awl, ArrowWeaveList):
        ...
        row_type = ArrowTableGroupByType(awl.object_type)
    else:
        raise errors.WeaveInternalError("Unexpected type: %s" % type(awl))
            ...
            if with_respect_to is None or any(
                ...
                if isinstance(node.type, ArrowTableGroupResultType):
                    return graph.VarNode(
                        ArrowTableGroupByType(node.type),
                        node.name,
                    )
