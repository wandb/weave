...
def resolve_trace_server_url() -> str | None:
    ...
    try:
        ...
        client = get_weave_client()
        if client and hasattr(client, "server") and hasattr(client.server, "_server_url"):
...
def upload_content(content: bytes, project_id: str) -> str:
    ...
    try:
        resp = requests.post(
            url,
            files={"file": ("content", io.BytesIO(content), "application/octet-stream")},
