...
class WeaveOTelProcessor(TracingProcessor):  # type: ignore[misc]
    ...
    def on_span_start(self, span: Span) -> None:
        ...
        if isinstance(data, AgentSpanData):
            ...
            self._start_generic_span(span, "transcription", getattr(data, "name", "transcription"))
    ...
    def _start_generic_span(
        self, span: Span, operation: str, name: str
    ) -> None:
        otel_span = self._tracer.start_span(
            name=name,
            kind=SpanKind.INTERNAL,
    ...
    def _capture_response_media(
        self, otel_span: trace.Span, response: Any
    ) -> None:
        ...
        try:
            ...
            try:
                for i, img_bytes in enumerate(images):
                    key = f"generated_image_{i}" if len(images) > 1 else "generated_image"
                    log_content(
