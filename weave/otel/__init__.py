...
def __getattr__(name: str) -> Any:
    ...
    if name == "instrument_openai_agents":
        from weave.otel.instrumentors.openai_agents import instrument as _inst_oai

        return _inst_oai
    if name == "instrument_google_adk":
        from weave.otel.instrumentors.google_adk import instrument as _inst_adk

        return _inst_adk
    if name == "instrument_claude_agent_sdk":
        from weave.otel.instrumentors.claude_agent_sdk import instrument as _inst_cas
...
def _append_to_span_attr(span: Any, attr_name: str, entry: dict) -> None:
    existing_raw = (
        span.attributes.get(attr_name) if hasattr(span, "attributes") else None
    )
    existing: list = []
    if existing_raw:
        try:
