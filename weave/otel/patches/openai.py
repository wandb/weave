...
def _capture_speech_output(response: Any) -> None:
    ...
    try:
        ...
        if content and isinstance(content, bytes):
            ...
            log_content(content, key="tts_audio", media_type="audio/mpeg", role="output")
    except Exception:
...
def _capture_images_response(response: Any) -> None:
    ...
    try:
        ...
        for i, item in enumerate(data):
            ...
            if url and not b64:
                from weave.otel import _get_active_span, _append_to_span_attr, _CONTENT_REFS_ATTR

                span = _get_active_span()
                if span:
                    _append_to_span_attr(span, _CONTENT_REFS_ATTR, {
                        "url": url,
...
def patch() -> None:
    ...
    global _original_speech_create, _original_images_generate
    ...
    if hasattr(openai, "resources") and hasattr(openai.resources, "audio"):
        ...
        if _original_speech_create is None:
            ...
            @functools.wraps(_original_speech_create)
            def patched_speech_create(self, *args, **kwargs):
                response = _original_speech_create(self, *args, **kwargs)
    ...
    if hasattr(openai, "resources") and hasattr(openai.resources, "images"):
        ...
        if _original_images_generate is None:
            ...
            @functools.wraps(_original_images_generate)
            def patched_images_generate(self, *args, **kwargs):
                if "response_format" not in kwargs:
                    kwargs["response_format"] = "b64_json"
                response = _original_images_generate(self, *args, **kwargs)
