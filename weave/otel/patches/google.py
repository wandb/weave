            ...
            for part in parts:
                ...
                if inline_data:
                    ...
                    if data_bytes and isinstance(data_bytes, bytes):
                        ...
                        log_content(data_bytes, key=key, media_type=mime_type, role="output")
                        idx += 1
    except Exception:
...
def patch() -> None:
    ...
    model_cls = getattr(models, "Models", None) or getattr(models, "GenerativeModel", None)
    ...
    if hasattr(model_cls, "generate_content_async") and _original_generate_content_async is None:
        _original_generate_content_async = model_cls.generate_content_async

        @functools.wraps(_original_generate_content_async)
        async def patched_generate_content_async(self, *args, **kwargs):
            response = await _original_generate_content_async(self, *args, **kwargs)
            _capture_inline_data(response)
            return response
