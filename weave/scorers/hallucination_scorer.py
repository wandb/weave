...
from weave.scorers.utils import stringify, check_score_param_type, MODEL_PATHS, ensure_hf_imports, load_hf_model_weights, ScorerResult
...
class HallucinationFreeScorer(LLMScorer):
    ...
    Note: This scorer's `score` method expects the `output` parameter to be a string and is what is being scored for hallucinations.
    ...
    @weave.op
    async def score(self, query: str, context: str | list[str], output: str) -> HallucinationResponse:
        """
        Analyzes the output to detect hallucinations based on the given context.

        Args:
            query: str, The query passed to the model, against which to analyze the output.
            context: str, The context passed to the model, against which to analyze the output.
            output: str, The output generated by the model, against which to analyze for hallucinations.
        """
