...
class AsyncCompletions:
    ...
    async def create(
        ...
        *,
    ...
    ) -> ChatCompletion | AsyncChatCompletionChunkStream:
        """
        ...
        return (
            await self._create_op(**kwargs)
            if track_llm_call
            else await self._create_non_op(**kwargs)
        )

    @op(name=COMPLETIONS_CREATE_OP_NAME, postprocess_inputs=postprocess_inputs)
    async def _create_op(self, **kwargs: Any) -> ChatCompletion | AsyncChatCompletionChunkStream:
        return await self._create_non_op(**kwargs)

    async def _create_non_op(
        self,
        **kwargs: Any,
    ) -> ChatCompletion | AsyncChatCompletionChunkStream:
            ...
            async with session.post(
                ...
                if response.status == 401:
                    raise aiohttp.ClientResponseError(
                        ...
                        message=f"{response.reason} - please make sure inference is enabled for entity {self._client.entity}",
                ...
                if request_stream:
                    return AsyncChatCompletionChunkStream(response)
