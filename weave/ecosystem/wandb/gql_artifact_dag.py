...
def get_run_checkpoint_chain(entity: str, project: str, leaf_checkpoint_name: str):
    ...
    target_artifact_id = [a["artifactNodeID"] for a in dag["artifacts"] if f"{a['artifactSequenceName']}:v{a['versionIndex']}" == leaf_checkpoint_name][0]
    ...
    while True:
        ...
        next_edges = [e for e in dag["edges"] if e["artifactNodeID"] == current_artifact_id and e["direction"] == "TowardArtifact"]
        ...
        sequence.append((runs_dict[current_run_id], artifacts_dict[current_artifact_id]))

        # Find the next artifact(s) where the run is the source and the direction is 'AwayFromArtifact'.
        next_edges = [e for e in dag["edges"] if e["runNodeID"] == current_run_id and e["direction"] == "AwayFromArtifact"]

        # Find the checkpoint artifact among the next artifacts.
        checkpoint_artifacts = [e for e in next_edges if artifacts_dict[e["artifactNodeID"]]["artifactTypeName"] == "checkpoint"]
