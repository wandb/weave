...
from weave.integrations.openai_realtime.models import create_message_from_dict
...
def cmd_replay_jsonl(args: argparse.Namespace) -> int:
    ...
    for line in in_path.read_text().splitlines():
        ...
        try:
            ...
            raise ValueError(
                f"create_message_from_dict for value: {rec} failed with error - {e}\n "
            ) from e
...
def build_parser() -> argparse.ArgumentParser:
    ...
    rp.add_argument(
        ...
        choices=["pcm", "wav"],
        default="pcm",
        help="Audio file format for dir export",
    )
    rp.set_defaults(func=cmd_replay_jsonl)

    rtp = sub.add_parser(
        "replay-turns", help="Replay a JSONL event file and export conversation turns"
    )
