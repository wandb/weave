...
def vertexai_postprocess_inputs(inputs: dict[str, Any]) -> dict[str, Any]:
    if "self" in inputs:
        model_name = (
            inputs["self"]._model_name if hasattr(inputs["self"], "_model_name") else inputs["self"]._model._model_name
        )
...
def vertexai_accumulator(acc: GenerationResponse | None, value: GenerationResponse) -> GenerationResponse:
    ...
    for i, value_candidate in enumerate(value.candidates):
        ...
        content = gapic_content_types.Content(role=value_candidate.content.role, parts=parts)
...
def vertexai_on_finish(call: Call, output: Any, exception: BaseException | None) -> None:
...
def get_vertexai_patcher(
    ...
    send_message_settings = base.model_copy(update={"name": base.name or "vertexai.ChatSession.send_message"})
