...
def save(obj: datetime.datetime, artifact: MemTraceFilesArtifact, name: str) -> None:
    ...
    with artifact.new_file(f"{name}.json") as f:
        json.dump({"isoformat": obj.isoformat()}, f)
...
def load(artifact: MemTraceFilesArtifact, name: str) -> DatetimeWithArtifact:
    ...
    # Convert to our custom class that can hold the artifact
    result = DatetimeWithArtifact.fromtimestamp(dt.timestamp(), dt.tzinfo)
    result.art = artifact
    return result
