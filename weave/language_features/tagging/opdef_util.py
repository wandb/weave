...
def should_tag_op_def_outputs(op_def: "OpDef.OpDef") -> bool:
    ...
    return (
        ...
        and (
            (isinstance(named_args[0].type, types.ObjectType) and named_args[0].type.name.startswith("_Test"))
...
def should_flow_tags(op_def: "OpDef.OpDef") -> bool:
    named_args = op_def.input_type.named_args()
    return not op_def_consumes_tags(op_def) and len(named_args) > 0 and not isinstance(named_args[0].type, types.Function) and not op_def.name.endswith("dropTags")


# Helper function to determine if the op_def consumes a tag
def op_def_consumes_tags(op_def: "OpDef.OpDef") -> bool:
    named_args = op_def.input_type.named_args()
    return len(named_args) > 0 and tagged_value_type.TaggedValueType(types.TypedDict({}), types.Any()).assign_type(named_args[0].type)


# Helper function to get the first argument of the op_def
def get_first_arg(op_def: "OpDef.OpDef", args: list[typing.Any], kwargs: dict[str, typing.Any]) -> typing.Tuple[str, typing.Any]:
