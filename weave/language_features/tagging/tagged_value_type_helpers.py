...
def push_down_tags_from_container_type_to_element_type(
    ...
    if not types.List().assign_type(container_type):
        raise ValueError(f"push_down_tags: expected container_type to be a list, got {container_type}")
    ...
    new_inner_type = tagged_value_type.TaggedValueType(new_tag_type, container_value_type)
...
def is_tagged_value_type(t: types.Type) -> bool:
    return isinstance(t, tagged_value_type.TaggedValueType) or isinstance(t, types.Const) and isinstance(t.val_type, tagged_value_type.TaggedValueType)
