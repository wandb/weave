...
Architecture::
    ...
    WALWriter ──► disk ──► WALSender ──► handlers (server calls)
                              │
                         background thread
                         polls on interval
...
File safety — two layers of protection:
    1. **Same-process** (``active_paths``): The sender never removes a
       file whose path appears in the ``active_paths`` set.  The caller
       provides a callable so the sender has explicit knowledge of which
       files are in use by the local writer.
    2. **Cross-process** (``flock``): The writer holds an exclusive
       advisory lock (``flock``) on each file it has open.  Before
       deleting, the sender probes the lock — if another process holds
       it, deletion is skipped.  When a process crashes, the OS releases
       all its locks automatically, so orphaned files become deletable.
"""
...
class WALSender:
    """Background thread that reads WAL records and dispatches them to handlers.
    ...
    File cleanup:
        ...
        3. No other process holds an exclusive ``flock`` on the file
           (cross-process protection).

        When the caller closes the writer before calling :meth:`stop`,
        the writer's path leaves the active set *and* the flock is
        released, so the final drain cleans it up automatically.
    ...
    def start(self) -> None:
        ...
        if self._thread is not None and self._thread.is_alive():
            raise RuntimeError("WALSender is already running")
    ...
    def drain_once(self) -> int:
        ...
        removed unless their path is in the active set.
    ...
    def _drain_unlocked(self) -> int:
        ...
        for path in self._mgr.list_files():
            ...
            try:
                ...
                if path in active:
                    continue
                # Layer 2: cross-process check via flock probe.
...
def _is_file_locked(path: str) -> bool:
    """Check if a WAL file is locked by an active writer (cross-process).

    Attempts to acquire an exclusive ``flock`` on the file.  If another
    process holds the lock, the attempt fails immediately (``LOCK_NB``)
    and we know a writer is alive.

    Returns ``False`` if ``flock`` is not available (e.g., Windows) — the
    caller falls back to ``active_paths`` for same-process protection.
    """
    if fcntl is None:
        return False
    try:
    ...
    try:
        ...
        # Lock acquired — no writer has it.  Release immediately.
        fcntl.flock(fd, fcntl.LOCK_UN)
        return False
    except OSError:
        # EWOULDBLOCK — another process holds an exclusive lock.
        return True
    finally:
        os.close(fd)
