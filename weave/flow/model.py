...
class Model(Object):
    ...
    def get_infer_method(self) -> Callable:
        for infer_method_names in ("predict", "infer", "forward"):
            ...
            if infer_method:
                return infer_method
        raise ValueError(f"Model {self} does not have a predict, infer, or forward method.")
...
def get_infer_method(model: Model) -> Callable:
    for infer_method_names in ("predict", "infer", "forward"):
        ...
        if infer_method:
            return infer_method
    raise ValueError(f"Model {model} does not have a predict, infer, or forward method.")
