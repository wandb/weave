...
from weave.trace.weave_init import get_viewer_id
...
class SavedView:
    ...
    def save(self) -> None:
        # Version creator is current user
        creator_user_id = get_viewer_id()
        if not creator_user_id:
            raise ValueError("No viewer ID found")
        self.base.creatorUserId = creator_user_id
        name = self.base.name
        if name is None:
            formatted_now = (
                datetime.now()
