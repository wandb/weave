...
def log_generation(
    ...
    if geneartor_config is not None:
        # Safetey check:
        if "self" in inputs:
            raise ValueError("inputs['self'] and generator_config cannot both be specified")
...
def _resolve_timestamps(
    ...
    if started_at is not None and latency_ms is not None and ended_at is not None:
        logger.warn("Should not specify all 3 of started_at, latency_ms, and ended_at, ignoring latency")
        return {
