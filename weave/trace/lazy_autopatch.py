...
INTEGRATION_PATCHERS: Dict[str, Callable[[IntegrationSettings], Any]] = {
    ...
    "mistral": lambda settings: _lazy_import(
    ...
    "instructor": lambda settings: _lazy_import(
        "weave.integrations.instructor.instructor_sdk", "get_instructor_patcher", settings
    ),
    ...
    "google.generativeai": lambda settings: _lazy_import(
        "weave.integrations.google_genai.google_genai_sdk", 
        "get_google_genai_patcher", settings
    ),
    ...
    "langchain": lambda settings: _lazy_import(
        "weave.integrations.langchain.langchain", "langchain_patcher", None, return_directly=True
    ),
    "llama_index": lambda settings: _lazy_import(
        "weave.integrations.llamaindex.llamaindex", "llamaindex_patcher", None, return_directly=True
    ),
    "mcp": lambda settings: [
    ...
    "swarm": lambda settings: _lazy_import(
        "weave.integrations.openai_agents.openai_agents", "get_openai_agents_patcher", settings
    ),
}


def _lazy_import(module_path: str, factory_name: str, settings: Any, return_directly: bool = False):
...
class WeaveImportHook:
    ...
    def find_module(self, fullname: str, path: Optional[str] = None) -> Optional['WeaveImportHook']:
    ...
    def _patch_module(self, module_name: str) -> None:
        ...
        try:
            ...
            settings_map = {
                ...
                "google.generativeai": self.settings.google_genai_sdk,
...
def _patch_already_imported_module(module_name: str, settings: AutopatchSettings) -> None:
    ...
    settings_map = {
        ...
        "google.generativeai": settings.google_genai_sdk,
        ...
        try:
            ...
            if patcher_factory:
                ...
                if isinstance(patchers, list):
                    ...
                    patchers.attempt_patch()
                logger.debug(f"Applied patches to already imported module {module_name}")
        except Exception as e:
            logger.warning(f"Failed to patch already imported module {module_name}: {e}")


def teardown_lazy_autopatch() -> None:
    """Remove import hooks and reset patched modules."""
    # Remove our import hook from sys.meta_path
    sys.meta_path = [hook for hook in sys.meta_path if not isinstance(hook, WeaveImportHook)]