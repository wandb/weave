"""Code capture module for saving and loading function source code.

This module provides functionality to capture source code and dependencies of functions
decorated with @weave.op, enabling them to be saved, serialized, and reloaded later.
"""
...
def get_source_or_fallback(fn: Callable, *, op=None, warnings: list[str]) -> str:
    """Get the source code of a function, or return a fallback if unavailable.
    
    Args:
        fn: The function to get source for
        op: The Op wrapper if fn is an op
        warnings: List to append warnings to
        
    Returns:
        Source code string or fallback code
    """
...
def save_op_code(obj: Any, artifact: MemTraceFilesArtifact, name: str) -> None:
    """Save the code for an Op to an artifact."""
    from weave.trace.op import Op, as_op
    
    op = as_op(obj) if not isinstance(obj, Op) else obj
    result = get_code_deps_safe(op.resolve_fn, artifact)
    import_code = result["import_code"]
    code = result["code"]
    warnings = result["warnings"]
    if warnings:
        message = f"Warning: Incomplete serialization for op {op}. This op may not be reloadable"
        for warning in warnings:
            message += "\n  " + warning

    op_function_code = get_source_or_fallback(op.resolve_fn, op=op, warnings=warnings)
    if not op._code_capture_enabled:
        import_code = []
        code = []
...
def load_op_code(
    ...
    """Load an Op from saved code in an artifact."""
...
def get_captured_code(op: Any) -> str:
