...
from weave.trace.context import call, urls, util, weave_client, weave_init
from weave.trace.context import weave_client as weave_client_context
from weave.trace.context.call import get_current_call, require_current_call
...
def attributes(attributes: dict[str, Any]) -> Iterator:
    ...
    ```
    """
    cur_attributes = {**call.call_attributes.get()}
    cur_attributes.update(attributes)

    token = call.call_attributes.set(cur_attributes)
    try:
        yield
    finally:
        call.call_attributes.reset(token)
...
def serve(
    ...
    trace_attrs = call.call_attributes.get()
