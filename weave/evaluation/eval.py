...
class Evaluation(Object):
    ...
    async def predict_and_score(self, model: Union[Op, Model], example: dict) -> dict:
        ...
        if scorers := self.scorers:
            ...
            # Process results and build scores dict and scorer_call_ids dict
        ...
        return {
            ...
            "scorer_call_ids": scorer_call_ids,
        }
    ...
    async def summarize(self, eval_table: EvaluationResults) -> dict:
        ...
        for name, vals in cols.items():
            ...
            elif name == "scorer_call_ids":
                # Collect all scorer call IDs into a dict
                scorer_call_ids_summary = {}
                if scorers := self.scorers:
                    for scorer in scorers:
                        scorer_attributes = get_scorer_attributes(scorer)
                        scorer_name = scorer_attributes.scorer_name
                        scorer_call_ids_summary[scorer_name] = [
    ...
    async def get_eval_results(self, model: Union[Op, Model]) -> EvaluationResults:
        ...
        async def eval_example(example: dict) -> dict:
            try:
                ...
                traceback.print_exc()
                return {self._output_key: None, "scores": {}, "scorer_call_ids": {}}
            return eval_row
        ...
        async for index, _example, eval_row in util.async_foreach(
            ...
            if eval_row is None:
                eval_row = {self._output_key: None, "scores": {}, "scorer_call_ids": {}}
            else:
                eval_row["scores"] = eval_row.get("scores", {})
                eval_row["scorer_call_ids"] = eval_row.get("scorer_call_ids", {})
            if self.scorers:
                for scorer in self.scorers:
                    ...
                    if scorer_name not in eval_row["scorer_call_ids"]:
                        eval_row["scorer_call_ids"][scorer_name] = None
