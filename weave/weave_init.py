...
class InitializedClient:
    def __init__(self, client: weave_client.WeaveClient):
        ...
        self.serverless_io_service_token = context_state._serverless_io_service.set(True)
...
def init_weave(project_name: str, ensure_project_exists: bool = True) -> InitializedClient:
    global _current_inited_client
    if _current_inited_client is not None:
        if _current_inited_client.client.project == project_name and _current_inited_client.client.ensure_project_exists == ensure_project_exists:
