...
export interface OtelExtensionOptions {
  /**
   * OTEL Tracer to use for creating spans.
   * Defaults to trace.getTracer('pi-coding-agent-weave-ext') which reads
   * from whatever TracerProvider the user has configured globally.
   */
  tracer?: Tracer;
  /**
   * Whether to attach message text as gen_ai.event.content on span events.
   * When false, events are still emitted (for timing/flow) but without content.
   * Default: true.
   */
