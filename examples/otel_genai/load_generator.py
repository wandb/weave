...
TEMPLATES = {
    "openai_agents": {
        ...
        "tools": [
            ...
            "calculate",
            "send_email",
            "transfer_to_WeatherBot",
            "transfer_to_TravelAdvisor",
            "transfer_to_Translator",
            "transfer_to_ResearchAgent",
        ],
        "turns_range": (3, 7),
        "tools_per_turn": (0, 3),
    },
        ...
        "agents": [
            {
                ...
                "versions": ["0.12.0", "0.13.0", "0.14.0", "0.15.0", "1.0.0"],
...
def make_tool_result(tool_name: str) -> str:
    ...
    if tool_name in {"bash", "run_command", "run_tests"}:
        ...
        return "\n".join(
            f"$ {lorem(random.randint(2, 8))}"
            if i == 0
            else lorem(random.randint(3, 15))
...
def build_conversation(
    ...
    """Build a single conversation payload for /agents/conversations/ingest."""
    ...
    agent_version = rng.choice(agent["versions"])
    ...
    for t_idx in range(n_turns):
        ...
        turn_version = (
            agent_version if turn_agent is agent else rng.choice(turn_agent["versions"])
        )
        ...
        output_text = assistant_text + " ".join(
...
def ingest_conversation(payload: dict, project_id: str) -> dict:
    ...
    url = f"{_url()}/agents/conversations/ingest"
...
def main():
    ...
    print(
        f"  Ingest time:   {ingest_elapsed:.1f}s ({ingested / max(ingest_elapsed, 0.001):.0f} conv/s)"
    )
