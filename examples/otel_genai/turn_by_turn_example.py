...
def main():
    ...
    # Turn 1: investigate
    conv2.system("You are an expert software engineer. Fix bugs by reading code and running tests.")
    ...
    conv2.assistant("The test expects a 200 from /login. Let me check the route handler.")
    conv2.tool_call(
        "read_file",
        arguments={"path": "app/routes/auth.py"},
        result='@app.post("/login")\ndef login(data):\n    if data.user == "admin":\n        return {"token": "..."}, 200\n    return {"error": "unauthorized"}, 401',
    )
    conv2.assistant("I see — the route reads `data.user` but the test sends `user` in the JSON body. The Pydantic model likely expects `username`. Let me check.")
    ...
    conv2.tool_call(
        "edit_file",
        arguments={"path": "app/routes/auth.py", "old": "data.user", "new": "data.username"},
    ...
    conv2.assistant("Fixed! The issue was that the route was reading `data.user` but the model field is `data.username`. Test passes now.")
