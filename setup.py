...
IS_BUILT = (ROOT / "weave_query" / "weave_query" / "frontend" / "assets").is_dir() or SKIP_BUILD
...
def download_frontend() -> None:
    sha = open(ROOT / "weave_query" / "weave_query" / "frontend" / "sha1.txt").read().strip()
    url = f"https://storage.googleapis.com/wandb-cdn-prod/weave/{sha}.tar.gz"
    try:
        download_and_extract_tarball(url, extract_path=ROOT / "weave_query" / "weave_query")
    except HTTPError:
